	...
	public class ZipFile : IEnumerable
	{
		...
		void ReadEntries()
		{
			...
			*/
			...
			for (int i = 0; i < count; i++) {
				...
				entry.offset = offset;
				entries[i] = entry;
				quickEntryFinder.Add(name, entry);	// fred
			}
		}
		private Hashtable quickEntryFinder = new Hashtable (StringComparer.InvariantCultureIgnoreCase);// fred
		...
		int GetEntryIndex(string name)
		{
			for (int i = 0; i < entries.Length; i++) {
				...
				if (string.Compare(name, entries[i].Name, true, CultureInfo.InvariantCulture) == 0)	// FRED - case insensitive compare
				{
					return i;
				}
			}
		...
		public ZipEntry GetEntry(string name)
		{
			...
			return quickEntryFinder[name] as ZipEntry;	// fred
//			int index = GetEntryIndex(name);								// commented by fred
//			return index >= 0 ? (ZipEntry) entries[index].Clone() : null;	// commented by fred
		}
