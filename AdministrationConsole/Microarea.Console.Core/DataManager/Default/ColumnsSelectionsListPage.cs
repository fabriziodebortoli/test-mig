...
namespace Microarea.Console.Core.DataManager.Default
{
	...
	public partial class ColumnsSelectionsListPage : InteriorWizardPage
	{
		...
		private void LoadAvailableTables()
		{
			...
			foreach (CatalogTableEntry catEntry in defaultSel.ExportSel.Catalog.TblDBList)
			{
				...
				if (defaultSel.ExportSel.AllTables || catEntry.Selected)
				...
				catEntry.LoadColumnsInfo(defaultSel.ExportSel.ContextInfo.Connection, true);

				TableNode tblNode = null;
				if (defaultSel.ExportSel.LoadFromConfigurationFile)
					tblNode = defaultSel.ExportSel.ConfigInfo.GetTableInConfigInfo(catEntry.Application, catEntry.Module, catEntry.TableName);

				foreach (CatalogColumn col in catEntry.ColumnsInfo)
				{
					if (defaultSel.ExportSel.LoadFromConfigurationFile && tblNode != null)
					{
						if (tblNode.ExistColumn(col.Name))
						{
							catEntry.AddSelectedColumn(col.Name);
							continue;
						}

						// aggiungo in automatico le colonne obbligatorie
					...
					else
					{
						...
						{
							// aggiungo in automatico le colonne obbligatorie
							if (string.Compare(col.Name, DatabaseLayerConsts.TBCreatedColNameForSql, true, CultureInfo.InvariantCulture) == 0 ||
								string.Compare(col.Name, DatabaseLayerConsts.TBModifiedColNameForSql, true, CultureInfo.InvariantCulture) == 0 ||
								string.Compare(col.Name, DatabaseLayerConsts.TBCreatedIDColNameForSql, StringComparison.InvariantCultureIgnoreCase) == 0 ||
								string.Compare(col.Name, DatabaseLayerConsts.TBModifiedIDColNameForSql, StringComparison.InvariantCultureIgnoreCase) == 0)
							{
								catEntry.AddSelectedColumn(col.Name);
		...
		{
			CatalogTableEntry entry = defaultSel.ExportSel.Catalog.GetTableEntry(text);
		...
		{
			CatalogColumn col = defaultSel.ExportSel.Catalog.GetColumnInfo(textParent, text);
		...
		{
			CatalogTableEntry entry = defaultSel.ExportSel.Catalog.GetTableEntry(textParent);
		...
		private void RemoveSingleItem(ListViewItem item)
		{
			...
			for (int i = 0; i < SourceTblTreeView.Nodes.Count; i++)
			{
				...
				if (string.Compare(selectedNode.Text, table, true, CultureInfo.InvariantCulture) == 0)
				{
					...
					CatalogTableEntry entry = defaultSel.ExportSel.Catalog.GetTableEntry(table);
		...
		{
			CatalogTableEntry entry = defaultSel.ExportSel.Catalog.GetTableEntry(tableName);
			
			StringCollection keys = new StringCollection();
			defaultSel.ExportSel.Catalog.GetPrimaryKeys(tableName, ref keys); 
