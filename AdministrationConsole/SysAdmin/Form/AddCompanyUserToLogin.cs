...
namespace Microarea.Console.Plugin.SysAdmin.Form
{
	...
	/// </summary>
	//=========================================================================
	public partial class AddCompanyUserToLogin : PlugInsForm
	{
		...
		private Diagnostic diagnostic = new Diagnostic("SysAdmin.AddCompanyUserToLogin");
		...
		private bool GrantLogin(UserListItem userItem, bool isCompanyDb)
		{
			...
			TransactSQLAccess connSqlTransact = isCompanyDb ? companyConnSqlTransact : dmsConnSqlTransact;

			if (companyProvider == DBMSType.SQLSERVER)
			{
				...
				else
				{
					...
					if (!connSqlTransact.ExistUserIntoDb(loginName, dbName))
					{
						result =
							connSqlTransact.SPGrantDbAccess(loginName, loginName, dbName) &&
							connSqlTransact.SPAddRoleMember(loginName, DatabaseLayerConsts.RoleDataWriter, dbName) &&
			...
			userItem.DbPassword = isCompanyDb 
								? AssignLoginPasswordForCompany(loginName, loginPassword)
...
/*		public void Save(object sender, System.EventArgs e)
		{
			...
			for (int i = 0; i < ListViewUsersCompany.CheckedItems.Count; i++)
			{
				...
				string loginName	 = currentItemSelected.Login;
				string loginPassword = currentItemSelected.DbPassword;
				bool   isWindowsAuth = currentItemSelected.DbWindowsAuthentication;
				bool   result		 = false;
				...
				if (companyProvider == DBMSType.SQLSERVER)
				{
					...
					if (!connSqlTransact.ExistLogin(loginName))
					{
						if (isWindowsAuth)
						{
							result = 
								...
								connSqlTransact.SPAddRoleMember(loginName, ConstString.roleDataWriter, this.dbCompanyName, connSqlTransact.CurrentConnection) &&
								connSqlTransact.SPAddRoleMember(loginName, ConstString.roleDataReader, this.dbCompanyName, connSqlTransact.CurrentConnection) &&
								connSqlTransact.SPAddRoleMember(loginName, ConstString.roleDbOwner, this.dbCompanyName, connSqlTransact.CurrentConnection);
						}
						else
						{
							result = 
								connSqlTransact.SPAddLogin(loginName, loginPassword, ConstString.masterDatabase, connSqlTransact.CurrentConnection) &&
								connSqlTransact.SPGrantDbAccess(loginName, loginName, this.dbCompanyName, connSqlTransact.CurrentConnection) &&
								connSqlTransact.SPAddRoleMember(loginName, ConstString.roleDataWriter, this.dbCompanyName, connSqlTransact.CurrentConnection) &&
								connSqlTransact.SPAddRoleMember(loginName, ConstString.roleDataReader, this.dbCompanyName, connSqlTransact.CurrentConnection) &&
								connSqlTransact.SPAddRoleMember(loginName, ConstString.roleDbOwner,    this.dbCompanyName, connSqlTransact.CurrentConnection) ;
					...
					{
						//se invece la login esiste controllo che non sia già associato come User al Db
						//else if (!connSqlTransact.IsUserIntoDb(loginName, this.dbCompanyName))
						if (!connSqlTransact.ExistLoginIntoDb(loginName, this.dbCompanyName))
						{
							...
							if (!result)
							{
								//se ho dato errore vado cmq avanti e provo a connettermi
				...
				if (string.Compare(loginName, NameSolverStrings.EasyLookSystemLogin, StringComparison.InvariantCultureIgnoreCase) == 0)
					currentItemSelected.IsAdmin = true;

				currentItemSelected.DbPassword = AssignLoginPasswordForCompany(loginName, loginPassword);

				//potrebbe avere disabled = true se è stato precedentemente cancellato
				currentItemSelected.Disabled = false;
				result = false;
				
				//aggiungo o modifico in MSD_CompanyLogins la riga relativa alla login
				result = (companyUserDb.ExistUser(loginId, this.companyId) == 0)
