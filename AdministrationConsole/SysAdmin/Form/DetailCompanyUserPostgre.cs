...
{
    //=========================================================================
    public partial class DetailCompanyUserPostgre : PlugInsForm
    {
        ...
        Diagnostic diagnostic = new Diagnostic("SysAdmin.DetailCompanyUserPostgre");
        ...
        private PostgreAccess companyConnSqlTransact = new PostgreAccess();
       ...
       // private bool dmsDbOwnerWinAuth = false;
        ...
        //--------------------------------------------------------------------
        public DetailCompanyUserPostgre
        (
        ...
        private bool CheckOneDatabase(bool existLoginOnCompanyServer)
        {
            ...
            {
                if (!companyConnSqlTransact.AddLogin(loginSelected, loginPassword))
                {
                ...
                if (!NewTryToConnectSpecificDB(isLoginNT, loginSelected, loginPassword, DatabaseLayerConsts.postgreMasterDatabase, false))
                {
            ...
            bool existLoginOnCompanyDb = companyConnSqlTransact.ExistLogin(loginSelected);
        ...
        private bool NewAddLogin(bool isLoginNT, string loginName, string loginPassword, bool isDmsManage)
        {
            ...
            PostgreAccess connSqlTransact =/* isDmsManage ? dmsConnSqlTransact :*/ companyConnSqlTransact;

            bool result = false;

            //if (isLoginNT)
            //{
            //    result = (connSqlTransact.SPGrantLogin(loginName, connSqlTransact.CurrentConnection) &&
            //            connSqlTransact.SPGrantDbAccess(loginName, loginName, dbName, connSqlTransact.CurrentConnection) &&
            //            connSqlTransact.SPAddRoleMember(loginName, DatabaseLayerConsts.RoleDataWriter, dbName, connSqlTransact.CurrentConnection) &&
            //            connSqlTransact.SPAddRoleMember(loginName, DatabaseLayerConsts.RoleDataReader, dbName, connSqlTransact.CurrentConnection) &&
            //            connSqlTransact.SPAddRoleMember(loginName, DatabaseLayerConsts.RoleDbOwner, dbName, connSqlTransact.CurrentConnection));
            //}
            //else
            //{
            result = (connSqlTransact.AddLogin(loginName, loginPassword) &&
        ...
        private bool NewGrantLogin(bool isLoginNT, string loginName, string loginPassword, bool isDmsManage)
        {
            ...
            //if (isLoginNT)
            //{
            //    result = (connSqlTransact.SPGrantDbAccess(loginName, loginName, dbName, connSqlTransact.CurrentConnection) &&
            //            connSqlTransact.SPAddRoleMember(loginName, DatabaseLayerConsts.RoleDataWriter, dbName, connSqlTransact.CurrentConnection) &&
            //            connSqlTransact.SPAddRoleMember(loginName, DatabaseLayerConsts.RoleDataReader, dbName, connSqlTransact.CurrentConnection) &&
            //            connSqlTransact.SPAddRoleMember(loginName, DatabaseLayerConsts.RoleDbOwner, dbName, connSqlTransact.CurrentConnection));
            //}
            //else
            //{
            result = (connSqlTransact.GrantDbAccess(loginName) &&
                    connSqlTransact.AddRoleMember(loginName, DatabaseLayerConsts.RoleDataWriter) &&
                    connSqlTransact.AddRoleMember(loginName, DatabaseLayerConsts.RoleDataReader) &&
                    connSqlTransact.AddRoleMember(loginName, DatabaseLayerConsts.RoleDbOwner));
            //}
        ...
        private bool NewTryToConnect(bool isLoginNT, string loginName, string loginPassword, bool isDmsManage)
        {
            ...
            tentativeConnSql.CurrentStringConnection = (isLoginNT)
                ? string.Format(NameSolverDatabaseStrings.PostgreWinNtConnection, dbCompanyServer, port, dbCompanyName, DatabaseLayerConsts.postgreDefaultSchema)
                : string.Format(NameSolverDatabaseStrings.PostgreConnection, dbCompanyServer, port, dbCompanyName, loginName.ToLower(), loginPassword, DatabaseLayerConsts.postgreDefaultSchema);
        ...
        private bool NewTryToConnectSpecificDB(bool isLoginNT, string loginName, string loginPassword, string database, bool isDmsManage)
        {
            ...
            tentativeConnSql.CurrentStringConnection = (isLoginNT)
                ? string.Format(NameSolverDatabaseStrings.PostgreWinNtConnection, dbCompanyServer, port, database, DatabaseLayerConsts.postgreDefaultSchema)
                : string.Format(NameSolverDatabaseStrings.PostgreConnection, dbCompanyServer, port, database, loginName.ToLower(), loginPassword, DatabaseLayerConsts.postgreDefaultSchema);
        ...
        private string CreateCompanyConnectionString()
        {
            ...
            if (companyData != null && companyData.Count > 0)
            {
                ...
                if (userDboData != null && userDboData.Count > 0)
                {
                    ...
                    {
                        connectionToCompanyServer = string.Format(NameSolverDatabaseStrings.PostgreWinNtConnection, this.dbCompanyServer, port, this.dbCompanyName, DatabaseLayerConsts.postgreDefaultSchema);
                        this.dbOwnerDomain = Path.GetDirectoryName(this.dbOwnerLogin);
                    }
                    else
                    {
                        if (string.IsNullOrEmpty(dbOwnerPassword)) dbOwnerPassword = DatabaseLayerConsts.postgreDefaultPassword;
        ...
        private bool ConnectToCompanyDatabase()
        {
            ...
            companyConnSqlTransact.OnAddUserAuthenticatedFromConsole += new PostgreAccess.AddUserAuthenticatedFromConsole(AddUserAuthentication);
            companyConnSqlTransact.OnGetUserAuthenticatedPwdFromConsole += new PostgreAccess.GetUserAuthenticatedPwdFromConsole(GetUserAuthenticatedPwd);
            companyConnSqlTransact.OnIsUserAuthenticatedFromConsole += new PostgreAccess.IsUserAuthenticatedFromConsole(IsUserAuthenticated);
            companyConnSqlTransact.OnCallHelpFromPopUp += new PostgreAccess.CallHelpFromPopUp(CallHelp);
        ...
        public bool RevokeCompanyLogin(string userName, string companyId)
        {
            ...
            {
                result = connSqlTransact.RevokeDbAccess(userName, this.dbCompanyName);
            }
...
//            {
//                return
//                    connSqlTransact.SPGrantLogin(loginName, connSqlTransact.CurrentConnection) &&
//                    connSqlTransact.SPGrantDbAccess(loginName, loginName, this.dbCompanyName, connSqlTransact.CurrentConnection) &&
//                    connSqlTransact.SPAddRoleMember(loginName, DatabaseLayerConsts.RoleDataWriter, this.dbCompanyName, connSqlTransact.CurrentConnection) &&
//                    connSqlTransact.SPAddRoleMember(loginName, DatabaseLayerConsts.RoleDataReader, this.dbCompanyName, connSqlTransact.CurrentConnection) &&
//                    connSqlTransact.SPAddRoleMember(loginName, DatabaseLayerConsts.RoleDbOwner, this.dbCompanyName, connSqlTransact.CurrentConnection);
...
//            {
//                return
//                    connSqlTransact.SPAddLogin(loginName, loginPassword, DatabaseLayerConsts.MasterDatabase, connSqlTransact.CurrentConnection) &&
//                    connSqlTransact.SPGrantDbAccess(loginName, loginName, this.dbCompanyName, connSqlTransact.CurrentConnection) &&
//                    connSqlTransact.SPAddRoleMember(loginName, DatabaseLayerConsts.RoleDataWriter, this.dbCompanyName, connSqlTransact.CurrentConnection) &&
//                    connSqlTransact.SPAddRoleMember(loginName, DatabaseLayerConsts.RoleDataReader, this.dbCompanyName, connSqlTransact.CurrentConnection) &&
//                    connSqlTransact.SPAddRoleMember(loginName, DatabaseLayerConsts.RoleDbOwner, this.dbCompanyName, connSqlTransact.CurrentConnection);
