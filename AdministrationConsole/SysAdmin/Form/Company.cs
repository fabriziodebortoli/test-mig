...
namespace Microarea.Console.Plugin.SysAdmin.Form
{
	...
	/// </summary>
	//=========================================================================
	public partial class Company : PlugInsForm
	{
		...
		private Diagnostic diagnostic = new Diagnostic("SysAdminPlugIn.Company");
		...
		/// </summary>
		//---------------------------------------------------------------------
		public Company(string ownerSystemDb, string connectionString, SqlConnection currentConnection, PathFinder pf, LicenceInfo li)
		{
			InitializeComponent();
			diagnostic.Clear();
		...
		/// </summary>
		//---------------------------------------------------------------------
		public Company(string ownerSystemDb, string connectionString, SqlConnection currentConnection, string id, PathFinder pf, LicenceInfo li)
		{
			InitializeComponent();
			diagnostic.Clear();
			...
			if (!companyDb.GetAllCompanyFieldsById(out fieldsAzienda, id))
			{
				...
				DiagnosticViewer.ShowDiagnostic(diagnostic, OrderType.Date, this);
			...
			if (fieldsAzienda.Count > 0)
			{
				...
				{
					case NameSolverDatabaseStrings.OraOLEDBProvider:
						{
							// macchina dalla quale mi sto connettendo
							ServerNameSystemDb = System.Net.Dns.GetHostName().ToUpper(CultureInfo.InvariantCulture);
							ServerIstanceSystemDb = string.Empty;
							// servizio oracle
							OracleService = companyItem.DbServer;
							// utente e nome azienda
							UserConnected = companyItem.DbName;
							userDbOwnerId = companyItem.DbOwner;
							companyDbName = companyItem.DbName;
							companyName = companyItem.Company;
							break;
						}

                    case NameSolverDatabaseStrings.PostgreOdbcProvider:
                        {
				...
				// il provider non può essere modificato se l'azienda è già stata creata
		...
		private void SetValuesInDBUserControls()
		{
			...
			switch (companyItem.Provider)
			{
				case NameSolverDatabaseStrings.OraOLEDBProvider:
					{
						//carico i dati oracle nello userControl
						CompanyUser dbOwnerOracle = (CompanyUser)dbOwnerData[0];

						DataBaseOracle dbOracle = (DataBaseOracle)DBCompanySettings.Controls[0];
						dbOracle.OnAddUserAuthenticatedFromConsole += new DataBaseOracle.AddUserAuthenticatedFromConsole(AddUserAuthentication);
						dbOracle.OnGetUserAuthenticatedPwdFromConsole += new DataBaseOracle.GetUserAuthenticatedPwdFromConsole(GetUserAuthenticatedPwd);
						dbOracle.OnIsUserAuthenticatedFromConsole += new DataBaseOracle.IsUserAuthenticatedFromConsole(IsUserAuthenticated);
						...
						userDb.GetAllUserFieldsById(out userData, companyItem.DbOwner);

						dbOracle.CompanyId = this.companyId;
						dbOracle.OracleService = OracleService;
						dbOracle.UserDbOwnerId = companyItem.DbOwner;
						dbOracle.LoadData();

						dbOracle.SelectedDbOwnerName = dbOwnerOracle.DBDefaultUser;
						dbOracle.SelectedDbOwnerPwd = dbOwnerOracle.DBDefaultPassword;
						dbOracle.SelectedDbOwnerIsWinNT = dbOwnerOracle.DBWindowsAuthentication;
						dbOracle.SelectedOracleServiceName = OracleService;
						dbOracle.ApplicationUserName = ((UserItem)userData[0]).Login;
						dbOracle.ApplicationUserId = companyItem.DbOwner;
						dbOracle.IsNewOracleCompany = false;

						dboIsEasyBuilderDeveloper = dbOwnerOracle.EasyBuilderDeveloper;

						// StandardEdition (in teoria questa opzione è inutile visto che Oracle non è distribuito con la Standard Ed.)
						if (hideDbChanges)
							dbOracle.HideDbChanges();

						// la combobox del dbo per il db aziendale e' abilitata solo se non gestisco gli slave
						dbOracle.EnableUsersComboBox(!companyItem.UseDBSlave);
						break;
					}
				...
				case NameSolverDatabaseStrings.SQLOLEDBProvider:
					{
						...
						DataBaseSql dbSql = (DataBaseSql)DBCompanySettings.Controls[0];
                ...
                case NameSolverDatabaseStrings.PostgreOdbcProvider:
                    {
                        ...
                        DataBasePostgre dbSql = (DataBasePostgre)DBCompanySettings.Controls[0];
                        dbSql.SelectedPostgreServerName = companyItem.DbServer.Split(Path.DirectorySeparatorChar)[0];
                        dbSql.SelectedPostgreIstanceName = companyItem.DbServer.Split(Path.DirectorySeparatorChar).Length > 1
                                                        ? companyItem.DbServer.Split(Path.DirectorySeparatorChar)[1]
                                                        : string.Empty;
                        dbSql.IsNewPostgreCompany = false;
                        dbSql.SelectedPostgreNewDatabaseName = string.Empty;
                        dbSql.SelectedPostgreDatabaseName = companyItem.DbName;
                        dbSql.CompanyDbName = companyItem.DbName;
                        dbSql.Company = companyItem.Company;
                        dbSql.PostgrePort = companyItem.Port;
			...
			if (companyItem.UseDBSlave)
			{
				...
				DataBaseSql dmsSql = (DataBaseSql)SlavePanel.Controls[0];
		...
		private bool UpdatingTables(UserInfo contextUser, bool isCreation, string preferrendLanguage, string applicationLanguage, bool isValidCompany)
		{
			...
			switch (contextUser.DbType)
			{
                ...
                case DBMSType.POSTGRE:
                    serverOrServiceName = contextUser.ServerComplete;
                    userListItem.DbUser = contextUser.LoginName;
                    contextUser.DatabaseName = contextUser.DatabaseName.ToLower();
                    tbCompany.Text = tbCompany.Text.ToLower();
                    userListItem.DbPassword = contextUser.LoginPassword;
                    break;
				case DBMSType.ORACLE:
					userListItem.DbUser = contextUser.UserOracleName;
					userListItem.DbPassword = contextUser.UserOraclePwd;
					//nel caso di oracle, il nome del server equivale a quello del servizio
					serverOrServiceName = contextUser.OracleService;
					break;
			}
		...
		public bool SaveCompany()
		{
			...
			bool result = false;
			bool isNewDb = false;
			bool isNewOracleUser = false;
			
			sql2012MsgDisplayed = false;

			Control companySetting = DBCompanySettings.Controls[0];
			if (companySetting is DataBaseSql)
			{
				DataBaseSql dbSql = (DataBaseSql)companySetting;
				isNewDb = dbSql.IsNewSQLCompany;
				companyDbName = (isNewDb) ? dbSql.SelectedSQLNewDatabaseName : dbSql.SelectedSQLDatabaseName;
				result = licenceInfo.IsAzureSQLDatabase ? SaveCompanyForAzure() : SaveCompanyForSQLProvider();
			}
			else if (companySetting is DataBaseOracle)
			{
				DataBaseOracle dbOracle = (DataBaseOracle)companySetting;
				isNewDb = dbOracle.IsNewOracleCompany;
				companyDbName = dbOracle.SelectedDbOwnerName;
				result = SaveOracleCompany(out isNewOracleUser);
			}
            else if (companySetting is DataBasePostgre)
            {
                DataBasePostgre dbPostgre = (DataBasePostgre)companySetting;
                isNewDb = dbPostgre.IsNewPostgreCompany;
                companyDbName = dbPostgre.SelectedDbOwnerName;
                result = SavePostgreCompany();
            }
			...
			if (result)
			{
                ...
                if ((companySetting is DataBaseOracle && isNewOracleUser) ||
                    (companySetting is DataBaseSql && isNewDb) ||
                    (companySetting is DataBasePostgre && isNewDb))
				{
		...
		private void BtnDelete_Click(object sender, System.EventArgs e)
		{
			...
			if (DBCompanySettings.Controls.Count > 0)
			{
				...
				else if (DBCompanySettings.Controls[0] is DataBaseOracle)
					DeleteOracleData(sender, e);
                else if (DBCompanySettings.Controls[0] is DataBasePostgre)
                    DeletePostgreData(sender, e);
			}
		...
		private string BuildStringConnection(UserInfo user)
		{
			...
			if (licenceInfo.IsAzureSQLDatabase)
				return string.Format(NameSolverDatabaseStrings.SQLAzureConnection, user.ServerComplete, user.DatabaseName, user.LoginName, user.LoginPassword);
			else
				return
					(user.IsWinNT)
		...
		//---------------------------------------------------------------------
		private string BuildStringConnectionPostgre(UserInfo user)
        {
            ...
            /// @@Anastasia TODO:
            /// Lascio il valore default per la porta. Da rifare
            if (user.DbType == DBMSType.POSTGRE)
                return
                    (user.IsWinNT)
                    ? string.Format(NameSolverDatabaseStrings.PostgreWinNtConnection, user.ServerComplete.ToLower(), port, user.DatabaseName.ToLower(), DatabaseLayerConsts.postgreDefaultSchema)
                    : string.Format(NameSolverDatabaseStrings.PostgreConnection, user.ServerComplete, port, user.DatabaseName.ToLower(), user.LoginName.ToLower(), password, DatabaseLayerConsts.postgreDefaultSchema);
        ...
        ///</summary>
        //---------------------------------------------------------------------
        private bool TryToConnectPostgre(PostgreAccess tentativeConnSql, UserInfo contextUserInfo)
        {
            tentativeConnSql.CurrentStringConnection = BuildStringConnectionPostgre(contextUserInfo);
            return tentativeConnSql.TryToConnect();
        }
        ...
        ///</summary>
        //---------------------------------------------------------------------
        private bool InsertLoginPostgre(PostgreAccess connToServer, UserInfo contextUser)
        {
            bool result = false;
            ...
            {
                result =
                    connToServer.AddRoleMember(contextUser.LoginName, DatabaseLayerConsts.RoleSysAdmin);
            }
            else
            {
                result =
                    connToServer.AddLogin(contextUser.LoginName, contextUser.LoginPassword) && connToServer.AddRoleMember(contextUser.LoginName, DatabaseLayerConsts.postgreSuperUser);
            }
        ...
        ///<summary>
        /// AskCredentialPostgre
        /// Chiede le crenziali per impersonificare un utente
        ///</summary>
        //---------------------------------------------------------------------
        private UserImpersonatedData AskCredentialPostgre
            (
            PostgreAccess connToServer,
            UserInfo contextUser,
            bool canChangeCredential,
            bool askPassword
            )
        ...
        ///<summary>
        /// SetCompanyUserPostgre
        /// Chiede le credenziali di un utente valido in modo da poter inserire l'utente applicativo 
        /// ed associarlo al database aziendale (l'associazione avviene 1 a 1)
        ///</summary>
        //---------------------------------------------------------------------
        private int SetCompanyUserPostgre(PostgreAccess connToServer, UserInfo contextUser)
        {
            string oldConnToServer = connToServer.CurrentStringConnection;

            //mostro la finestra per l'autenticazione - Di default propongo sa
            UserInfo contextUserSuperUserPostgre = new UserInfo();
            contextUserSuperUserPostgre.LoginName = DatabaseLayerConsts.postgreSuperUser;
            contextUserSuperUserPostgre.LoginPassword = string.Empty;
            contextUserSuperUserPostgre.Domain = string.Empty;
            contextUserSuperUserPostgre.ServerPrimary = contextUser.ServerPrimary;
            contextUserSuperUserPostgre.ServerIstance = contextUser.ServerIstance;
            contextUserSuperUserPostgre.DatabaseName = DatabaseLayerConsts.postgreMasterDatabase;
            contextUserSuperUserPostgre.DbType = DBMSType.POSTGRE;

            //Chiedo un utente per effettuare il check
            UserImpersonatedData newCredential = AskCredentialPostgre(connToServer, contextUserSuperUserPostgre, true, true);
            //se l'impersonificazione non è andata a buon fine
            if (newCredential == null)
                return -1; //ho premuto cancel
            ...
            userForConnection.LoginPassword = newCredential.Password;
            userForConnection.Domain = newCredential.Domain;
            userForConnection.DbType = DBMSType.POSTGRE;
            //mi connetto al master database
            userForConnection.DatabaseName = DatabaseLayerConsts.postgreMasterDatabase;
            //il resto lo setto come l'utente
            userForConnection.ServerIstance = contextUser.ServerIstance;
            userForConnection.ServerPrimary = contextUser.ServerPrimary;

            //costruisco la stringa di connessione
            if (TryToConnectPostgre(connToServer, userForConnection))
            {
                bool ended = false;
                if (userForConnection.LoginName == contextUser.LoginName)
                {
                    contextUser.LoginPassword = userForConnection.LoginPassword;
                    connToServer.CurrentStringConnection = BuildStringConnectionPostgre(contextUser);
                    ended = true;
                }
                ...
                {
                    //Ok (ha senso richiedere le credenziali di sa???  o bisogna usare userForConnection.LoginName
                    if (contextUser.LoginName == DatabaseLayerConsts.postgreSuperUser)
                    {
                        //l'utente è sa ma ho dato una pwd sbagliata
                        UserImpersonatedData userSuperUserCredential = AskCredentialPostgre(connToServer, contextUser, false, true);
                        if (userSuperUserCredential != null)
                        {
                            contextUser.Impersonification = userSuperUserCredential;
                            contextUser.LoginPassword = userSuperUserCredential.Password;
                            connToServer.CurrentStringConnection = BuildStringConnectionPostgre(contextUser);
                            ended = true;
                        }
                        else
                    ...
                    {
                        //l'utente imputato esiste è sbagliata la pwd
                        UserImpersonatedData userCredential = AskCredentialPostgre(connToServer, contextUser, false, true);
                        if (userCredential != null)
                        {
                            contextUser.LoginPassword = userCredential.Password;
                            contextUser.Impersonification = userCredential;
                            connToServer.CurrentStringConnection = BuildStringConnectionPostgre(contextUser);
                            if (CheckHasRoleSysAdminPostgre(contextUser, connToServer))
                                ended = true;
                        }
                        else
                        {
                    ...
                    else
                    {
                        ...
                        {
                            ended = InsertLoginPostgre(connToServer, contextUser);
                            if (!ended)
                            {
                                if (connToServer.Diagnostic.Error || connToServer.Diagnostic.Warning || connToServer.Diagnostic.Information)
                                    diagnostic.Set(connToServer.Diagnostic);
        ...
        ///<summary>
        /// CheckRoleDbOwnerPostgre
        /// Controllo i ruoli associati ad un utente e, se le credenziali sono corrette, gli associo quelli mancanti
        ///</summary>
        //---------------------------------------------------------------------------
        private int CheckRoleDbOwnerPostgre(UserInfo currentUser, PostgreAccess currentConnection)
        {
            bool result = false;

            if (!currentConnection.LoginIsSystemAdminRole(currentUser.LoginName))
            {
                string messageForPermissionUser = string.Format(Strings.NoPermissionUser, currentUser.LoginName);
                diagnosticViewer.Message = messageForPermissionUser;
                diagnosticViewer.Title = Strings.Error;
                ...
                //Siccome non ha un ruolo sysadmin devo chiedere nuove credenziali
                result = AskAndTestNewCredentialPostgre
                    (
                    currentUser.ServerComplete,
                    DatabaseLayerConsts.postgreSuperUser,
                    string.Empty,
                    out ownerLogin,
                    out ownerPwd,
                    out ownerDomain,
                ...
                result = currentConnection.AddRoleMember(currentUser.LoginName, DatabaseLayerConsts.postgreSuperUser);
                currentConnection.CurrentStringConnection = oldString;
        ...
        /// <summary>
        /// POSTGRE
        /// CheckHasRoleSysAdminPostgre
        /// Verifica che l'utente  SysAdmin (nel caso lo aggiunge su richiesta)
        /// </summary>
        //---------------------------------------------------------------------
        private bool CheckHasRoleSysAdminPostgre(UserInfo contextUser, PostgreAccess currentConnection)
        {
            bool result = false;

            if (!currentConnection.LoginIsSystemAdminRole(contextUser.LoginName))
            {
                string messageForPermissionUser = string.Format(Strings.NoPermissionUser, contextUser.LoginName);
                diagnosticViewer.Message = messageForPermissionUser;
                diagnosticViewer.Title = Strings.Error;
                ...
                result = currentConnection.AddRoleMember(contextUser.LoginName, DatabaseLayerConsts.RoleSysAdmin);
        ...
        ///<summary>
        ///POSTGRE
        /// AskAndTestNewCredentialPostgre
        /// Chiede e verifica le nuove credenziali inserite
        ///</summary>
        //---------------------------------------------------------------------
        private bool AskAndTestNewCredentialPostgre
        (
            string serverName,
            string logiName,
            string loginPassword,
            ...
            string primarySQLServer,
            string istanceSQLSever,
            PostgreAccess currentConnection
        )
        {
            string ownerLog = logiName;
            string ownerPwd = loginPassword;
            ...
            if (newCredential != null)
            {
                string password = DatabaseLayerConsts.postgreDefaultPassword ;
                if (!string.IsNullOrEmpty(newCredential.Password)) password = newCredential.Password;
                //ora provo a connettermi
                currentConnection.CurrentStringConnection =
                    (newCredential.WindowsAuthentication)
                    ...
                    : string.Format(NameSolverDatabaseStrings.PostgreConnection, serverName, port, DatabaseLayerConsts.postgreMasterDatabase, newCredential.Login.ToLower(), password, DatabaseLayerConsts.postgreDefaultSchema);
		...
		public void LoadDefaultData()
		{
			...
			if (DBCompanySettings.Controls.Count > 0)
			{
                ...
                {
                    DataBasePostgre dbPostgre = (DataBasePostgre)DBCompanySettings.Controls[0];
                    dbPostgre.LoadData(false);

                    // StandardEdition (in teoria questa opzione è inutile visto che Oracle non è distribuito con la Standard Ed.)
                    if (hideDbChanges)
                        dbPostgre.HideDbChanges();
                }
			}
		}
		#endregion
		...
		private void PopolateComboProvider(string providerToSelect)
		{
			...
			// M.6176 rimane comunque non possibile utilizzare Oracle
			// anche in caso di licenza per Azure e' ammesso solo il provider SQL
			if (licenceInfo.DBNetworkType == DBNetworkType.Small || licenceInfo.IsAzureSQLDatabase)
			{
				providerToSelect = DatabaseLayerConsts.SqlOleProviderDescription;
		...
		/// <summary>
		/// CheckOracleCompanyValidator
		/// Check validità dei dati inseriti nella form per Oracle
		/// </summary>
		//---------------------------------------------------------------------
		private bool CheckOracleCompanyValidator(string oracleServiceName, string oracleOwnerName, string oracleOwnerPwd, bool oracleOwnerIsWinNT)
		{
			bool result = true;
			...
			{
				diagnostic.Set(DiagnosticType.Error, Strings.EmptyCompany);
				result = false;
			}
			...
			if (oracleServiceName.Trim().IndexOfAny(new char[] { '?', '*', '"', '$', '&', '(', ')', '=', '[', '#', ']', Path.DirectorySeparatorChar, '<', '>', ':', '!', '|' }) != -1)
			{
				diagnostic.Set(DiagnosticType.Warning, Strings.WrongCharactersInOracleServiceName);
				result = false;
			}
			...
			if (oracleOwnerName.Trim().IndexOfAny(new char[] { '?', '*', '"', '$', '&', '/', '(', ')', '=', '[', '#', ']', '<', '>', ':', '!', '|' }) != -1)
			{
				diagnostic.Set(DiagnosticType.Warning, Strings.WrongCharactersInOracleOwnerName);
				result = false;
			}
			if (string.IsNullOrWhiteSpace(oracleOwnerName))
			{
				diagnostic.Set(DiagnosticType.Warning, Strings.EmptyOracleOwnerName);
				result = false;
			}
			if (!oracleOwnerIsWinNT && oracleOwnerName.CountChar(Path.DirectorySeparatorChar) != 0)
			{
				diagnostic.Set(DiagnosticType.Warning, Strings.ForbiddedNtUser);
				result = false;
			}
			if (!oracleOwnerIsWinNT && string.IsNullOrWhiteSpace(oracleOwnerPwd))
			{
		...
		/// </summary>
		//---------------------------------------------------------------------
		private bool CheckSqlValidator(DataBaseSql dbSql)
		{
			bool result = true;
			...
			if (this.DBCompanySettings.Controls.Count > 0)
				dbSql = (DataBaseSql)this.DBCompanySettings.Controls[0];
        ...
        /// <summary>
        /// CheckPostgreCompanyValidator
        /// Check validità dei dati inseriti nella form per Postgre
        /// </summary>
        //---------------------------------------------------------------------
        private bool CheckPostgreValidator(DataBasePostgre dbSql)
        {
            bool result = true;
            ...
            if (this.DBCompanySettings.Controls.Count > 0)
                dbSql = (DataBasePostgre)this.DBCompanySettings.Controls[0];
            ...
            if (dbSql != null)
            {
                ...
                port = dbSql.PostgrePort;


                if (dbSql.IsNewPostgreCompany)
                {
                    if (!string.IsNullOrWhiteSpace(dbSql.SelectedPostgreNewDatabaseName) &&
                        (dbSql.SelectedPostgreNewDatabaseName.IndexOfAny(
                        new char[] { '?', '*', Path.DirectorySeparatorChar, '/', '<', '>', ':', '!', '|', '+', '[', ']', ',', '@', '=' }) != -1))
                    {
                        diagnostic.Set(DiagnosticType.Warning, Strings.WrongCharactersInDatabaseName);
                        result = false;
                    ...
                    else if (!IsValidDatabaseNameForSQL(dbSql.SelectedPostgreNewDatabaseName))
                    {
                        // controllo che il nome del database non corrisponda ad uno di quelli di sistema/esempio
                        diagnostic.Set(DiagnosticType.Warning, string.Format(Strings.InvalidDatabase, dbSql.SelectedPostgreNewDatabaseName));
                        result = false;
                    }
                }
                else
                    if (string.IsNullOrWhiteSpace(dbSql.SelectedPostgreDatabaseName))
                    {
                        diagnostic.Set(DiagnosticType.Warning, Strings.NotSelectedDatabase);
                        result = false;
                    }
		...
		private bool CheckDMSValidator()
		{
			...
			DataBaseSql dmsDb = null;
			if (SlavePanel.Controls.Count > 0)
			{
				if (SlavePanel.Controls[0] is DataBaseSql)
					dmsDb = (DataBaseSql)SlavePanel.Controls[0];
			}
...
/*			if (dmsDb.SelectedDbOwnerIsWinNT)
			{
				diagnostic.Set(DiagnosticType.Warning, "The dbowner of Easy Attachment database cannot be a user in Windows Auth! Please select another one!");
				result = false;
			}*/
			...
			// in base al provider selezionato carico le info dello usercontrol corretto
			ProviderItem selectedProvider = (ProviderItem)cbProvider.SelectedItem;
			if (selectedProvider != null)
			{
				DataBaseSql dbSql = null;
				...
				{
					if (DBCompanySettings.Controls.Count > 0 && DBCompanySettings.Controls[0] is DataBaseSql)
						dbSql = (DataBaseSql)DBCompanySettings.Controls[0];
				}
				...
				if (dbSql != null)
				{
					...
					{
						string dmsDbName = dmsDb.IsNewSQLCompany ? dmsDb.SelectedSQLNewDatabaseName : dmsDb.SelectedSQLDatabaseName;
						string companyDbName = dbSql.IsNewSQLCompany ? dbSql.SelectedSQLNewDatabaseName : dbSql.SelectedSQLDatabaseName;
		...
		#region Funzioni di salvataggio azienda (per tipo provider)
		///<summary>
		/// SaveOracleCompany - salvataggio di un'azienda con provider Oracle
		///</summary>
		//---------------------------------------------------------------------
		private bool SaveOracleCompany(out bool isAddedOracleUser)
		{
			isAddedOracleUser = false;
			bool isSaved = true;
			//bool isNewUser = false;

			diagnostic.Clear();
			//disabilito il bottone di save
			BtnSave.Enabled = false;
			//Cursore in stato di attesa
			Cursor.Current = Cursors.WaitCursor;

			//Collezione delle info x Oracle
			DataBaseOracle dbOracle = (DataBaseOracle)DBCompanySettings.Controls[0];
			string dbOracleOwnerName = dbOracle.SelectedDbOwnerName;
			string dbOracleOwnerPwd = dbOracle.SelectedDbOwnerPwd;
			bool dbOracleOwnerIsWinNT = dbOracle.SelectedDbOwnerIsWinNT;
			string dbOracleServiceName = string.IsNullOrWhiteSpace(dbOracle.SelectedOracleServiceName) ? OracleService : dbOracle.SelectedOracleServiceName;
			bool isNewOracleUser = dbOracle.IsNewOracleUser;

			if (CheckOracleCompanyValidator(dbOracleServiceName, dbOracleOwnerName, dbOracleOwnerPwd, dbOracleOwnerIsWinNT))
			{
				//se l'utente esiste devo effettuare il check della connessione altrimento no
				if ((!isNewOracleUser && dbOracle.CheckOracleConnection(true)) || isNewOracleUser)
				{
					if (isNewOracleUser)
					{
						if (!AddNewOracleUser(dbOracleServiceName, dbOracleOwnerName, dbOracleOwnerPwd, dbOracleOwnerIsWinNT))
						{
							//serve perchè, se l'utente è stato inserito ed è quindi "nuovo" è necessario
							//creare la struttura di db (viene chiesto), altrimenti no
							isAddedOracleUser = false;
							isSaved = false;
							//riabilito il bottone di save
							BtnSave.Enabled = true;
							//Cursore attivo
							Cursor.Current = Cursors.Default;
							return isSaved;
					...
					// lingue impostate
					string preferredLanguage = (cultureUICombo.SelectedItem != null)
											? cultureUICombo.SelectedLanguageValue
											: string.Empty;
					...
					//nel caso di Oracle, il  nome del database = nome dell'utente applicativo
					contextUser.DatabaseName = dbOracleOwnerName;
					contextUser.LoginId = dbOracle.ApplicationUserId;
					contextUser.LoginName = dbOracle.ApplicationUserName;
					contextUser.UserOracleName = dbOracleOwnerName;
					contextUser.UserOraclePwd = dbOracleOwnerPwd;
					contextUser.OracleService = dbOracleServiceName;
					contextUser.DbType = DBMSType.ORACLE;
					contextUser.Domain = (dbOracleOwnerIsWinNT) ? contextUser.LoginName.Split(Path.DirectorySeparatorChar)[0] : string.Empty;

					string companyConnection =
							(dbOracleOwnerIsWinNT)
							? string.Format(NameSolverDatabaseStrings.OracleWinNtConnection, contextUser.OracleService)
							: string.Format(NameSolverDatabaseStrings.OracleConnection, contextUser.OracleService, contextUser.UserOracleName, contextUser.UserOraclePwd);

					if (TBCheckDatabase.GetDatabaseVersion(companyConnection, NameSolverDatabaseStrings.OraOLEDBProvider) != DatabaseVersion.Oracle)
					{
						diagnostic.Set(DiagnosticType.Error, Strings.WrongOracleDatabaseVersion);
						isSaved = false;
						if (contextUser.Impersonification != null)
							contextUser.Impersonification.Undo();
					}

					contextUser.UseUnicode = cbUseUnicode.Checked;
					bool setIsValid = true;

					// se non è un nuovo utente effettuo il controllo unicode sul database
					if (!isNewOracleUser)
					{
						// costruisco la stringa di connessione al db oracle
						string connToOracleDb = (dbOracleOwnerIsWinNT)
							? string.Format(NameSolverDatabaseStrings.OracleWinNtConnection, contextUser.OracleService)
							: string.Format(NameSolverDatabaseStrings.OracleConnection, contextUser.OracleService, contextUser.UserOracleName, contextUser.UserOraclePwd);
						...
						OnCheckDBRequirementsUsed(connToOracleDb, DBMSType.ORACLE, contextUser.UseUnicode, out isUnicode, out italianTableName);

						# region Check valore Unicode
						if (isUnicode != contextUser.UseUnicode)
						{
							//errore 
							string dbUnicodeType = (isUnicode) ? Strings.DbIsUnicode : Strings.DbIsNotUnicode;
							string companyUnicodeType = (contextUser.UseUnicode) ? Strings.CompanyIsUnicode : Strings.CompanyIsNotUnicode;
							string detail = string.Format(Strings.WrongUnicodeSetting, (contextUser.UseUnicode) ? Strings.UnicodeSetting : Strings.NotUnicodeSetting);
					...
					// CONTROLLO DI COMPATIBILITA' DELLA DATABASECULTURE PER ORACLE
					CompatibilityOracleDatabaseCulture
						(dbOracle.SelectedDbLanguage, dbOracle.SelectedDbTerritory, companyConnection, contextUser.UseUnicode);

					isSaved = UpdatingTables(contextUser, dbOracle.IsNewOracleCompany, preferredLanguage, applicationLanguage, setIsValid);
				}
				else
					isSaved = false;
			}
			...
			bool isSavedDMS = true;
			// la creazione dell'azienda e' andata a buon fine, quindi posso procedere
			// alla gestione dell'eventuale database documentale
			if (isSaved)
			{
				// solo se il modulo e' attivato
				// se ho scelto di creare il database documentale, eseguo la creazione e poi inserisco i dati nel db di sistema
				// altrimenti, solo se esistono riferimenti nel database di sistema, li elimino
				if (isDMSActivated)
					isSavedDMS = UseDBSlave.Checked ? SaveDMS() : DeleteSlave();
				else
				{
					// se il modulo non e' attivato e mi ha tolto il flag allora elimino i riferimenti nel database di sistema
					// se ha lasciato il flag invece non salvo niente
					if (!UseDBSlave.Checked)
						isSavedDMS = DeleteSlave();
				}
			...
			if (isSaved && isSavedDMS) // se tutti i salvataggi sono andati a buon fine allora ridisegno il tree e riabilito i pulsanti
			{
			...
			if (dbOracle.Diagnostic.Error || dbOracle.Diagnostic.Information || dbOracle.Diagnostic.Warning)
			{
				diagnostic.Set(dbOracle.Diagnostic);
				dbOracle.Diagnostic.Clear();
			}

			//riabilito il bottone di save
			BtnSave.Enabled = true;
			Cursor.Current = Cursors.Default;
			return isSaved && isSavedDMS;
		}

		/// <summary>
		/// AddNewOracleUser
		/// Inserisce un nuovo utente Oracle
		/// </summary>
		//---------------------------------------------------------------------
		private bool AddNewOracleUser
			(
			string oracleService,
			string schemaOwner,
			string schemaOwnerPwd,
			bool schemaOwnerIsWinNt
			)
		{
			bool result = false;
			...
			else
			{
				...
				try
				{
					...
					if (schemaOwnerIsWinNt)
						oracleAccess.CreateNTUser(schemaOwner, schemaOwnerPwd);
					else
						oracleAccess.CreateUser(schemaOwner, schemaOwnerPwd);
				...
				catch (TBException tbExc)
				{
					...
					extendedInfo.Add(DatabaseLayerStrings.Library, "Microarea.SysAdmin.Form.Company");
					extendedInfo.Add(DatabaseLayerStrings.Function, "AddNewOracleUser");
					...
					diagnostic.Set
						...
						(tbExc.Number == 1920) ? string.Format(Strings.OracleUserAlreadyExists, schemaOwner) : tbExc.Message,
						extendedInfo
						);
					oracleAccess.CloseConnection();
					result = false;
        ...
        ///<summary>
        ///    private bool SavePostgreCompany()- salvataggio di un'azienda con provider Postgre
        ///</summary>
        //---------------------------------------------------------------------
        private bool SavePostgreCompany()
        {
            diagnostic.Clear();
            ...
            if (!CheckPostgreValidator(dbPostgre))
            {
                BtnSave.Enabled = true;
                Cursor.Current = Cursors.Default;
                return false;
            ...
            //--------------------------------------------------------------
            PostgreAccess transactCompanyDb = new PostgreAccess();
            UserInfo contextUserCompanyDb = new UserInfo();

            string dbSqlDatabaseName = (dbPostgre.IsNewPostgreCompany) ? dbPostgre.SelectedPostgreNewDatabaseName : dbPostgre.SelectedPostgreDatabaseName;
            ...
            if (!CheckPostgreCompanyDbSettings(dbPostgre, dbSqlDatabaseName, transactCompanyDb, contextUserCompanyDb))
            {
            ...
            if (isDMSActivated && UseDBSlave.Checked)
            {
                ...
                string dmsDatabaseName = (dmsSql.IsNewSQLCompany) ? dmsSql.SelectedSQLNewDatabaseName : dmsSql.SelectedSQLDatabaseName;
            ...
            if (dbPostgre.IsNewPostgreCompany)
            {
                bool useUnicode = false;

                bool resultCreationDB = CreateDatabasePostgre(contextUserCompanyDb, false, out useUnicode,  false);
                if (!resultCreationDB)
                {
                    if (contextUserCompanyDb.Impersonification != null)
                        contextUserCompanyDb.Impersonification.Undo();
                ...
                else
                {
                    ...
                    isCompanyDbSaved = resultCreationDB &&
                        UpdatingTables(contextUserCompanyDb, dbPostgre.IsNewPostgreCompany, preferredLanguage, applicationLanguage, true);
                }
            }
            else
            {
                ...
                if (isDMSActivated)
                {
                    ...
                    if (UseDBSlave.Checked)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                // togliere questo msg inutile!
                                /*string message = string.Format(Strings.DifferentDbo, contextUserDMSDb.DatabaseName, dboOfDataBase, contextUserDMSDb.LoginName);
                                diagnosticViewer.Message = message;
                                diagnosticViewer.Title = Strings.ChangeDbo;
                                ...
                                /*else
                                {
                                    // il database Easy Attachment non deve fare modifiche sugli utenti!
                        ...
                        // alla fine di tutto, devo rimettere a posto le login di SQL e gli utenti associati al database
                        // sul server SQL, perchè potrebbe essere stato cambiato dall'utente.
                        isDMSDbSaved = isDMSDbSaved && ManageUsersAndLoginsForDMS(transactDMSDb, contextUserDMSDb.DatabaseName);
                    }
		...
		/// <summary>
		/// CompatibilityOracleDatabaseCulture - Solo per i database Oracle
		/// Dati un linguaggio e un territorio ritornati dal server Oracle cerco un lcid compatibile
		/// da memorizzare sul record della company
		/// </summary>
		/// <param name="connString">connection string for database</param>
		/// <param name="language">language set on Oracle server</param>
		/// <param name="territory">territory set on Oracle server</param>
		//---------------------------------------------------------------------
		private void CompatibilityOracleDatabaseCulture(string language, string territory, string connString, bool isUnicode)
		{
			companyLcid =
				DBGenericFunctions.AssignOracleDatabaseCultureValue
				(
				this.aPathFinder,
				this.licenceInfo.IsoState,
				language,
			...
			supportColsCollation =
				DBGenericFunctions.CalculateSupportColumnCollation(connString, companyLcid, DBMSType.ORACLE, isUnicode);
		}
		# endregion
		...
		private bool CreateDatabase(UserInfo contextUser, bool showUnicodeCheck, out bool useUnicode, bool showDialog, bool disableDBCultureComboBox)
		{
			...
			string connString =  (licenceInfo.IsAzureSQLDatabase) 
				? string.Format(NameSolverDatabaseStrings.SQLAzureConnection, serverName, DatabaseLayerConsts.MasterDatabase, contextUser.LoginName, contextUser.LoginPassword)
				: (!string.IsNullOrWhiteSpace(contextUser.Domain))
						? string.Format(NameSolverDatabaseStrings.SQLWinNtConnection, serverName, DatabaseLayerConsts.MasterDatabase)
		...
		/// <summary>
		/// Creazione del database (Postgre)
		/// Richiama la form con i dettagli per la creazione del database aziendale
		/// </summary>
		//---------------------------------------------------------------------
		private bool CreateDatabasePostgre(UserInfo contextUser, bool showUnicodeCheck, out bool useUnicode, bool disableDBCultureComboBox)
		{
			Cursor.Current = Cursors.WaitCursor;

			// in fase di creazione database devo proporre una culture per il database 
			// da visualizzare poi nella CreateDBForm
			// ma se ho scelto di disabilitare la combobox significa che sto gestendo il documentale e allora 
			// devo inizializzarla con quella prescelta per l'azienda
			string initializeDBCulture = string.Empty;
			if (disableDBCultureComboBox)
			...
			// carico nella form i parametri per la creazione del database
			CreateDBForm createDBForm = new CreateDBForm
			(
				true,                           // isCompanyDB
				contextUser.ServerPrimary,      // serverName
			...
			// creo il database in modalita' silente 
			createDBForm.CreateDatabasePostgre();
			if (createDBForm.Result)
				companyDBCulture = createDBForm.DatabaseCulture;
			...
			//lo reimposto al termine della creazione
			Cursor.Current = Cursors.WaitCursor;
		...
		/// <summary>
		/// DeleteOracleData
		/// Cancello le anagrafiche ed eventualmente il db + gli eventuali sinonimi su di esso
		/// piu' quello documentale se gestito
		/// </summary>
		//---------------------------------------------------------------------
		public bool DeleteOracleData(object sender, System.EventArgs e)
		{
			...
			DeleteCompany dc = new DeleteCompany(companyItem.UseDBSlave, isDMSActivated);
			...
			if (dc.DeleteCompanyDB)
			{
				...
				//leggo tutti gli utenti associati a quella azienda
				ArrayList usersOfCompany = new ArrayList();
				userOwnerdb.SelectAllDistinct(out usersOfCompany, companyId);

				//procedo a cancellare i synomini per gli utenti associati (se ce ne sono)
				if (usersOfCompany.Count > 0)
				{
					OracleAccess oracleAccess = new OracleAccess();
					oracleAccess.NameSpace = "Module.MicroareaConsole.SysAdmin";
					oracleAccess.OnAddUserAuthenticatedFromConsole += new OracleAccess.AddUserAuthenticatedFromConsole(this.AddUserAuthentication);
					oracleAccess.OnGetUserAuthenticatedPwdFromConsole += new OracleAccess.GetUserAuthenticatedPwdFromConsole(this.GetUserAuthenticatedPwd);
					oracleAccess.OnIsUserAuthenticatedFromConsole += new OracleAccess.IsUserAuthenticatedFromConsole(this.IsUserAuthenticated);
					oracleAccess.OnCallHelpFromPopUp += new OracleAccess.CallHelpFromPopUp(SendHelp);
					...
					else
					{
						...
						try
						{
							...
							for (int i = 0; i < usersOfCompany.Count; i++)
							{
								...
								CompanyUser currentCompanyUser = (CompanyUser)usersOfCompany[i];
								//è il dbo
								if (userOwnerdb.IsDbo(currentCompanyUser.LoginId, currentCompanyUser.CompanyId))
									resultDeletion = oracleAccess.DeleteSchema(currentCompanyUser.DBDefaultUser);
								else //è un utente associato
									resultDeletion = oracleAccess.DropSynonym(dataSourceName, currentCompanyUser.DBDefaultUser);

								if (!resultDeletion)
								{
									if (oracleAccess.Diagnostic.Error || oracleAccess.Diagnostic.Information || oracleAccess.Diagnostic.Warning)
										diagnostic.Set(oracleAccess.Diagnostic);
									else
										diagnostic.Set(DiagnosticType.Error, string.Format(Strings.CannotDeleteCompanyUser, currentCompanyUser.DbUser));
								}
								else
									companyDbDeleted = true;
							}
						}
						catch (TBException excTb)
						{
							ExtendedInfo extendedInfo = new ExtendedInfo();
							extendedInfo.Add(Strings.Description, excTb.Message);
							extendedInfo.Add(Strings.Details, excTb.ExtendedMessage);
							if (excTb.Number == 1940) // con questo codice visualizzo un warning
								diagnostic.Set(DiagnosticType.Warning, Strings.CannotDeleteCompany, extendedInfo);
							else
			...
			if (dc.DeleteDMSDB && currentCompany.UseDBSlave && isDMSActivated)
			{
				...
				string serverName = dbSlaveItem.ServerName;
				string dataSourceName = dbSlaveItem.DatabaseName;
				string ownerId = dbSlaveItem.SlaveDBOwner;
				...
				string[] serverInstance = serverName.Split(Path.DirectorySeparatorChar);
				...
				// istanzio la classe per l'impersonificazione
			...
			// cancello le sole anagrafiche dal db di sistema
			bool companyDelete = DeleteOracleCompany(sender, e);
			...
			// se non sono riuscita ad eliminare qualcosa mostro il diagnostico
			...
			// se ho cancellato almeno l'anagrafica azienda, devo ridisegnare la porzione di tree
			// interessata e mandare un evento agli altri plugIns
			if (companyDelete)
			{
		...
		///<summary>
		/// DeleteOracleCompany
		/// Cancella dal db di sistema l'azienda Oracle
		///</summary>
		//---------------------------------------------------------------------
		private bool DeleteOracleCompany(object sender, System.EventArgs e)
		{
			bool isDeleted = false;
			...
			if (companyDb.Delete(tbCompanyId.Text))
			{
				diagnostic.Set(DiagnosticType.Information, Strings.CompanyIsDeleted);
				isDeleted = true;
			}
			else
			{
        ...
        /// <summary>
        /// DeletePostgreData
        /// Cancellazione azienda per Postgre:
        /// 1. anagrafica nel db di sistema (SEMPRE)
        /// 2. database aziendale (su richiesta)
        /// 3. database documentale (su richiesta e se il modulo e' attivato)
        ///</summary>
        //---------------------------------------------------------------------
        public bool DeletePostgreData(object sender, System.EventArgs e)
        {
            bool companyDbDeleted = false;
            bool dmsDbDeleted = false;

            // visualizzo la form con le opzioni di cancellazione
            DeleteCompany dc = new DeleteCompany(companyItem.UseDBSlave, false);
            //dc.OnIsActivated += new DeleteCompany.IsActivated(deleteCompany_OnIsActivated);
            ...
            connSqlTransact.NameSpace = "Module.MicroareaConsole.SysAdmin";
            connSqlTransact.OnAddUserAuthenticatedFromConsole += new PostgreAccess.AddUserAuthenticatedFromConsole(this.AddUserAuthentication);
            connSqlTransact.OnGetUserAuthenticatedPwdFromConsole += new PostgreAccess.GetUserAuthenticatedPwdFromConsole(this.GetUserAuthenticatedPwd);
            connSqlTransact.OnIsUserAuthenticatedFromConsole += new PostgreAccess.IsUserAuthenticatedFromConsole(this.IsUserAuthenticated);
            connSqlTransact.OnCallHelpFromPopUp += new PostgreAccess.CallHelpFromPopUp(SendHelp);
            connSqlTransact.CurrentStringConnection = connectionString;
            ...
            string dataSourceName = currentCompany.DbName;
            string ownerId = currentCompany.DbOwner;
            int companyPort = currentCompany.Port;
            //lo espongo per comunicarlo al lock manager se necessario
            companyDbName = currentCompany.DbName;
            ...
            // non esiste il dbowner e non procedo
            if (userData.Count == 0) //continene login e password per connection string postgre
                return companyDbDeleted;

            //password e login per la costruzione di connection string di postgre
            CompanyUser currentUser = (CompanyUser)userData[0];

            connSqlTransact.CurrentStringConnection = string.Format(NameSolverDatabaseStrings.PostgreConnection, serverName, companyPort, dataSourceName, ownerId.ToLower(), currentUser.Password == "" ? DatabaseLayerConsts.postgreDefaultPassword : currentUser.Password, DatabaseLayerConsts.postgreDefaultSchema);

            connSqlTransact.OpenConnection();
            ...
            if (dc.DeleteDMSDB && currentCompany.UseDBSlave && dc.IsDMSActivated)
            {
                ...
                {
                    //@@ Anastasia per adesso non gestisco win auth
                    //// se l'utente dbowner e' in win auth effettuo impersonificazione silente
                    //// senza richiedere le credenziali di connessione (accedo con l'utente DmsSqlUser)
                    //dataToConnectionServer = connSqlTransact.LoginImpersonification
                    //    (
		...
		public void BuildTabDatabaseSettings()
		{
			if (cbProvider.SelectedItem != null)
			{
				...
				switch (selectedProvider.ProviderValue)
				{
					case NameSolverDatabaseStrings.OraOLEDBProvider:
						{
							DataSettingsGroupBox.Visible = false;

							if (DBCompanySettings.Controls.Count > 0)
							{
								if (DBCompanySettings.Controls[0] is DataBaseOracle)
									return;
								else
							...
							dbOracle.OnCallHelp += new DataBaseOracle.CallHelp(SendHelp);
							dbOracle.OnAddUserAuthenticatedFromConsole += new DataBaseOracle.AddUserAuthenticatedFromConsole(AddUserAuthentication);
							dbOracle.OnGetUserAuthenticatedPwdFromConsole += new DataBaseOracle.GetUserAuthenticatedPwdFromConsole(GetUserAuthenticatedPwd);
							dbOracle.OnIsUserAuthenticatedFromConsole += new DataBaseOracle.IsUserAuthenticatedFromConsole(IsUserAuthenticated);
							dbOracle.OnSetValueUsersComboBox += new DataBaseOracle.SetValueUsersComboBox(dbSql_OnSetValueUsersComboBox);
							dbOracle.OnUnableToContinue += new EventHandler(dbOracle_OnUnableToContinue);

							dbOracle.ServerNameSystemDb = this.ServerNameSystemDb;
							dbOracle.ServerIstanceSystemDb = this.ServerIstanceSystemDb;
							dbOracle.OracleService = this.OracleService;
							//dbOracle.SelectedDbOwnerName = userConnected;
							dbOracle.OracleCompanyName = (inserting) ? tbCompany.Text : companyDbName;
							dbOracle.UserConnected = (UserConnected.Length == 0) ? UserSystemConnected : UserConnected;
							dbOracle.UserDbOwnerId = userDbOwnerId;
							dbOracle.CompanyId = this.companyId;
							dbOracle.ConnectionString = this.connectionString;
							dbOracle.CurrentConnection = this.currentConnection;
							dbOracle.IsNewOracleCompany = inserting;
							dbOracle.SettingLayout();
							dbOracle.LoadData();

							// disable checkbox UseDataSynchronizer for Oracle
							UseDataSynchro.Checked = false;
							UseDataSynchro.Enabled = false;

							DBCompanySettings.Controls.Add(dbOracle);
					...
					case NameSolverDatabaseStrings.SQLOLEDBProvider:
						{
							...
							DataBaseSql dbSql = new DataBaseSql(licenceInfo.UseUnicodeSet());
							dbSql.OnCallHelp += new DataBaseSql.CallHelp(SendHelp);
							dbSql.OnSetValueForUnicodeCheckBox += new DataBaseSql.SetValueForUnicodeCheckBox(dbSql_OnSetValueForUnicodeCheckBox);
							dbSql.OnSetValueUsersComboBox += new DataBaseSql.SetValueUsersComboBox(dbSql_OnSetValueUsersComboBox);
							dbSql.IsNewSQLCompany = inserting;
							dbSql.Inserting = inserting;

							if (dbSql.IsNewSQLCompany)
								dbSql.NewCompanyDbName = tbCompany.Text;
							else
                    ...
                    case NameSolverDatabaseStrings.PostgreOdbcProvider:
                        {
                            ...
                            if (DBCompanySettings.Controls.Count > 0)
                            {
                                if (DBCompanySettings.Controls[0] is DataBasePostgre)
                                    return;
                                else
                                    DBCompanySettings.Controls.RemoveAt(0);
                            }

                            DataBasePostgre dbSql = new DataBasePostgre(licenceInfo.UseUnicodeSet());
                            dbSql.OnCallHelp += new DataBasePostgre.CallHelp(SendHelp);
                            dbSql.OnSetValueForUnicodeCheckBox += new DataBasePostgre.SetValueForUnicodeCheckBox(dbSql_OnSetValueForUnicodeCheckBox);
                            dbSql.OnSetValueUsersComboBox += new DataBasePostgre.SetValueUsersComboBox(dbSql_OnSetValueUsersComboBox);
                            dbSql.IsNewPostgreCompany = inserting;
                            dbSql.Inserting = inserting;
		...
		private void tbCompany_Leave(object sender, System.EventArgs e)
		{
			if (tabControl1.TabPages.Contains(DBCompanySettings))
			{
				if (DBCompanySettings.Controls.Count > 0)
				{
					if (DBCompanySettings.Controls[0] is DataBaseSql)
					{
						DataBaseSql dbSql = (DataBaseSql)DBCompanySettings.Controls[0];
						if (dbSql.IsNewSQLCompany)
							dbSql.NewCompanyDbName = ((TextBox)sender).Text;
					}
                    else if (DBCompanySettings.Controls[0] is DataBasePostgre)
                    {
                        DataBasePostgre dbPostgre = (DataBasePostgre)DBCompanySettings.Controls[0];
                        if (dbPostgre.IsNewPostgreCompany)
                            dbPostgre.NewCompanyDbName = ((TextBox)sender).Text;
                    }
					else if (DBCompanySettings.Controls[0] is DataBaseOracle)
					{
						DataBaseOracle dbOracle = (DataBaseOracle)DBCompanySettings.Controls[0];
						if (dbOracle.IsNewOracleCompany)
							dbOracle.OracleCompanyName = ((TextBox)sender).Text;
					}
				}
			}
			...
			if (tabControl1.TabPages.Contains(DBSlave))
			{
				...
				if (SlavePanel.Controls.Count > 0)
				{
					if (SlavePanel.Controls[0] is DataBaseSql)
					{
						DataBaseSql dmsDb = (DataBaseSql)SlavePanel.Controls[0];
						if (dmsDb.IsNewSQLCompany)
							dmsDb.NewCompanyDbName = ((TextBox)sender).Text + DatabaseLayerConsts.DMSSignature;
					}
				}
			}
		}
		...
		//---------------------------------------------------------------------
		private void dbOracle_OnUnableToContinue(object sender, System.EventArgs e)
		{
			if (DBCompanySettings.Controls.Count > 0)
				DBCompanySettings.Controls[0].Enabled = false;
			if (BtnSave.Enabled)
				BtnSave.Enabled = false;
			if (BtnDelete.Enabled)
				BtnDelete.Enabled = false;
			OnDisableSaveButton?.Invoke(sender, e);
		}
		...
		///<summary>
		/// SaveDMS
		/// Creazione/Salvataggio del database documentale
		///</summary>
		//---------------------------------------------------------------------
		private bool SaveDMS()
		{
			bool isSaved = true;
			diagnostic.Clear();
			...
			TransactSQLAccess connSqlTransact = null;
			...
			if (CheckDMSValidator())
			{
				...
				contextUser.ServerPrimary = dmsSql.SelectedSQLServerName;
				contextUser.ServerIstance = dmsSql.SelectedSQLIstanceName;
				contextUser.DatabaseName = (dmsSql.IsNewSQLCompany) ? DatabaseLayerConsts.MasterDatabase : dmsDatabaseName;
				contextUser.LoginId = dmsSql.SelectedDbOwnerId;
				contextUser.LoginName = dmsSql.SelectedDbOwnerName;
				contextUser.LoginPassword = dmsSql.SelectedDbOwnerPwd;
				contextUser.DbType = DBMSType.SQLSERVER;

				if (dmsSql.SelectedDbOwnerIsWinNT)
					contextUser.Domain = contextUser.LoginName.Split(Path.DirectorySeparatorChar)[0];

				//impersonifico l'utente
				if (contextUser.Impersonification == null)
				{
					contextUser.Impersonification = new UserImpersonatedData();
					if (contextUser.IsWinNT)
					{
						contextUser.Impersonification = this.AskCredential(connSqlTransact, contextUser, false, false);
						if (contextUser.Impersonification == null)
						{
						...
						contextUser.LoginPassword = contextUser.Impersonification.Password;
					}
					else
					{
						string pwd = OnGetUserAuthenticatedPwdFromConsole(contextUser.LoginName, contextUser.ServerComplete);
						bool alreadyLogged = OnIsUserAuthenticatedFromConsole(contextUser.LoginName, pwd, contextUser.ServerComplete);

						if (alreadyLogged)
							contextUser.Impersonification.Password = contextUser.LoginPassword = pwd;
						else
							contextUser.Impersonification.Password = contextUser.LoginPassword;

						contextUser.Impersonification.Login = contextUser.LoginName;
						contextUser.Impersonification.Domain = string.Empty;
						contextUser.Impersonification.WindowsAuthentication = false;
					}

					//ho impersonificato correttamente prendo la pwd costruisco la stringa di connessione
					connSqlTransact.CurrentStringConnection = BuildStringConnection(contextUser);
				}

				// devo effettuare dei controlli sulla versione del server utilizzato per il DMS
				using (SqlConnection connection = new SqlConnection(connSqlTransact.CurrentStringConnection))
				{
					try
					{
						connection.Open();
				...
				while (!ended)
				{
					if (!TryToConnect(connSqlTransact, contextUser))
					{
						int resultAfterSettingDbo = SetCompanyDboUser(connSqlTransact, contextUser);
					...
					else
					{
						...
						int resultAfterCheckRole = CheckRoleDbOwner(contextUser, connSqlTransact);
						if (resultAfterCheckRole == 0 || resultAfterCheckRole == -1)
							ended = true;
					}
				}

				//se è Sql il server può essere sia msde che sql - nulla da controllare
				if (dmsSql.IsNewSQLCompany)
				{
					//imposto il nome del db da creare
					contextUser.DatabaseName = dmsDatabaseName;

					bool useUnicode = false;
					if (!CheckIfSqlDbExist(connSqlTransact, contextUser))
					{
						diagnostic.Set(DiagnosticType.Warning, DatabaseItemsStrings.StopDataBaseCreation);
						isSaved = false;
						if (contextUser.Impersonification != null)
					...
					// Creo il database documentale
					bool resultCreationDB = CreateDatabase(contextUser, false, out useUnicode, dmsSql.ShowCreationParameters, true);
					if (!resultCreationDB)
					{
						if (contextUser.Impersonification != null)
							contextUser.Impersonification.Undo();
					...
					{
						// per il Easy Attachment non devo impostare ne' il valore Unicode ne' la DatabaseCulture
						// perche' la leggo dall'anagrafica azienda
						if (contextUser.Impersonification != null)
							contextUser.Impersonification.Undo();

						isSaved = resultCreationDB && UpdatingDMSTables(contextUser);
					}
				}
				else
				{
					...
					//verificare requirements database
					OnCheckDBRequirementsUsed(BuildStringConnection(contextUser), DBMSType.SQLSERVER, contextUser.UseUnicode, out isUnicode, out italianTableName);
					...
					int checkDb = CompatibilitySQLDatabaseCulture(contextUser);
					...
					//sono in modifica
					string dboOfDataBase = string.Empty;
					if (!connSqlTransact.CurrentDbo(contextUser.LoginName, contextUser.DatabaseName, out dboOfDataBase) ||
						(dboOfDataBase.Length == 0))
					{
						string message = string.Format(Strings.DifferentDbo, contextUser.DatabaseName, dboOfDataBase, contextUser.LoginName);
						diagnosticViewer.Message = message;
						diagnosticViewer.Title = Strings.ChangeDbo;
						diagnosticViewer.ShowButtons = MessageBoxButtons.OK;
						diagnosticViewer.ShowIcon = MessageBoxIcon.Question;
						diagnosticViewer.Show();
						if (!ChangeCompanyDbOwner(connSqlTransact, contextUser))
							isSaved = false;
                        /*						else
												{
													// il database Easy Attachment non deve fare modifiche sugli utenti!
                    ...
                    // vado ad aggiornare le varie tabelle di sistema
                    isSaved = isSaved & UpdatingDMSTables(contextUser);
				}
			...
			// alla fine di tutto, devo rimettere a posto le login di SQL e gli utenti associati al database
			// sul server SQL, perchè potrebbe essere stato cambiato dall'utente.
			isSaved = isSaved && ManageUsersAndLoginsForDMS(connSqlTransact, contextUser.DatabaseName);

			if (isSaved)
				if (OnAfterModifyCompany != null) OnAfterModifyCompany(this, tbCompanyId.Text);
...
*/
			...
			catch
			{
				...
				DiagnosticViewer.ShowDiagnostic(connSqlTransact.Diagnostic, OrderType.Date, this);
				if (this.OnSendDiagnostic != null)
				{
					OnSendDiagnostic(this, diagnostic);
					diagnostic.Clear();
		...
		private void BuildTabDBSlave()
		{
			if (cbProvider.SelectedItem != null)
			{
				...
				DataBaseSql dmsSql = new DataBaseSql(licenceInfo.UseUnicodeSet());
				...
				dmsSql.OnSetNewDatabaseName += new DataBaseSql.SetNewDatabaseName(dmsSql_OnSetNewDatabaseName);
				dmsSql.EnableUsersComboBox(false); // disabilito la combobox degli utenti applicativi

				dmsSql.IsNewSQLCompany = (companyItem != null) ? !companyItem.UseDBSlave : inserting;
				dmsSql.Inserting = (companyItem != null) ? !companyItem.UseDBSlave : inserting;

				if (dmsSql.IsNewSQLCompany) // il nome del nuovo database documentale di default ha il suffisso DMS
					dmsSql.NewCompanyDbName = tbCompany.Text + DatabaseLayerConsts.DMSSignature;
				...
				switch (selectedProvider.ProviderValue)
				{
					...
					case NameSolverDatabaseStrings.SQLOLEDBProvider:
						{
							...
							if (DBCompanySettings != null && DBCompanySettings.Controls != null && DBCompanySettings.Controls.Count > 0)
							{
								if (DBCompanySettings.Controls[0] is DataBaseSql)
								{
									DataBaseSql companyDbSql = (DataBaseSql)DBCompanySettings.Controls[0];
					...
					case NameSolverDatabaseStrings.OraOLEDBProvider:
						{
							// devo proporre l'utente applicativo e il nome del server scelto per il database di sistema
							dmsSql.ServerNameSystemDb = this.ServerNameSystemDb;
							dmsSql.ServerIstanceSystemDb = this.ServerIstanceSystemDb;
							dmsSql.UserConnected = this.userConnected;
							dmsSql.UserPwdConnected = this.userPwdConnected;
							dmsSql.SelectedSQLServerName = this.ServerNameSystemDb;
							dmsSql.SelectedSQLIstanceName = this.ServerIstanceSystemDb;
							...
							if (DBCompanySettings != null && DBCompanySettings.Controls != null && DBCompanySettings.Controls.Count > 0)
							{
								if (DBCompanySettings.Controls[0] is DataBaseOracle)
								{
									DataBaseOracle companyDbOra = (DataBaseOracle)DBCompanySettings.Controls[0];
									dmsSql.SelectedDbOwnerId = companyDbOra.ApplicationUserId;
									dmsSql.SelectedDbOwnerName = companyDbOra.ApplicationUserName;
								}
							}

							// provo a commentare
							/*dmsSql.SelectedDbOwnerId = this.userDbOwnerId;
							dmsSql.SelectedDbOwnerName = this.userConnected;
							dmsSql.SelectedDbOwnerPwd = this.userPwdConnected;
							dmsSql.SelectedDbOwnerIsWinNT = false;*/
		...
		private bool SaveCompanyForSQLProvider()
		{
			...
			string dbSqlDatabaseName = (dbSql.IsNewSQLCompany) ? dbSql.SelectedSQLNewDatabaseName : dbSql.SelectedSQLDatabaseName;
			...
			else
			{
				...
				{
					// togliere questo msg inutile!
					/*string message = string.Format(Strings.DifferentDbo, contextUserCompanyDb.DatabaseName, dboOfDataBase, contextUserCompanyDb.LoginName);
					diagnosticViewer.Message = message;
					diagnosticViewer.Title = Strings.ChangeDbo;
					diagnosticViewer.ShowButtons = MessageBoxButtons.OK;
					diagnosticViewer.ShowIcon = MessageBoxIcon.Question;
			...
			OnAfterChangedAuditing?.Invoke(this, tbCompanyId.Text, UseAuditing.Checked);
			OnAfterChangedOSLSecurity?.Invoke(this, tbCompanyId.Text, UseSecurity.Checked);
			OnAfterChangedCompanyDisable?.Invoke(this, tbCompanyId.Text, CbCompanyDisabled.Checked);

			if (isCompanyDbSaved && isDMSDbSaved) // se tutti i salvataggi sono andati a buon fine allora ridisegno il tree e riabilito i pulsanti
			{
				OnModifyTreeOfCompanies?.Invoke(this, ConstString.containerCompanyUsers, tbCompanyId.Text);
				OnModifyTree?.Invoke(this, ConstString.containerCompanies);
				OnAfterModifyCompany?.Invoke(this, tbCompanyId.Text);
			}
		...
		///</summary>
		//---------------------------------------------------------------------
		private bool CheckDMSDbSettings(DataBaseSql dmsSql, string dmsDatabaseName, TransactSQLAccess transactDMSDb, UserInfo contextUserDMSDb)
		{
			...
			contextUserDMSDb.DatabaseName = (dmsSql.IsNewSQLCompany) ? DatabaseLayerConsts.MasterDatabase : dmsDatabaseName;
			...
			)
		{
			...
			contextUserCompanyDb.DatabaseName = (dbSql.IsNewSQLCompany) ? DatabaseLayerConsts.MasterDatabase : dbSqlDatabaseName;
        ...
        ///</summary>
        //---------------------------------------------------------------------
        private bool CheckPostgreCompanyDbSettings
            (
            DataBasePostgre dbSql,
            string dbSqlDatabaseName,
            PostgreAccess transactCompanyDb,
            UserInfo contextUserCompanyDb
            )
        {
            transactCompanyDb.OnAddUserAuthenticatedFromConsole += new PostgreAccess.AddUserAuthenticatedFromConsole(this.AddUserAuthentication);
            transactCompanyDb.OnGetUserAuthenticatedPwdFromConsole += new PostgreAccess.GetUserAuthenticatedPwdFromConsole(this.GetUserAuthenticatedPwd);
            transactCompanyDb.OnIsUserAuthenticatedFromConsole += new PostgreAccess.IsUserAuthenticatedFromConsole(this.IsUserAuthenticated);

            contextUserCompanyDb.ServerPrimary = dbSql.SelectedPostgreServerName;
            contextUserCompanyDb.ServerIstance = dbSql.SelectedPostgreIstanceName;
            contextUserCompanyDb.DatabaseName = (dbSql.IsNewPostgreCompany) ? DatabaseLayerConsts.postgreMasterDatabase : dbSqlDatabaseName;
            contextUserCompanyDb.LoginId = dbSql.SelectedDbOwnerId;
            contextUserCompanyDb.LoginName = dbSql.SelectedDbOwnerName;
            contextUserCompanyDb.LoginPassword = dbSql.SelectedDbOwnerPwd;
            contextUserCompanyDb.DbType = DBMSType.POSTGRE;
            ...
            if (contextUserCompanyDb.Impersonification == null)
            {
                ...
                if (contextUserCompanyDb.IsWinNT)
                {
                    contextUserCompanyDb.Impersonification = this.AskCredentialPostgre(transactCompanyDb, contextUserCompanyDb, false, false);
                    if (contextUserCompanyDb.Impersonification == null)
                    {
                        diagnostic.Set(DiagnosticType.Warning, DatabaseItemsStrings.StopDataBaseCreation);
                        return false;
                ...
                //ho impersonificato correttamente prendo la pwd costruisco la stringa di connessione
                transactCompanyDb.CurrentStringConnection = BuildStringConnectionPostgre(contextUserCompanyDb);
            }
            ...
            while (!ended)
            {
                if (!TryToConnectPostgre(transactCompanyDb, contextUserCompanyDb))
                {
                    // assegno l'utente dbowner al database (associazione 1 a 1)
                    int resultAfterSettingDbo = SetCompanyUserPostgre(transactCompanyDb, contextUserCompanyDb);
                    if (resultAfterSettingDbo == 0 || resultAfterSettingDbo == -1)
                    {
                        ended = true;
                        pressCancelDuringSettingDbo = (resultAfterSettingDbo == -1) ? true : false;
                ...
                {
                    // Ora verifico che l'utente abbia il Ruolo SysAdmin
                    int resultAfterCheckRole = CheckRoleDbOwnerPostgre(contextUserCompanyDb, transactCompanyDb);
                    if (resultAfterCheckRole == 0 || resultAfterCheckRole == -1)
                        ended = true;
                    pressCancelDuringCheckRoleDbo = (resultAfterCheckRole == -1) ? true : false;
                }
            ...
            if (dbSql.IsNewPostgreCompany)
            {
                ...
                // controllo che il database specificato non esista gia'
                if (transactCompanyDb.ExistDataBase(contextUserCompanyDb.DatabaseName))
                {
            ...
            else // il database esiste gia'
            {
                ...
                //    TBCheckDatabase.IsDBSizeOverMaxLimit(transactCompanyDb.CurrentStringConnection, DBMSType.SQLSERVER))
                //{
                //    diagnostic.Set(DiagnosticType.Error, Strings.DBSizeError);
                //    return false;
                //}
		...
		///<summary>
		/// Salvataggio di un'azienda su database Azure (per ora e' solo una copia di quello di SQL da fare!!!)
		///</summary>
		//---------------------------------------------------------------------
		private bool SaveCompanyForAzure()
		{
			diagnostic.Clear();
			...
			// mi impersonifico, eseguo la connessione al server, controllo le impostazioni del database aziendale (unicode, collate, etc.)
			// creo anche la login su SQL
			if (!CheckSqlAzureCompanyDbSettings(dbSql, dbSqlDatabaseName, contextUserCompanyDb))
			{
				if (contextUserCompanyDb.Impersonification != null)
					contextUserCompanyDb.Impersonification.Undo();
				BtnSave.Enabled = true;
			...
			// alla fine di tutto, devo rimettere a posto le login di SQL e gli utenti associati al database
			// sul server SQL, perchè potrebbe essere stato cambiato dall'utente.
			// TODO: pensare a come gestire gli utenti e l'eventuale cambio di dbo... anche se semplificherei
			//isCompanyDbSaved = isCompanyDbSaved && ManageUsersAndLoginsForCompany(transactCompanyDb, contextUserCompanyDb.DatabaseName);
				...
				if (isDMSActivated)
				{
					...
					if (UseDBSlave.Checked)
					{
						...
						else
						{
							//sono in modifica
							string dboOfDataBase = string.Empty;
							if (!transactDMSDb.CurrentDbo(contextUserDMSDb.LoginName, contextUserDMSDb.DatabaseName, out dboOfDataBase) || (dboOfDataBase.Length == 0))
							{
								if (!ChangeCompanyDbOwner(transactDMSDb, contextUserDMSDb))
									isDMSDbSaved = false;
							}
		...
		///</summary>
		//---------------------------------------------------------------------
		private bool CheckSqlAzureCompanyDbSettings
			(
			DataBaseSql dbSql,
			string dbSqlDatabaseName,
			UserInfo contextUserCompanyDb
			)
		{
			...
			sqlAccess.CurrentStringConnection = BuildStringConnection(contextUserCompanyDb);
			...
			using (SqlConnection myConnection = new SqlConnection(sqlAccess.CurrentStringConnection))
			{
				...
				{
					diagnostic.Set(DiagnosticType.Warning, string.Format("The server {0} you are trying to connect is not a SQL Azure edition, as expected by your licence. Please choose another one!", contextUserCompanyDb.ServerComplete));
					DiagnosticViewer.ShowDiagnostic(diagnostic, OrderType.Date, this);
					return false;
				}
			}
			...
			if (!licenceInfo.IsSQL2012Allowed)
			{
				using (TBConnection myConnection = new TBConnection(sqlAccess.CurrentStringConnection, DBMSType.SQLSERVER))
				{
					try
					{
						myConnection.Open();
			...
			if (dbSql.IsNewSQLCompany)
			{
				...
				companyDb.ConnectionString = this.connectionString;
				companyDb.CurrentSqlConnection = this.currentConnection;
				if (companyDb.ExistsCompanyByName(tbCompany.Text))
				{
					diagnostic.Set(DiagnosticType.Error, Strings.CompanyWithSameName);
					DiagnosticViewer.ShowDiagnostic(diagnostic, OrderType.Date, this);
					if (this.OnSendDiagnostic != null)
				...
				if (sqlAccess.ExistDataBase(contextUserCompanyDb.DatabaseName))
				{
					diagnostic.Set(DiagnosticType.Error, string.Format("A database with name '{0}' already exists on server '{1}'. Unable to proceed.", contextUserCompanyDb.DatabaseName, contextUserCompanyDb.ServerComplete));
					DiagnosticViewer.ShowDiagnostic(diagnostic, OrderType.Date, this);
					return false;
				}
			}
			...
			{
				// controllo la size del db (se DBNetworkType == Small non deve essere superiore a 2GB)
				if (InstallationData.CheckDBSize &&	Functions.IsDBSizeOverMaxLimit(sqlAccess.CurrentStringConnection))
				{
					diagnostic.Set(DiagnosticType.Error, Strings.DBSizeError);
					return false;
				}
				...
				//verificare requirements database
				OnCheckDBRequirementsUsed(sqlAccess.CurrentStringConnection, DBMSType.SQLSERVER, contextUserCompanyDb.UseUnicode, out isUnicode, out italianTableName);
