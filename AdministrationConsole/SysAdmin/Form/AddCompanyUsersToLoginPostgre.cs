...
namespace Microarea.Console.Plugin.SysAdmin.Form
{
	...
	/// </summary>
	//=========================================================================
	public partial class AddCompanyUsersToLoginPostgre : PlugInsForm
	{
		...
		Diagnostic diagnostic = new Diagnostic("SysAdmin.AddCompanyUsersToLoginPostgre"); 
        ...
        //private PostgreAccess dmsConnSqlTransact = new PostgreAccess();
		...
		/// </summary>
		//---------------------------------------------------------------------
		public AddCompanyUsersToLoginPostgre(string connectionStr, SqlConnection connection, string companyId,  BrandLoader brandLoader)
		{
			InitializeComponent();
            ...
            if (connection.State == ConnectionState.Closed)
                connection.Open();
            SqlCommand command = new SqlCommand(portQuery, connection);
            try
            {
		...
		#region LoadLogins - Carica nella combo le Logins presenti sul server postgre
		/// <summary>
		/// LoadLogins
		/// </summary>
		//---------------------------------------------------------------------
		private void LoadLogins()
		 {
			...
			if (!connSqlTransact.GetLogins
                (out loginsOfCompany)) // leggo le logins
			{
				//TO DO GESTIRE
			...
			if (loginsOfCompany.Count > 0)
			{
				for (int i = 0; i < loginsOfCompany.Count; i++)
				{
                    ...
                    CbLogins.Items.Add(new LoginItem(companyLogin.Login, false)); //@@Anastasia al posto di false ci andava companyLogin.IsNTUser
				}
		...
		private bool CheckData()
		{
			...
			else if (RadioNewLogin.Checked)
			{
                ...
                if (string.IsNullOrEmpty(TbLoginPassword.Text) || string.IsNullOrWhiteSpace(TbLoginPassword.Text))
                {
                    diagnostic.Set(DiagnosticType.Information, Strings.ImpossibleCreateUserWithEmptyPassword);
                    checkSuccess = false;
                }
		...
		private bool GrantLoginToCompany(bool isLoginNT, string loginName, string loginPassword, PostgreAccess connSqlTransact)
		{
            //if (isLoginNT)
            //{
            //    return (connSqlTransact.GrantAccess(loginName));
            //}
            //else
            //{
		...
		/// </summary>
		//---------------------------------------------------------------------
		private bool UpdatedCompanyLogins(bool isLoginNT, string loginName, string loginPassword, PostgreAccess connSqlTransact)
		{
			bool updated = false;
		...
		private string CreateCompanyConnectionString()
		{
			...
			if (companyData.Count > 0)
			{
				...
				if (userDboData.Count > 0)
				{
                    ...
                    //@@Anastasia Non gestisco WinAuth
                    ////ora compongo la stringa di connessione
                    //if (this.dbOwnerWinAuth)
                    //{
                    //    connectionToCompanyServer = string.Format(NameSolverDatabaseStrings.SQLWinNtConnection, this.dbCompanyServer, this.dbCompanyName);
	...
	public class LoginItemPostgre
	{
		...
		//---------------------------------------------------------------------
		public LoginItemPostgre(string loginName, bool isNTUser)
		{
			LoginName = loginName;
			IsNTUser  = isNTUser;
		}
