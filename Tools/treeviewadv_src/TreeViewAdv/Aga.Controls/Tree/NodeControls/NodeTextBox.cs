...
namespace Aga.Controls.Tree.NodeControls
{
	public class NodeTextBox: BaseTextControl
	{
        ...
        private String _defaultTextForEditing = "";
        ...
        protected override Size CalculateEditorSize(EditorContext context)
		{
			...
			else
			{
				...
				Size size = GetLabelSize(textBox.Text, context.CurrentNode.Bold);
				int width = Math.Max(size.Width + Font.Height, MinTextBoxWidth); // reserve a place for new typed character
				return new Size(width, size.Height);
			}
		}

        //5071 - disable automatic editing
		public override void KeyDown(KeyEventArgs args)
		{
			if (args.KeyCode == Keys.F2 && Parent.CurrentNode != null && Parent.CanDirectlyEditing)
			{
				args.Handled = true;
				BeginEdit();
			}
		...
		protected override void DoApplyChanges(TreeNodeAdv node)
		{
            ...
            if (String.IsNullOrEmpty(_label) || String.Compare(_label, _defaultTextForEditing, true) == 0)
            {
                Node parent = (node.Tag as Node).Parent;
                parent.Nodes.Remove(node.Tag as Node);
                (node.Tag as Node).Parent = null;
                Parent.SmartFullUpdate();
                return;
            }
