...
namespace Microarea.Tools.TBLocalizer
{
    ...
    public class WebContentParser : Parser
    {
        ...
        private void AddUserKeywords(LexanParser lex)
        {
        ...
        public override void Parse(string f)
        {
            ...
            foreach (var info in strings)
            {
                ...
                if (string.IsNullOrEmpty(info.ClassName))
                    throw new Exception(string.Format("Cannot parse typescript class name in file {0}", f));
            ...
            try
            {
                ...
                while (!lex.Eof)
                {
                    if (lex.Parsed((Token)WebToken.COMPONENT) && lex.Parsed(Token.ROUNDOPEN))
                    {
                        ...
                        {
                            lex.ClearErrors();//non devo essere fiscale; la skiptotoken non gradisce la presenza di errori
        ...
        private void ExtractHTMLStrings(string f, StringInfos infos)
        {
            ...
            ExtractHTMLStringsFromContent(content, infos);


        }

        private void ExtractHTMLStringsFromContent(string content, StringInfos infos)
        {
            LexanParser lexan = new LexanParser(LexanParser.SourceType.FromString);
            lexan.PreprocessInclude = false;
            AddUserKeywords(lexan);
            lexan.Open(content);
            try
            {
                while (!lexan.Eof)
                {
                    Token t = lexan.LookAhead();
                    if (t == ((Token)WebToken._TB))
                    {
                        ParseString(infos, lexan);
                        continue;
                    }
                    else if (t == Token.TEXTSTRING)
                    {
                        string s;
                        lexan.ParseString(out s);
                        ExtractHTMLStringsFromContent(s, infos);
            ...
            if (lex.LookAhead() == Token.ID)
            {
                ...
                if (lex.SkipToToken(Token.BRACEOPEN, true, false))
                {
                    ...
                    while (level > 0 && !lex.Eof)
                    {
                        ...
                        {
                            ParseString(infos, lex);
                            continue;
                        }
                        else if (lex.LookAhead(Token.TEXTSTRING))
                        {
                            string s;
                            lex.ParseString(out s);
                            ExtractHTMLStringsFromContent(s, infos);
        ...
        private void ParseString(StringInfos infos, LexanParser lexan)
        {
            lexan.SkipToken();

            if (lexan.Parsed(Token.ROUNDOPEN))
            {
                string s;
                if (lexan.ParseString(out s))
                    infos.Add(s);
            }
        }
    }
