...
namespace HttpNamespaceManager.UI
{
    public partial class AccessControlListDialog : Form
    {
		...
		private bool Check(string account)
		{
			try
			{
				SecurityIdentity id = SecurityIdentity.SecurityIdentityFromName(account);
			}
			catch (Exception ex)
			{
        ...
        {
            string accountName;
            if (InputBox.Show("Add User or Group", "Enter the User or Group Name:", new InputBox.OnCheckMethod(Check), out accountName) == DialogResult.OK)
            {
                try
                {
