...
{
    public partial class NamespaceMngControl : UserControl
    {
        ...
        public NamespaceMngControl()
        {
			...
			if (owner != null)
			{
				owner.Shown += new System.EventHandler(this.MainForm_Shown);
				owner.FormClosed += new System.Windows.Forms.FormClosedEventHandler(this.MainForm_FormClosed);
			}
        }

        public NamespaceMngControl(NamespaceManagerAction action, string url)
        {
            nsManager = new HttpApi();
            InitializeComponent();
        ...
        private void buttonAdd_Click(object sender, EventArgs e)
        {
			if (!String.IsNullOrEmpty((string)listHttpNamespaces.SelectedItem))
			{
				...
				InputBox.Show("Enter URL", "Enter the URL to add:", new InputBox.OnCheckMethod(Check), out url);
