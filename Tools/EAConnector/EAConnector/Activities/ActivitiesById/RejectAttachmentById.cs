...
namespace EAConnector.Activities.ActivitiesById {
	[ToolboxItem(typeof(EAToolBoxItemReject))]
	[Designer("EADesigner.ActivitiesDesigner.EAActivityControlReject, EADesigner")]
	[Description("Reject Attachment by Id")]
	public class RejectAttachmentById : EAActivity {
		...
		protected override void Execute(NativeActivityContext context) {
			...
			try {
                ...
                easyAttachmentSync.Approve(attachmentId.Get(context), requesterUserName.Get(context), approvalUserName.Get(context), 2, approvalComments.Get(context));
            }
            catch (Exception e)
            {
