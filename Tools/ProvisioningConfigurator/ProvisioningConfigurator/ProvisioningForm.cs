...
namespace Microarea.Tools.ProvisioningConfigurator.ProvisioningConfigurator
{
	...
	/// </summary>
	//---------------------------------------------------------------------
	public partial class ProvisioningForm : Form
	{
		...
		//---------------------------------------------------------------------
		public ProvisioningForm(string instanceName = "", bool preconfigurationMode = false, bool loadDataFromFile = true )
		{
			InitializeComponent();
        ...
        private void UpdateControls()
        {
            ...
            TxtLogin.Enabled = !addCompanyMode;
		...
		private void FillProvisioningData()
		{
			...
			provisioningData.User = TxtLogin.Text;
			provisioningData.Password = TxtPassword.Text;
		...
		private void FillProvisioningDataInControls()
		{
			...
			TxtLogin.Text = provisioningData.User;
			TxtPassword.Text = provisioningData.Password;
        ...
        //---------------------------------------------------------------------
        private void FillInstanceDataInControls()
        {
            if (!string.IsNullOrWhiteSpace(instanceName))
            {
                if (string.IsNullOrWhiteSpace(TxtCompany.Text))
                    TxtCompany.Text = instanceName;

                if (string.IsNullOrWhiteSpace(TxtCompanyDB.Text))
                    TxtCompanyDB.Text = instanceName + "_DB";

                if (string.IsNullOrWhiteSpace(TxtSystemDB.Text))
                    TxtSystemDB.Text = instanceName + "_SystemDB";

                if (string.IsNullOrWhiteSpace(TxtDmsDb.Text))
                    TxtDmsDb.Text = TxtCompanyDB.Text + "DMS";

                if (string.IsNullOrWhiteSpace(TxtAdminName.Text))
                    TxtAdminName.Text = instanceName + "_Admin";

                if (string.IsNullOrWhiteSpace(TxtUserNameSQL.Text))
                    TxtUserNameSQL.Text = instanceName + "_User";
            }
        }
        ...
        private void BtnOK_Click(object sender, EventArgs e)
		{
            ...
            ElaborationForm ef = new ElaborationForm(provisioningEngine, provisioningData.AddCompanyMode);
	    ...
	    //---------------------------------------------------------------------
        private void TxtCompanyDB_Leave(object sender, EventArgs e)
        {
            if (String.IsNullOrWhiteSpace(this.TxtCompanyDB.Text))
                return;
            if (String.IsNullOrWhiteSpace(this.TxtDmsDb.Text))
                TxtDmsDb.Text = TxtCompanyDB.Text + DatabaseLayerConsts.DMSSignature;
            if (String.IsNullOrWhiteSpace(this.TxtCompany.Text))
                TxtCompany.Text = TxtCompanyDB.Text;
        }
