	...
	public class ProcedureSymbolTable : ArrayList
	{
		...
		{ 
			foreach (Procedure procedure in this)
				if (string.Compare(name, procedure.PublicName, true, CultureInfo.InvariantCulture) == 0)
					return procedure;
	...
	public class QueryObjectSymbolTable : ArrayList
	{
		...
		{ 
			foreach (QueryObject queryObject in this)
				if (string.Compare(name, queryObject.Name, true, CultureInfo.InvariantCulture) == 0)
					return queryObject;
		...
		//-----------------------------------------------------------------------------
		public bool Unparse(Woorm.Lexan.Unparser unparser)
		{
            if (this.Count == 0)
                return true;
