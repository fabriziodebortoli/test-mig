...
namespace Microarea.RSWeb.Applications
{
	...
	public class DataTypeParser
	{
		...
		static public bool Parse(Parser lex, Enums enums, out string type, out string woormType, out ushort tag, out string baseType)
		{
			...
			if (string.Compare(Language.GetTokenString(Token.ARRAY), woormType, StringComparison.OrdinalIgnoreCase) == 0)
			{
				string dummyType = string.Empty;
				if (!DataTypeParser.Parse(lex, enums, out baseType, out dummyType, out tag))
					return false;

				if (string.Compare(Language.GetTokenString(Token.ARRAY), baseType, StringComparison.OrdinalIgnoreCase) == 0)
					return false;
			}
			return true;
		}
