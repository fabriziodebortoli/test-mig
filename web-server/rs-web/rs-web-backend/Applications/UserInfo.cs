...
namespace Microarea.RSWeb.Applications
{
	...
	public class UserInfo : IDisposable	
	{
		...
		public ITbLoaderClient GetTbLoaderInterface()
		{
			lock (this)
			{
				...
				if (LoginManager == null)
					throw new Exception(ApplicationsStrings.LoginNotYetPerformed);
		...
		public bool Login(string user, string password, string company, bool useNTAuth)
		{
			...
			//TODO RSWEB - crea ed esegue la login attraverso il WebService
			LoginManager = new LoginManager();
			TbServices = new TbServices();
			...
			if (ErrorCode != 0)
			{
				ErrorExplain = string.Format(ApplicationsStrings.PasswordError, LoginManagerWrapperStrings.GetString(ErrorCode));
				return false;
			}
			...
			if (ErrorCode != 0)
			{
				ErrorExplain = string.Format(ApplicationsStrings.LoginError, LoginManagerWrapperStrings.GetString(ErrorCode));
				return false;
			}
