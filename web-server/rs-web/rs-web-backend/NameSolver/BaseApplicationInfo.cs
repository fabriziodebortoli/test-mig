...
namespace Microarea.RSWeb.NameSolver
{
	...
	public class LibraryInfo : ILibraryInfo
	{
			...
			get
			{
				if (documents == null)
				{
					...
					foreach (IDocumentInfo docInfo in parentModuleInfo.Documents)
						if (string.Compare(docInfo.NameSpace.Library, Path, StringComparison.InvariantCultureIgnoreCase) == 0)
		...
		public IDocumentInfo GetDocumentInfoByNameSpace(string nameSpace)
		{
            ...
            foreach (IDocumentInfo aDocumentInfo in libraryDocuments)
                if (string.Compare(aDocumentInfo.NameSpace.FullNameSpace, nameSpace, StringComparison.InvariantCultureIgnoreCase) == 0)
	...
	public class BaseModuleInfo : IBaseModuleInfo
	{
		...
		public string GetServerOutPutPath(string build)
		{
			...
			string relPath = ModuleConfigInfo.GetOutPutPath(string.Compare(build, NameSolverStrings.Debug, true, CultureInfo.InvariantCulture) == 0);
		...
		public ILibraryInfo GetLibraryInfoByPath(string aLibraryPath)
		{
			...
			{
				if (String.Compare(aLibInfo.Path, aLibraryPath, StringComparison.InvariantCultureIgnoreCase)== 0)
					return aLibInfo;
			}
		...
		public IDocumentInfo GetDocumentInfoByTitle(string documentTitle)
		{
			...
			foreach (IDocumentInfo aDocumentInfo in docs)
			{
				if (string.Compare(aDocumentInfo.NameSpace.Application, ParentApplicationName, StringComparison.InvariantCultureIgnoreCase) == 0 &&
					string.Compare(aDocumentInfo.NameSpace.Module, name, StringComparison.InvariantCultureIgnoreCase) == 0 &&
					string.Compare(aDocumentInfo.Title, documentTitle, StringComparison.InvariantCultureIgnoreCase) == 0)
					return aDocumentInfo;
			}
	...
	public class BaseApplicationInfo : IBaseApplicationInfo
	{
		...
		private IList ModulesInternal
		{
			get
			{
				lock (instanceLockTicket)
				{
					...
					Generic.Functions.ReadSubDirectoryList(Path, out tempModules);
		...
		{
			if (string.Compare(typeString, "Tb", true, CultureInfo.InvariantCulture) == 0)
				return ApplicationType.TaskBuilder;
			if (string.Compare(typeString, "TbOtherComponents", true, CultureInfo.InvariantCulture) == 0)
				return ApplicationType.TaskBuilder;
			if (string.Compare(typeString, "TbNet", true, CultureInfo.InvariantCulture) == 0)
				return ApplicationType.TaskBuilderNet;
			if (string.Compare(typeString, "TbApplication", true, CultureInfo.InvariantCulture) == 0)
				return ApplicationType.TaskBuilderApplication;
			if (string.Compare(typeString, "Customization", true, CultureInfo.InvariantCulture) == 0)
				return ApplicationType.Customization;
			if (string.Compare(typeString, "EasyBuilderApplication", true, CultureInfo.InvariantCulture) == 0)
				return ApplicationType.Standardization;
			if (string.Compare(typeString, "StandardModuleWrapper", true, CultureInfo.InvariantCulture) == 0)
		...
		public virtual IBaseModuleInfo GetModuleInfoByName(string moduleName)
		{
			foreach(IBaseModuleInfo moduleInfo in Modules)
				if (string.Compare(moduleInfo.Name, moduleName, StringComparison.InvariantCultureIgnoreCase) == 0)
					return moduleInfo;
		...
		{
			foreach (IBaseModuleInfo moduleInfo in Modules)
				if (string.Compare(moduleInfo.Title, moduleTitle, StringComparison.InvariantCultureIgnoreCase) == 0)
					return moduleInfo;
		...
		public static bool IsValidObjectFileName(string aObjectFileName, string fileExtension)
		{
			...
			aObjectFileName = aObjectFileName.ToLower(CultureInfo.InvariantCulture);

			if (fileExtension != null && fileExtension != String.Empty)
			{
				if (aObjectFileName.EndsWith(fileExtension.ToLower(CultureInfo.InvariantCulture)))
