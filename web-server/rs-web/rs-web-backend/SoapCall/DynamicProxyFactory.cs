    ...
    public class DynamicProxyFactory
	{
		...
		public DynamicProxy CreateProxy (ServiceEndpoint endpoint, TimeSpan timeout)
		{
				...
				string.Compare(endpoint.Address.Uri.Host, BasePathFinder.BasePathFinderInstance.RemoteWebServer, System.StringComparison.OrdinalIgnoreCase) != 0)
			{
                ...
                //TODO RSWEB LoginManager loginMng = new LoginManager();    // -> GetProxySettings();
                //settings = loginMng.GetProxySettings();

                if (settings != null && !string.IsNullOrEmpty(settings.HttpProxy.Server))
				{
					UriBuilder ub = new UriBuilder(settings.HttpProxy.Server);
					if (settings.HttpProxy.Port != 0)
						ub.Port = settings.HttpProxy.Port;
