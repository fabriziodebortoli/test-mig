...
export class State {
  ...
  with(a: Partial<State> | ((s: State) => Partial<State>)): Readonly<State> {
    if (typeof a === 'function') a = a(this as any);
...
export class CustomisableGridComponent extends ControlComponent
  ...
  @Input() anchorAlign: Align = { horizontal: 'center', vertical: 'top' };
  @Input() popupAlign: Align = { horizontal: 'center', vertical: 'bottom' };
  @Input() animate: { direction: 'up' };
  @Input() pageable = true;
  ...
  @ContentChild('customisableGridButtonsTemplate', { read: TemplateRef })
  customisableGridButtonsTemplate;
  ...
  private reshape = memoize((d: GridData, cols) =>
    tryOrDefault(
      () =>
        this.limit(d).with(s => ({
          columns: this.reorder(this.resize(s.columns))
        })),
      d
    )
  );
  ...
  ngOnInit() {
    this.loadSettings();
    this.filterer.filterChanged$.subscribe(_ =>
      this.gridStyle$.next(GridStyles.default)
    );
    this.filterer.filterChanging$.subscribe(_ =>
      this.gridStyle$.next(GridStyles.waiting)
    );
    this.paginator.waiting$.subscribe(b => {
      ...
      setTimeout(
        () =>
          this.gridStyle$.next(
            this.isWaiting ? GridStyles.waiting : GridStyles.default
          ),
        0
      );
    });
  ...
  loadSettings = () =>
    (this._settings = this.m.storage.getOrDefault(
      storageKeySuffix,
      new Settings()
    ));
  ...
  saveWidths = () => {
    this._settings.widths = this.grid.columns.reduce(
      (o, c) => ({ ...o, [c['field']]: c.width }),
      {}
    );
    this.saveSettings();
  };
  ...
  limit = (d: GridData): GridData => {
    ...
    const data =
      d.columns.length < maxCols
        ? d.data
        : d.data.map(r =>
            this.moveToStart(
              c => this.selectionColumnId && c === this.selectionColumnId,
              Object.keys(r)
            )
              .slice(0, maxCols)
              .reduce((o, k) => ({ ...o, [k]: r[k] }), {})
          );
    const columns = this.moveToStart(
      c => this.selectionColumnId && c.id === this.selectionColumnId,
      d.columns
    ).slice(0, maxCols);
  ...
  reorder = cols => {
    ...
    return cols.sort(
      (a, b) => reorderMap.indexOf(a.id) - reorderMap.indexOf(b.id)
    );
  };
  ...
  private resetSettingsIfNew = cols => {
    ...
    if (
      new Settings().version !== this._settings.version ||
      hash !== this._settings.hash
    ) {
      this._settings = new Settings();
      this._settings.hash = hash;
      this._settings.reorderMap = cols.map(x => x.id);
      this._settings.columnsVisibility = cols.reduce(
        (o, c) => ({ ...o, [c.id]: true }),
        {}
      );
      this.saveSettings();
  ...
  columnReorder(e) {
    setTimeout(() => {
      this._settings.reorderMap = this.grid.columns
        .toArray()
  ...
  get areFiltersVisibleIcon() {
    return this.areFiltersVisible
      ? 'tb-filterandsortfilled'
      : 'tb-filterandsort';
  }

  get areFiltersVisibleText() {
    return this.areFiltersVisible
      ? this._TB('Hide Filters')
      : this._TB('Show Filters');
  }
  ...
  private contains(target: any): boolean {
    return (
      (this.popupAnchor || this.anchor).nativeElement.contains(target) ||
  ...
  columnVisibilityChanged() {
    this.saveSettings();
    this.filterer.hiddenFields = Object.keys(
      this.settings.columnsVisibility
    ).filter(k => !this.settings.columnsVisibility[k]);
