...
import {
  State,
  CustomisableGridComponent
} from './../customisable-grid/customisable-grid.component';
...
export class RadarComponent extends ControlComponent implements OnInit {
  ...
  public anchorElRef: ElementRef;
  public set state(s: State) {
    (this.state$ as BehaviorSubject<State>).next(s);
  }
  public get state(): State {
    return (this.state$ as BehaviorSubject<State>).getValue();
  }
  ...
  ngOnInit() {
    ...
    this.paginator.start(
      ...
      Observable.combineLatest(
        this.m.eventData.openRadar.filter(b => b),
        this.filterer.filterChanged$,
        this.filterer.sortChanged$,
      ...
      .subscribe(
        m =>
          (this.state = this.state.with({
            canNavigate:
              m !== FormMode.EDIT && m !== FormMode.NEW && m !== FormMode.FIND
          }))
      );
    this.m.eventData.openRadar.pipe(untilDestroy(this)).subscribe(this.show);
    this.paginator.waiting$.subscribe(
      b => (this.state = this.state.with({ canNavigate: !b }))
    );
    super.ngOnInit();
  }
  ...
  selectedKeysChange(e) {
    if (e.length && e[0]) {
      this.state = this.state.with({
        selectedIndex: this.state.rows.findIndex(
          x => x[this.selectionColumnId] === e[0]
        )
      });
  ...
  private show = (show: boolean) => {
    ...
    if (show) this.elRef.nativeElement.focus();
  };

  async selectFirst() {
    this.unselect();
    if (!this.paginator.isFirstPage) await this.paginator.firstPage();
    this.selectByIndex(0);
  }

  async previousPage() {
    this.unselect();
    await this.paginator.pageChange(
      (this.paginator.currentPage - 1) * this.pageSize,
      this.pageSize
    );
  ...
  get canFirst() {
    return (
      ...
      (!this.paginator.isFirstPage || this.state.selectedIndex > 0)
    );
  }
  ...
  get canNext() {
    return (
      this.state.selectedIndex !== -1 &&
      (!this.paginator.noMorePages ||
        this.state.selectedIndex < this.state.rows.length - 1)
    );
  }
  ...
  private restoreViewSelection() {
    this.unselect();
    let idx = this.state.rows.findIndex(
      x => x[this.selectionColumnId] === this.lastSelectedKey
    );
    if (idx === -1) return;
    this.state = this.state.with({
      ...this.state,
      selectionKeys: [this.lastSelectedKey],
      selectedIndex: idx
    });
  }

  toggleColumnHandler() {
    if (this.state && this.state.columns && this.state.columns.length) {
      this.grid.showColumnHandler = !this.grid.showColumnHandler;
      setTimeout(() => this.elRef.nativeElement.querySelector('.popup .popup-list').click());
    }
  }

  coerce = (value: number, min: number, max: number): number =>
    Math.min(max - 1, Math.max(value, min));
  isFormMode = (formMode: FormMode): boolean =>
    _.get(this.m.eventData, 'model.FormMode.value') === formMode;
  toggle = () =>
    (this.viewState =
      this.viewState === ViewStates.opened
        ? ViewStates.closed
        : ViewStates.opened);
}
