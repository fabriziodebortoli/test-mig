...
//	TODO: in presenza di TableInfoSorted forse si pu� implementare clonando completamente il master record della TableInfo
...
// se � la stessa non faccio niente altrimenti devo 
...
void SqlRecord::SetConnection(SqlConnection* pConnection)
{
	...
	if (m_pTableInfo)
	{
		...
		// le volte successive non devo cancellarla ne� ricrearla altrimenti in fase di rebinding delle colonne i dataobj dei campi aggiunti riferiscono ad un area di memoria 
		// non pi� esistente BugFix#21666
		...
		// il rebing lo faccio solo se ho gi� effettuato la bindrecord
		...
		//la tabella non esiste il SqlRecord � invalido
		SetValid(FALSE);
}
...
BOOL SqlRecord::BindRecordItem(SqlRecordItem* pRecItem, int nPos, BOOL bAutoIncrement /*=FALSE*/)
{
	...
	if (pColumnInfo == NULL)
	{
		...
		if (pColumnInfo == NULL)
		{
			//il SqlRecord registrato utilizza il metodo pi� efficiente per trovare le colonne (per posizione)
...
#ifdef _DEBUG
	//effettuo il check di compatibilit� tra i tipi
	...
	{
		// se sono in debug controllo che non sia stata gi� fatto una BIND con lo stesso nome di colonna
        ...
        //verifico che non ci sia gi� un campo con lo stesso nome
...
{	
	if (m_nType == TABLE_TYPE && m_pTableInfo)
	{
		...
		//Impr# 5936
		//se � una tabella master e non � stata fatta la BIND_GUID espilicita allora effettuo il binding del TBGuid
...
SqlRecordItem* SqlRecordLocals::AddLocalField(::DataObj* pDataObj, const CString& strName, int lLength /*= 0*/)
{
	...
	m_ColumnInfos.Add(pInfo);	//far� la delete
