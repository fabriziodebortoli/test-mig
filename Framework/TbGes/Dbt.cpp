...
DataObj* GetDataObjFromName(SqlRecord* pRec, const CString& sFieldName)
{
	...
	if (!pDataObj)
		pDataObj = pRec->GetDataObjFromName(_T("l_") + sFieldName);//se � un campo local, la bind aggiunge l_
	return pDataObj;
}
...
void DBTObject::CreateAndInit(CAbstractFormDoc* pDocument, const CString& sName)
{
	...
	// all'OldSqlRecord
	// m_pRecord invece � gi� stato aggiornato poich� agganciato al SqlTable
	m_pOldRecord->SetConnection(m_pDocument->GetSqlConnection());

	// vuol dire che il DBT � costruito su una view. Non � possibile effettuare il salvataggio
...
BOOL DBTObject::LoadXMLDBTInfo()
{
	...
	if (m_pXMLDBTInfo) // le informazioni sono state gi� caricate....
		return TRUE;

	m_pXMLDBTInfo = new CXMLDBTInfo;
	// il terzo parametro mi fa capire se � stato agganciato da un clientdoc
...
BOOL DBTObject::PrepareSymbolTable(SymTable* pTable)
{
	...
	for (int i = 0; i < m_pRecord->GetSizeEx(); i++)
	{
		...
		if (IsKindOf(RUNTIME_CLASS(DBTSlaveBuffered)))
			pF->SetProvider(dynamic_cast<DBTSlaveBuffered*>(this));	//Il DBT sar� il provider del dato tramite il metodo virtuale GetData
...
BOOL DBTMaster::Update()
{
	...
	// se la master non � stata modificata ma � stato modificato almeno un 
...
BOOL DBTMaster::Delete()
{
	...
	//@@AUDITING
	// se � stato cancellato almeno un dbtslave allora traccio la modifica 
...
SqlRecord* DBTSlaveBuffered::AddRecord()
{
	...
	{
		//per il DBTTree l'evento � posticipato al completamento dell'azione
...
void DBTSlaveBuffered::RemoveAllRecords()
{
	...
	//TODO da verificare se si pu� mettere qui; 
	//m_nCurrentRow = -1;
}
...
DataObj* DBTSlaveBuffered::CheckRow(int nRow, BOOL bCheckAll/*= TRUE*/, BOOL bFromBody/*= TRUE*/)
{
	...
	// altrimenti questa � gi� stata fatta nel loop nella checkrecords
	if (bFromBody)
	{
		OnPreparePrimaryKey(nRow, pRec);
		GetClientDocs()->OnPreparePrimaryKey(this, nRow, pRec);
...
void DBTSlaveBuffered::PrepareDynamicColumns(BOOL bUpdateDescriptions)
{
	if (m_arHKLDescriptionInfos.GetCount())
	{
		...
		//la riga corrente la faccio pe ultima, cos� un eventuale hotlink composito (unico per tutte le righe) mi rimane
	...
	for (int i = 0; i < m_arHKLDescriptionInfos.GetCount(); i++)
	{
		...
		if (!pInfo->m_DescriptionField.IsEmpty())
		{
			...
			if (pKey)
			{
				...
				{
					//se sono in fase di changing, l'hotlink � stato gi� findato, ma devo
...
DataObj* DBTSlaveBuffered::GetBindingData(const CString& strParentDataSource, const CString& strDataSource, CString& sFieldName, CString& sBindingName, bool &fromHKL)
{
	...
	else //caso del campo di decodifica di hotlink
	{
		...
		if (pHotLink)
		{
			...
			//prima controllo se c'�, magari creato da una precedente istanziazione di body edit
...
void DBTSlaveBuffered::SetCurrentDBTSlave(DBTSlaveMap* pData, DBTSlave* pSlave)
{
	...
	if (pCurrentDBTSlave->IsKindOf(RUNTIME_CLASS(DBTSlaveBuffered)))
	{
		...
		//non devo propagare lo stato di readonly ai figli, non � detto che siano legati!
		...
		if (parBodies && parBodies->GetCount())
		{
			for (int i = 0; i < parBodies->GetCount(); i++)
			{
				...
				if (pEdit)
				{
					...
					//il prototipo non pu� essere messo come dbt del bodyedit,
...
DBTSlave* DBTSlaveBuffered::CreateDBTSlave(DBTSlave* pPrototype)
{
	...
	if (bBuffered && pDoc)
	{
		...
		//perch� sono i suoi dataobj ad essere addlinkati nelle colonne
...
DBTSlave* DBTSlaveBuffered::GetDBTSlave(SqlRecord* pRecordMaster, DBTSlaveMap* pData, BOOL bForceCreate /*= FALSE*/)
{
	...
	VERIFY(pSlave->Open());
	if (m_bFindDataCalled) //solo se ho chiamato la FindData sul pap� chiamo quella sul figlio
...
DataObj* DBTSlaveBuffered::GetData(const CString& sName, int nRow/*= -1*/)
{
	if (GetMaster())
	{
		if (GetMaster()->IsKindOf(RUNTIME_CLASS(DBTSlaveBuffered)))
		{
			...
			if (!pCurrent)
			{
				ASSERT_TRACE1(FALSE, "Non � stato trovato il dato relativo al campo %s\n", sName);
				return NULL;
			}
...
DBTSlave* DBTSlaveBuffered::GetMainPrototype()
{
	...
	//bisogna ricordare quindi che se i dbt sono su tre livelli, per il terzo livello esistono pi� dbt prototipo
	//quello che conta � il primo, associato al prototipo del parent
	...
	// 2 - vedo se ad esso � associato un prototipo
	DBTSlaveBuffered* pUncleOrFather = (DBTSlaveBuffered*)pFather->GetMainPrototype();
	// 3 - se non c'�, vuol dire che il parent � il nodo radice, uso lui stesso come ancestor
	if (!pUncleOrFather)
...
CBodyEditPointers* DBTSlaveBuffered::GetBodyEdits()
{
	...
	if (!m_pDBTMaster || !m_pDBTMaster->IsKindOf(RUNTIME_CLASS(DBTSlaveBuffered)))
		return &m_arBodyPtr;
	//altrimenti, siccome ho una serie di istanze di dbt, la lista � detenuta dal main prototype
