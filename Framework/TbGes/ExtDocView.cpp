...
void CAbstractFormView::EnableViewControls()
{
	...
	// Non deve essere chiamata la DisableControlsAlways perch� viene
...
//	CalculateOSLInfo	(); TODO LARA
	...
	if (pFrame && GetParent() == pFrame)
	{
		...
		//perch� la view dovrebbe modificare l'id della frame?
	...
	// possibilit� ai clientdoc di intervenire sulla BuildDataControlLinks
	if (pDoc)
		pDoc->OnBuildDataControlLinks(this);

	ApplyTBVisualManager();
	
	if (!m_pJsonContext)//se sono json, il font letto dai settings � gi� impostato
		SetDefaultFont();
...
{	
	...
	//tardare la SHOW della finestra pi� possibile, cio� fino alla resume della toolbar/tabbedtoolbar
	...
	{
		//visualizza la finestra se � stata nascosta e reset flag m_bTemporaryHidden
...
void CWizardFormView::OnWizardNext()
{
	...
	if (/*TODO RIMUOVERE TEST SU BATCH FRAME*/TRUE || GetDocument()->GetMasterFrame()->IsKindOf(RUNTIME_CLASS(CWizardBatchFrame)))
		...
		{
			//La dispatch � gi� nella OnWizardFinish, in questo modo non viene dispatchata due volte
			//((CWizardFormDoc*)GetDocument())->DispatchWizardFinish(currentIDD);

			//altera il booleano che indica se la OnWizardFinish proviene da WizardNext o � diretta
			m_bIsDirectCallToWizardFinishCall = FALSE;
			OnWizardFinish();
...
void CWizardFormView::OnWizardFinish()
{
	...
	//questa dispatch NON VA REGISTRATA se la chiamata OnWizardFinish � originata dall'ultima OnWizardNext
	CWizardTabDialog* pTabDlg = ((CWizardTabDialog*)GetTabManager()->GetActiveDlg());

	//se � una chiamata diretta al metodo OnWizardFinish (Non proveniente da wizardNext) funziona normalmente
...
void CWizardFormView::SetWizardButtons(DWORD dwFlags)
{
	...
	else
	{
		...
		if (bOnLastTab)
		{
			// la CanDoBatchExecute del documento va a decidere se la batch pu� essere fatta partire o meno
