...
static const Win32Type g_Shell = WinXP;
...
BCMenu::BCMenu()
{
	...
	m_bitmapBackground= AfxGetThemeManager()->GetBCMenuBitmapBkgColor();
	m_bitmapBackgroundFlag=FALSE;
	m_loadmenu=FALSE;

//EX STATIC member
	// how the menu's are drawn in win9x/NT/2000
	original_drawmode=BCMENU_DRAWMODE_ORIGINAL;
	original_select_disabled=TRUE;
	// how the menu's are drawn in winXP

	xp_drawmode=BCMENU_DRAWMODE_ORIGINAL;
	xp_select_disabled=FALSE;
	xp_draw_3D_bitmaps=TRUE;
	hicolor_bitmaps=FALSE;
	// Variable to set how accelerators are justified. The default mode (TRUE) right
	// justifies them to the right of the longes string in the menu. FALSE
	// just right justifies them.
	xp_space_accelerators=TRUE;
	original_space_accelerators=TRUE;
...
void BCMenuData::SetWideString(const wchar_t *szWideString)
{
	...
	if (szWideString)
    {
		...
		m_szMenuText = new wchar_t[sizeof(wchar_t)*len];
		if (m_szMenuText)
...
{
    ASSERT_TRACE(lpDIS != NULL,"lpDIS parameter cannot be NULL");
...
void BCMenu::DrawItem_Win9xNT2000 (LPDRAWITEMSTRUCT lpDIS)
{
	...
	if(IsWinXPLuna())m_clrBack=AfxGetThemeManager()->GetBCMenuBkgColor();
	else 
		m_clrBack=	AfxGetThemeManager()->GetBCMenuBkgColor();
	...
	if(state & MF_SEPARATOR){
		...
		COLORREF crText = AfxGetThemeManager()->GetBCMenuForeColor();
		CBrush m_brSelect;
		CPen m_penBack;
		int x0,y0,dy;
		...
		m_brSelect.CreateSolidBrush(AfxGetThemeManager()->GetControlsHighlightBkgColor());
		...
		if(state&ODS_SELECTED){ // draw the down edges
			...
			crText = AfxGetThemeManager()->GetControlsHighlightForeColor();
		}
		else {
			CPen *pOldPen = pDC->SelectObject (&m_penBack);
			pDC->FillRect (rect,&m_brBackground);
			...
			if(checkflag && checkmaps){
				...
				if (IsNewShell()){
					if(state&ODS_CHECKED)
						...
						(
							rect2,
							AfxGetThemeManager()->GetBCMenuShadowColor(),
							AfxGetThemeManager()->GetControlsHighlightBkgColor()
						);
					else
						pDC->Draw3dRect
						(
				...
				if(state&ODS_CHECKED){
					...
					col = TBThemeManager::LightenColor(col,0.6);
					...
					rect2.SetRect(rect.left,rect.top+dy,rect.left+m_iconX+4,
                        rect.top+m_iconY+4+dy);
					if (IsNewShell())
						pDC->Draw3dRect(rect2, AfxGetThemeManager()->GetBCMenuShadowColor(),
						AfxGetThemeManager()->GetControlsHighlightBkgColor());
				}
				else{
					pDC->FillRect (rect2,&m_brBackground);
		...
		if(!strText.IsEmpty()){
			...
			if(!(lpDIS->itemState & ODS_GRAYED)){
				...
				if(!(state & ODS_SELECTED)){
					...
					offset.top+=1;
					offset.bottom+=1;
					pDC->SetTextColor(AfxGetThemeManager()->GetButtonFaceHighLightColor());
...
void BCMenu::DrawItem_WinXP (LPDRAWITEMSTRUCT lpDIS)
{
	...
	CRect rect,rect2;
	UINT state = (((BCMenuData*)(lpDIS->itemData))->nFlags);
	COLORREF m_newclrBack=AfxGetThemeManager()->GetBCMenu3DFaceColor();
	COLORREF m_clrBack=AfxGetThemeManager()->GetEnabledControlBkgColor();
	m_clrBack=TBThemeManager::DarkenColor(m_clrBack,0.02);
	CFont m_fontMenu,*pFont=NULL;
	LOGFONT m_lf;
	if(!IsWinXPLuna())m_newclrBack=TBThemeManager::LightenColor(m_newclrBack,0.25);
	...
	if(state & MF_SEPARATOR){
		...
		BOOL checkflag=FALSE;
		COLORREF crText = AfxGetThemeManager()->GetBCMenuForeColor();
		COLORREF crSelect = AfxGetThemeManager()->GetControlsHighlightBkgColor();
		COLORREF crSelectFill;
		if(!IsWinXPLuna())crSelectFill=TBThemeManager::LightenColor(crSelect,0.85);
		else crSelectFill=TBThemeManager::LightenColor(crSelect,0.7);
		CBrush m_brSelect;
		CPen m_penBack;
		int x0,y0,dx,dy;
		int nIconNormal=-1,xoffset=-1,global_offset=-1;
		...
		if(checkflag||standardflag||selectedflag||disableflag){
			if(checkflag && checkmaps){
				...
				if(state&ODS_CHECKED){
					CBrush brushin;
					brushin.CreateSolidBrush(TBThemeManager::LightenColor(crSelect,0.55));
				...
				if(state&ODS_CHECKED){
					CBrush brushin;
					brushin.CreateSolidBrush(TBThemeManager::LightenColor(crSelect,0.85));
					pDC->FillRect(rect2,&brushin);
					brushin.DeleteObject();
					pDC->Draw3dRect(rect2,crSelect,crSelect);
		...
		if(!strText.IsEmpty()){
			...
			if(!(lpDIS->itemState & ODS_GRAYED)){
				...
				offset.bottom+=1;
				if(!IsWinXPLuna()){
					COLORREF graycol= AfxGetThemeManager()->GetDisabledControlForeColor();
					if(!(state&ODS_SELECTED))graycol = TBThemeManager::LightenColor(graycol,0.4);
					pDC->SetTextColor(graycol);
				}
				else pDC->SetTextColor(AfxGetThemeManager()->GetDisabledControlForeColor());
				pDC->DrawText(leftStr,rectt, nFormat);
				if(tablocr!=-1) pDC->DrawText (rightStr,rectt,nFormatr);
			}
			pDC->SetBkMode( iOldMode );
...
BOOL BCMenu::GetBitmapFromImageList(CDC* pDC,CImageList *imglist,int nIndex,CBitmap &bmp)
{
	...
	m_newclrBack=AfxGetThemeManager()->GetBCMenu3DFaceColor();
...
void BCMenu::MeasureItem( LPMEASUREITEMSTRUCT lpMIS )
{
	...
	if(state & MF_SEPARATOR){
		...
		if(IsLunaMenuStyle())
			...
			lpMIS->itemHeight = max(temp, (m_iconY + BCMENU_PAD) / 2);
	}
	else{
		CFont m_fontMenu;
		LOGFONT m_lf;
...
BOOL BCMenu::ModifyODMenuW(wchar_t *lpstrText,UINT nID,int nIconNormal)
{
	...
	do{
		...
		ASSERT_TRACE(mdata,"Failed to create a new BCMenuData structure");
		if(lpstrText)
			mdata->SetWideString(lpstrText);  //SK: modified for dynamic allocation
		mdata->menuIconNormal = -1;
		mdata->xoffset = -1;
...
BOOL BCMenu::ModifyODMenuW(wchar_t *lpstrText,UINT nID,CBitmap *bmp)
{
	if(bmp){
		...
		else temp.Add(bmp,AfxGetThemeManager()->GetBCMenu3DFaceColor());
...
//	if(nFlags & MF_POPUP)m_AllSubMenus.Add((HMENU)nID);
	...
	if (nFlags&MF_OWNERDRAW){
		ASSERT_TRACE(!(nFlags&MF_STRING),"strings not allowed as ownerdraw");
		ModifyMenu(pos,nFlags,nID,(LPCTSTR)mdata);
	}
	else if (nFlags&MF_STRING){
		ASSERT_TRACE(!(nFlags&MF_OWNERDRAW),"ownerdraw not allowed as strings");
		ModifyMenu(pos,nFlags,nID,mdata->GetString());
	}
	else{
		ASSERT_TRACE(nFlags&MF_SEPARATOR,"a separator is required in this context");
...
{
    ASSERT_TRACE(arID,"arID parameter null not allowed");
...
BCMenuData *BCMenu::FindMenuItem(UINT nID)
{
	...
	if (!pData){
		...
		ASSERT_TRACE1(pMenu != this,"Invalid menu option to find: %d",nID);
		if (loc >= 0){
...
{
	ASSERT_VALID(this);
	ASSERT_TRACE(lpszResourceName != NULL,"lpszResourceName parameter null not allowed");
...
void BCMenu::DitherBlt (HDC hdcDest, int nXDest, int nYDest, int nWidth, 
						...
						COLORREF bgcolor)
{
	ASSERT_TRACE(hdcDest && hbm,"DC or bitmap handle parameters null not allowed");
	ASSERT_TRACE2(nWidth > 0 && nHeight > 0,"Both dimensions must be greater than zero: nWidth = %d, nHeight = %d",nWidth,nHeight);
	
	// Create a generic DC for all BitBlts
	HDC hDC = CreateCompatibleDC(hdcDest);
    ASSERT_TRACE(hDC,"Failed to create a compatible DC");
	
	if (hDC)
	{
		// Create a DC for the monochrome DIB section
		HDC bwDC = CreateCompatibleDC(hDC);
		ASSERT_TRACE(bwDC,"Failed to create a compatible DC");
		...
		if (bwDC)
		{
	        ...
	        ASSERT_TRACE(hbmBW,"Failed to create a DIB section");
			...
			if (hbmBW)
			{
				...
				HBRUSH hb = CreateSolidBrush(AfxGetThemeManager()->GetControlsHighlightBkgColor ());
				...
				// bits in the destination DC
				hb = CreateSolidBrush(AfxGetThemeManager()->GetBCMenuShadowColor());
...
void BCMenu::GetFadedBitmap(CBitmap &bmp)
{
	...
	col=AfxGetThemeManager()->GetBCMenu3DFaceColor();
	brush.CreateSolidBrush(col);
	...
	for(int i=0;i<BitMap.bmWidth;++i){
		for(int j=0;j<BitMap.bmHeight;++j){
			...
			if(col!=bgcol)ddc.SetPixel(i,j,TBThemeManager::LightenColor(col,0.3));
		}
	}
...
void BCMenu::GetTransparentBitmap(CBitmap &bmp)
{
	...
	ddc2.CreateCompatibleDC(NULL);
	bmp2.CreateCompatibleBitmap(&ddc,BitMap.bmWidth,BitMap.bmHeight);
	col= AfxGetThemeManager()->GetBCMenuBitmapBkgColor();
	brush.CreateSolidBrush(col);
	CBitmap * pddcOldBmp2 = ddc2.SelectObject(&bmp2);
	CRect rect(0,0,BitMap.bmWidth,BitMap.bmHeight);
	ddc2.FillRect(rect,&brush);
	...
	brush.DeleteObject();
	ddc2.SelectObject(pddcOldBmp2);
	newcol=AfxGetThemeManager()->GetBCMenu3DFaceColor();
...
void BCMenu::GetDisabledBitmap(CBitmap &bmp,COLORREF background)
{
	...
	brush.DeleteObject();
	ddc2.SelectObject(pddcOldBmp2);
	discol=AfxGetThemeManager()->GetBEToolbarBtnShadowColor();
	...
	for(int i=0;i<BitMap.bmWidth;++i){
		for(int j=0;j<BitMap.bmHeight;++j){
			...
			if(col!=bgcol){
				...
				ddc.SetPixel(i,j,TBThemeManager::LightenColor(discol,factor));
			}
			else{
...
{
	CDC ddc;
	COLORREF bgcol,col,shadowcol=AfxGetThemeManager()->GetBEToolbarBtnShadowColor();
	BITMAP BitMap;

	if(!IsWinXPLuna())shadowcol=TBThemeManager::LightenColor(shadowcol,0.49);
	bmp.GetBitmap(&BitMap);
	ddc.CreateCompatibleDC(NULL);
	CBitmap * pddcOldBmp = ddc.SelectObject(&bmp);
...
BOOL BCMenu::AddBitmapToImageList(CImageList *bmplist,UINT nResourceID)
{
	...
	if (m_bDynIcons){
		...
		if(hbmp){
			...
			if(m_bitmapBackgroundFlag){
				...
				if(bmplist->Add(&bmp,AfxGetThemeManager()->GetBCMenu3DFaceColor())>=0)bReturn=TRUE;
			}
			bmp.Detach();
			DeleteObject(hbmp);
		}
			...
			if(mybmp.LoadBitmap(nResourceID)){
				...
				if(m_bitmapBackgroundFlag){
					...
					if(bmplist->Add(&mybmp,AfxGetThemeManager()->GetBCMenu3DFaceColor())>=0)bReturn=TRUE;
				}
			}
		}
	}
	if(bReturn&&IsLunaMenuStyle()&&xp_draw_3D_bitmaps){
		...
		GetFadedBitmap(bmp);
		bmplist->Add(&bmp,AfxGetThemeManager()->GetBCMenu3DFaceColor());
		GetBitmapFromImageList(pDC,bmplist,0,bmp2);
		GetShadowBitmap(bmp2);
		bmplist->Add(&bmp2,AfxGetThemeManager()->GetBCMenu3DFaceColor());
		GetBitmapFromImageList(pDC,bmplist,0,bmp3);
		GetDisabledBitmap(bmp3);
		bmplist->Add(&bmp3,AfxGetThemeManager()->GetBCMenu3DFaceColor());
		pWnd->ReleaseDC(pDC);  // Release the DC
	}
	return(bReturn);
}
                             ...
                             BOOL bSelected, HBITMAP hbmCheck)
{
	...
	CBrush brush;
	COLORREF col;
	if(IsWinXPLuna())col=AfxGetThemeManager()->GetBCMenu3DFaceColor();
	else col=AfxGetThemeManager()->GetBCMenuBitmapBkgColor();
	if(!bSelected)col = TBThemeManager::LightenColor(col,0.6);
	brush.CreateSolidBrush(col);
	dc->FillRect(rcDest,&brush);
	brush.DeleteObject();
	if (IsNewShell()) //SK: looks better on the old shell
		dc->DrawEdge(&rcDest, BDR_SUNKENOUTER, BF_RECT);
	if (!hbmCheck)DrawCheckMark(dc,rc.left+4,rc.top+4,AfxGetThemeManager()->GetBCMenuForeColor());
	else DrawRadioDot(dc,rc.left+5,rc.top+4,AfxGetThemeManager()->GetBCMenuForeColor());
	return TRUE;
}
...
BOOL BCMenu::DrawXPCheckmark(CDC *dc, const CRect& rc, HBITMAP hbmCheck,COLORREF &colorout,BOOL selected)
{
	...
	if(selected) col = TBThemeManager::LightenColor(colorout,0.55);
	else col = TBThemeManager::LightenColor(colorout,0.85);
	brushin.CreateSolidBrush(col);
	dc->FillRect(rc,&brushin);
	brushin.DeleteObject();
	dc->Draw3dRect (rc,colorout,colorout);

	if (!hbmCheck)DrawCheckMark(dc,rc.left+dx,rc.top+dy, AfxGetThemeManager()->GetBCMenuForeColor());
	else DrawRadioDot(dc,rc.left+dx,rc.top+dy, AfxGetThemeManager()->GetBCMenuForeColor());
	return TRUE;
}
						...
						COLORREF bgcolor)
{
	...
	dc.SetBkColor(AfxGetThemeManager()->GetButtonFaceBkgColor());
	ddc.BitBlt(0, 0, nWidth, nHeight, &dc, nXSrc,nYSrc, SRCCOPY);
	dc.SetBkColor(AfxGetThemeManager()->GetBEToolbarBtnHighlightColor());
	...
	//SK: Looks better on the old shell
	dc.SetBkColor(AfxGetThemeManager()->GetBCMenuBkgColor());
	dc.SetTextColor(AfxGetThemeManager()->GetBCMenuForeColor());
	CBrush brShadow, brHilight;
	brHilight.CreateSolidBrush(AfxGetThemeManager()->GetBEToolbarBtnHighlightColor());
	brShadow.CreateSolidBrush(AfxGetThemeManager()->GetBEToolbarBtnShadowColor());
...
BOOL BCMenu::ImageListDuplicate(CImageList *il,int xoffset,CImageList *newlist)
{
	...
	if(IsLunaMenuStyle()&&xp_draw_3D_bitmaps){
		...
		GetFadedBitmap(bmp);
		newlist->Add(&bmp,AfxGetThemeManager()->GetBCMenu3DFaceColor());
		GetBitmapFromImageList(pDC,newlist,0,bmp2);
		GetShadowBitmap(bmp2);
		newlist->Add(&bmp2,AfxGetThemeManager()->GetBCMenu3DFaceColor());
		GetBitmapFromImageList(pDC,newlist,0,bmp3);
		GetDisabledBitmap(bmp3);
		newlist->Add(&bmp3,AfxGetThemeManager()->GetBCMenu3DFaceColor());
		pWnd->ReleaseDC(pDC);  // Release the DC
	}
	return TRUE;
}
...
int BCMenu::RemoveMenu(wchar_t* pText, BC_Seperator sPos)
{
	...
	if(sPos == BCMENU_HEAD)
	{
		ASSERT_TRACE2(nPos - 1 >= 0,"Menu position of item %s must be greater than 1, nPos = %d",pText,nPos);
...
void BCMenu::ColorBitmap(CDC* pDC,CBitmap& bmp,CSize bitmap_size,CSize icon_size,COLORREF fill,COLORREF border,int hatchstyle)
{
	...
	if(IsWinXPLuna())m_newclrBack=AfxGetThemeManager()->GetBCMenu3DFaceColor();
	else m_newclrBack=AfxGetThemeManager()->GetBCMenuBkgColor();
...
int BCMenu::AddToGlobalImageList(CImageList *il,int xoffset,int nID)
{
	...
	if(hIcon){
		...
		if(existsloc>=0){
			...
			if(IsLunaMenuStyle()&&xp_draw_3D_bitmaps){
				if(existsloc+1<numcurrent&&m_AllImagesID[existsloc+1]==nID){
					if(existsloc+2<numcurrent&&m_AllImagesID[existsloc+2]==nID){
						...
						il2.Add(&bmp,AfxGetThemeManager()->GetBCMenu3DFaceColor());
						HICON hIcon2 = il2.ExtractIcon(0);
						m_AllImages.Replace(existsloc+1,hIcon2);
						il2.Add(&bmp2,AfxGetThemeManager()->GetBCMenu3DFaceColor());
						HICON hIcon3 = il2.ExtractIcon(1);
						m_AllImages.Replace(existsloc+2,hIcon3);
						il2.Add(&bmp3,AfxGetThemeManager()->GetBCMenu3DFaceColor());
			...
			loc=numcurrent;
			if(IsLunaMenuStyle()&&xp_draw_3D_bitmaps){
				m_AllImages.Add(&bmp,AfxGetThemeManager()->GetBCMenu3DFaceColor());
				m_AllImages.Add(&bmp2,AfxGetThemeManager()->GetBCMenu3DFaceColor());
				m_AllImages.Add(&bmp3,AfxGetThemeManager()->GetBCMenu3DFaceColor());
				m_AllImagesID.Add(nID);
				m_AllImagesID.Add(nID);
				m_AllImagesID.Add(nID);
			}
...
bool BCMenu::AppendMenu (BCMenu* pMenuToAdd, bool add_separator /*= true*/, int num_items_to_remove_at_end /*= 0*/)
{
	...
	if (pMenuToAdd == NULL || !pMenuToAdd->IsKindOf (RUNTIME_CLASS(BCMenu))){
		ASSERT_TRACE(pMenuToAdd != NULL && pMenuToAdd->IsKindOf (RUNTIME_CLASS(BCMenu)),"Bad pMenuToAdd parameter: either null or not the right kind");
		return false;
	}
		...
		if (original_num_menu_items >= num_items_to_remove_at_end)
		{
			...
			for (int i=first_delete_item; i>=last_delete_item; i--){
				if (!DeleteMenu (i, MF_BYPOSITION)){
					ASSERT_TRACE1(FALSE,"Error deleting at position i = %d",i); // Something went wrong, but not so critical to abort everything. Just stop deleting items.
					break;
				}
			}
		}else{
			ASSERT_TRACE(FALSE,"Number of items to delete is greater than existing number of menu items"); // Number of items to delete is greater than existing number of menu items!
		}
	}
	...
	for (int iLoop = 0; iLoop < iMenuAddItemCount; iLoop++ )
	{
		...
		if (!pSubMenu)
		{
			...
			if (!AppendMenu( nState, nItemID, (LPTSTR)(LPCTSTR)sMenuAddString )){
				ASSERT_TRACE(FALSE,"AppendMenu failed"); //TRACE( "MergeMenu: AppendMenu failed!\n" );
				return false;
			}
		}
		else
		{
			...
			if (pNewPopupMenu == NULL){
				ASSERT_TRACE(pNewPopupMenu != NULL,"CreatePopupMenu failed"); //TRACE( "MergeMenu: CreatePopupMenu failed!\n" );
				return false;
			}
...
CTBMenu::CTBMenu() 
{ 
	SetBitmapBackground(AfxGetThemeManager()->GetBCMenuBitmapBkgColor());
}

//-------------------------------------------------------------------------------------
BOOL CTBMenu::AppendMenu(UINT nFlags, UINT_PTR nIDNewItem /*= NULL*/, LPCTSTR lpszNewItem /*= NULL*/, UINT nIDImg /*= 0*/)
{	
	if (nIDImg > 0)
	{
		CBitmap cBmp;
		LoadBitmapOrPng(&cBmp, nIDImg);
		__super::AppendMenu	(nFlags, nIDNewItem, (LPTSTR) lpszNewItem, &cBmp);
	}
	else
	{
		__super::AppendMenu	(nFlags, nIDNewItem, (LPTSTR) lpszNewItem);
	}
	return FALSE;
}
