...
void DataObj::AssignStatus(const DataObj& aDataObj)
{
	...
	if (aDataObj.IsPrivate()) //@@BAUZI lo stato di PRIVATE � legato al singolo valore del dato 
		SetPrivate();
}
//-----------------------------------------------------------------------------
bool DataObj::AlignHKL(HotKeyLink* pHKL)
{
	//se il puntatore � presente in lista, vuol dire che la find per questo valore � stata fatta
...
{
	Signal(ON_CHANGED);
	m_arAlignedHKLs.RemoveAll();//tutti gli evedntuali hotlink dovranno rieffettuare la find, perch� il valore � cambiato
}
//-----------------------------------------------------------------------------
...
void DataObj::Clear(BOOL bValid)
{
	...
	if (IsPrivate()) //@@BAUZI lo stato di PRIVATE � legato al singolo valore del dato
		SetPrivate(FALSE);
...
void DataStr::StripBlank()
{
	...
	m_strText.TrimRight(BLANK_CHAR); //bugfix 24701 (When used with no parameters, TrimRight removes trailing newline, space, and tab characters from the string.) 
									// invece � necessario eliminare solo il ' '
...
BOOL DataDate::SetTime(const UWORD wHour, const UWORD wMin, const UWORD wSec)
{
	...
	if (wHour >= 0 && wHour <= 23 && wMin >= 0 && wMin <= 59 && wSec >= 0 && wSec <= 59)
	{
		...
		// data � nulldate. Non sappiamo perch�, il controllo risale a . 
		// i tempi di MagoXp
		if (IsNullDate(m_DateStruct))
			return FALSE;
...
// occorre considerarlo unsigned per compatibilit� con TBC#
//-----------------------------------------------------------------------------
...
BOOL DataArray::FixDataType(DataType newType)
{
	for (int i = 0; i < m_arData.GetCount(); i++)
	{
		...
		if (!DataType::IsCompatible(newType, tv))
		{
			...
			SetAt(i, pVal);	//la delete di pE � interna
		}
	}
...
void RDEData::SetData(RDEcmd Status, const DataObj* pDataObj, BOOL bIsTailMultiLineString /* = FALSE */)
{
	...
	if (pDataObj)
	{
		...
		if (m_Len > 0x7FFF)
		{
			...
			m_dsOverflowError = cwsprintf(_TB("*** The string value is too long: {0-%d}, the max allowed length is {1-%d} ***"), m_Len / 2 - 3, (0x7FFF / 2)); // /2 � per l'unicode
