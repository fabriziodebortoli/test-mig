...
{ 
	...
	// se il font � di report, la traduzione potrebbe non essere disponibile
...
// scaletta delle priorit� � la seguente:
...
//	5) l'ultimo caricato
//	- a parit� di font, il font custom � pi� forte di quello standard
//------------------------------------------------------------------------------
FontStyle* FontStylesGroup::BestFontForContext (CTBNamespace* pContext) const
{
	...
	// Cerco il mio corrispondente preciso, e mi predispongo gi� quello 
	...
	for (int i=0; i <= m_FontsStyles.GetUpperBound(); i++)
	{
		...
		if (pFont->GetOwner() == *pContext && HasPriority(pExactFont, pFont, pContext))
			pExactFont = pFont;	//TODO perch� non fa return ?
...
int	FontStyleTable::AddFont (FontStyle* pFont)
{
	...
	// cerca se esiste gi� un formattatore con lo stesso nome rappresentato dal suo gruppo
	...
	// se non c'� ancora il gruppo lo creo
	if (!pFontsGroup)
	{
...
// � ammesso personalizzare sulla singola stazione il facename e il charset del font 
