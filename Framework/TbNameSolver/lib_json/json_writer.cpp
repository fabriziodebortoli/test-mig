...
namespace Json {
	...
	std::wstring valueToString(Int value, Char* buffer)
	{
		Char *current = buffer + BUFF_SIZE;
	...
	std::wstring valueToString(UInt value, Char* buffer)
	{
		Char *current = buffer + BUFF_SIZE;
	...
	std::wstring valueToString(double value, Char* buffer)
	{
		swprintf_s(buffer, BUFF_SIZE, L"%#.16g", value);

		Char* ch = buffer + wcslen(buffer) - 1;
	...
	std::wstring valueToQuotedString(const Char *value)
	{
		...
		FastWriter::writeValue(const Value &value)
	{
		...
		case nullValue:
			...
			break;
		case intValue:
			document_ += valueToString(value.asInt(), buffer);
			break;
		case uintValue:
			document_ += valueToString(value.asUInt(), buffer);
			break;
		case realValue:
			document_ += valueToString(value.asDouble(), buffer);
			break;
		case stringValue:
		...
		StyledWriter::writeValue(const Value &value)
	{
		...
		case nullValue:
			...
			break;
		case intValue:
			pushValue(valueToString(value.asInt(), buffer));
			break;
		case uintValue:
			pushValue(valueToString(value.asUInt(), buffer));
			break;
		case realValue:
			pushValue(valueToString(value.asDouble(), buffer));
			break;
		case stringValue:
	...
	void
		StyledWriter::indent()
	{
		indentString_ += std::wstring(indentSize_, ' ');
	}
