...
#define CMP_RETURN(token) if (_tcscmp(szName, L#token) == 0) return token;
//-----------------------------------------------------------------------------
CTBNamespace::NSObjectType CTBNamespace::FromString(LPCTSTR szName)
{
	CMP_RETURN(NOT_VALID);
	CMP_RETURN(MODULE);
	CMP_RETURN(LIBRARY);
	CMP_RETURN(DOCUMENT);
	CMP_RETURN(REPORT);
	...
	CMP_RETURN(TABDLG);
	CMP_RETURN(GRID);
	CMP_RETURN(GRIDCOLUMN);
	CMP_RETURN(CONTROL);
	CMP_RETURN(EVENTHANDLER);
	CMP_RETURN(PROCEDURE);
	CMP_RETURN(DATAFILE);
	CMP_RETURN(ALIAS);
	CMP_RETURN(PROFILE);
	CMP_RETURN(EXCELDOCUMENT);
	CMP_RETURN(EXCELTEMPLATE);
	CMP_RETURN(WORDDOCUMENT);
	CMP_RETURN(WORDTEMPLATE);
	CMP_RETURN(TOOLBARBUTTON);
	CMP_RETURN(REPORTSCHEMA);
	CMP_RETURN(DOCUMENTSCHEMA);
	CMP_RETURN(VIRTUAL_TABLE);
	CMP_RETURN(TOOLBAR);
	CMP_RETURN(ENTITY);
	CMP_RETURN(BEHAVIOUR);
	CMP_RETURN(PDF);
	CMP_RETURN(RTF);
	CMP_RETURN(ODS);
	CMP_RETURN(ODT);
	CMP_RETURN(BARPANEL);
	CMP_RETURN(TILEPANEL);
	CMP_RETURN(TILEPANELTAB);
	CMP_RETURN(JSON);
	CMP_RETURN(ITEMSOURCE);
	CMP_RETURN(HOTFILTER);
	CMP_RETURN(VALIDATOR);
	CMP_RETURN(DATA_ADAPTER);
	CMP_RETURN(CONTEXT_MENU);
	CMP_RETURN(CONTROL_BEHAVIOUR);
	return NOT_VALID;
}
// inline functions 
//-----------------------------------------------------------------------------
