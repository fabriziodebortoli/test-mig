...
protected:
	...
	BOOL						m_bClosing;			// indica se il documento � in fase di chiusura
	BOOL						m_bRetryingLock;	// indica se il documento ha dei tentativi di lock in corso

	CXMLDataManagerObj*			m_pXMLDataManager;
	BOOL						m_bUnattendedMode;	// se il documento � istanziato senza interfaccia visibile (vedi importo\export o scheduler)
	
	DataObj*					m_pAssignedCounter;	// tiene traccia del fatto che al documento � stato assegnato un identificatore per evitare di assegnarlo pi� volte
...
public :
	...
	CSingleExtDocTemplate*	m_pTemplate; //Template da cui � stato istanziato
...
public:
	...
	BOOL CanRunDocumentInStandAloneMode(); //restituisce TRUE se non c'� un unico documento aperto e se non ci sono altri utenti connessi all'azienda, FALSE altrimenti
										   //---------------------------------------------------------------------------
...
public:
	...
	// determina se devo o meno aggiornare un record gi� esistente
	...
	// questo comportamento pu� essere abilitato invocando EnableCounterCaching(TRUE);
	// di default il comportamento � disattivo (per compatibilit� col passato, onde evitare
	...
	// Dice se il documento � in modalit� unattended (perch� gestisce solo logiche 
	// di business senza interfaccia (ad es. in fase import/export) o perch� lanciato 
	// dallo scheduler
