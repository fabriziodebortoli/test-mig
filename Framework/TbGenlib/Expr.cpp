...
//ci sono due metodi quasi uguali ma � difficile unificarli perch� differiscono per alcune righe
//il metodo con il dataobj reference � quello storico ed � virtuale quindi potrebbe essere stato reimplementato
...
void Expression::AssignResult(DataObj& d, ExpItemVal& res)
{
	...
	switch (d.GetDataType().m_wType)
	{
		...
		default: TRACE(_T(" Il tipo del DataObj che deve contenere il risultato dell�espressione non � dei tipi supportati.")); ASSERT(FALSE);		// se succede vuol dire che le mappe di compatibilita` sono errate
	} // switch
