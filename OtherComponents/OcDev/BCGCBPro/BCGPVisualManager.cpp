...
void CBCGPVisualManager::ShowFrameShadows(BOOL bShow)
{
	...
	if (m_bCanShowFrameShadows != bShow)
	{
		...
		if (pThis != NULL && bCanShow != pThis->CanShowFrameShadows())
		{
			...
			if (DYNAMIC_DOWNCAST(CDialog, pWndMain) != NULL || DYNAMIC_DOWNCAST(CPropertySheet, pWndMain) != NULL)
			{
...
void CBCGPVisualManager::OnDrawTabDot(CDC* pDC, CBCGPBaseTabWnd* pWndTab, int nShape, const CRect& recrIn, int iTab, BOOL bIsActive, BOOL bIsHighlighted)
{
	...
	COLORREF clrShadow = (pWndTab->IsDialogControl () && !pWndTab->IsVisualManagerStyle ()) ? globalData.clrBtnShadow : globalData.clrBarShadow;
	COLORREF clrHighlight = (pWndTab->IsDialogControl () && !pWndTab->IsVisualManagerStyle ()) ? GetSysColor(COLOR_HIGHLIGHT) : globalData.clrHilite;
...
void CBCGPVisualManager::OnDrawRibbonMainButtonLabel(CDC* pDC, CBCGPRibbonButton* pButton, CRect rectText, const CString strText)
{
	...
	if (globalData.IsHighContastMode())
	{
		pButton->DoDrawText(pDC, strLabel, rectText, nDTFlags, globalData.clrWindowText);
	}
	else
	{
		pButton->DoDrawText(pDC, strLabel, rectShadow, nDTFlags, RGB(100, 100, 100));
...
void CBCGPVisualManager::OnDrawControlInfoTip(CDC* pDC, CRect rect, CWnd* pWndCtrl)
{
	...
	if (rect.Height() > 10)
	{
		dm.DrawLine(x, y + 5, x, rect.bottom - 4, clrText);
	}
}
//*******************************************************************************
