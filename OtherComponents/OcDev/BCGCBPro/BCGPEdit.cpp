	...
	if (m_Mode == BrowseMode_PopupDialog)
	{
		...
		else
		{
			...
			if (pPopupMenu != NULL)
			{
				...
				pPopupMenu->Create(this, rectWindow.left - 1, rectWindow.bottom, NULL, FALSE, FALSE);
			}
		}
		return;
	}
...
//********************************************************************************
void CBCGPEdit::EnablePopupDialog(CRuntimeClass* pRTI, UINT nIDTemplate, BOOL bIsResizable, BOOL bComboBoxMode)
{
	EnablePopupDialog(pRTI, MAKEINTRESOURCE(nIDTemplate), bIsResizable, bComboBoxMode);
}
//********************************************************************************
void CBCGPEdit::EnablePopupDialog(CRuntimeClass* pRTI, LPCTSTR lpszTemplateName, BOOL bIsResizable, BOOL bComboBoxMode)
{
	ASSERT(pRTI != NULL);
...
#ifndef _BCGSUITE_
	return (globalData.m_nThemedScrollBars & BCGP_THEMED_SCROLLBAR_EDITBOX) != 0;
#else
	return FALSE;
#endif
}
