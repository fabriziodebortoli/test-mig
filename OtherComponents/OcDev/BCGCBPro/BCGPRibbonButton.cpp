...
CSize CBCGPRibbonButton::DrawBottomText (CDC* pDC, BOOL bCalcOnly)
{
	...
	int nMenuArrowWidth = globalUtils.ScaleByDPI((HasMenu () || IsDefaultPanelButton ()) ? (CBCGPMenuImages::Size ().cx) : 0);
	...
	else
	{
		...
		if (pPopupMenu != NULL)
		{
			...
			pPopupMenu->Create(pWndEmbedded->GetSafeHwnd() == NULL ? pWndParent : pWndEmbedded, 
				rect.left, rect.bottom, NULL, FALSE, FALSE);
...
//********************************************************************************
void CBCGPRibbonButton::EnablePopupDialog(CRuntimeClass* pRTI, UINT nIDTemplate, BOOL bIsResizable, BOOL bIsDefaultCommand)
{
	EnablePopupDialog(pRTI, MAKEINTRESOURCE(nIDTemplate), bIsResizable, bIsDefaultCommand);
}
//********************************************************************************
void CBCGPRibbonButton::EnablePopupDialog(CRuntimeClass* pRTI, LPCTSTR lpszTemplateName, BOOL bIsResizable, BOOL bIsDefaultCommand)
{
	ASSERT(pRTI != NULL);
