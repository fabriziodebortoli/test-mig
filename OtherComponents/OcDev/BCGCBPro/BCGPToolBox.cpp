...
int CBCGPToolBoxPage::AddButton(LPCTSTR lpszText, HICON hIcon, BOOL bAlphaBlend)
{
	...
	if (m_Images.IsScaled())
	{
		...
		imageIcon.SetImageSize(CSize(bmp.bmWidth, bmp.bmHeight));
...
HRESULT CBCGPToolBoxEx::get_accChild(VARIANT varChild, IDispatch** ppdispChild)
{
	...
	if ((varChild.vt == VT_I4) && (varChild.lVal >= 1))
	{
		...
		if (m_bFilterBox)
		{
			if (varChild.lVal == 1)
			{
				hWnd = m_wndFilter.GetSafeHwnd();
			}
			else
			{
				hWnd = GetPage((int)varChild.lVal - 2, TRUE)->GetSafeHwnd();
			}
		}
		else
#endif
		{
			hWnd = GetPage((int)varChild.lVal - 1, TRUE)->GetSafeHwnd();
		}

		if (hWnd != NULL)
		{
			*ppdispChild = NULL;
			AccessibleObjectFromWindow(hWnd, (DWORD)OBJID_CLIENT, IID_IAccessible, (void**)ppdispChild);
...
void CBCGPToolBoxEx::EnableSearchBox(BOOL bEnable, LPCTSTR lpszPrompt, LPCTSTR lpszOutOfFilter)
{
	...
	if (bEnable)
	{
		if (m_wndFilter.GetSafeHwnd() == NULL)
		{
			...
			editDummy.SetFont(&globalData.fontRegular);
			...
			m_wndFilter.SetFont(&globalData.fontRegular);
