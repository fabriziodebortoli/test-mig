...
//******************************************************************************
void CBCGPRibbonCommandsListBox::FillFromCategory (CBCGPRibbonCategory* pCategory, BOOL bExludeDefaultPanelButtons/* = FALSE*/)
{
	ASSERT_VALID (this);
	...
	FillFromArray (arElements, TRUE, TRUE, bExludeDefaultPanelButtons);
...
void CBCGPRibbonCommandsListBox::FillFromArray (
		...
		BOOL bIgnoreSeparators, BOOL bExludeDefaultPanelButtons)
{
	ASSERT_VALID (this);
	...
	for (int i = 0; i < arElements.GetSize (); i++)
	{
		...
		if (bExludeDefaultPanelButtons && pElem->IsKindOf (RUNTIME_CLASS (CBCGPRibbonDefaultPanelButton)))
		{
			continue;
		}
