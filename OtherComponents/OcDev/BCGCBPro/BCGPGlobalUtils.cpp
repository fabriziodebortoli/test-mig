...
{
	return globalData.GetRibbonImageScale() == 1.0 ? val : (int)(.5 + globalData.GetRibbonImageScale() * val);
}
...
CSize CBCGPGlobalUtils::ScaleByDPI(CSize size)
{
	...
	{
		size.cx = (int)(.5 + globalData.GetRibbonImageScale() * size.cx);
		size.cy = (int)(.5 + globalData.GetRibbonImageScale() * size.cy);
	}
