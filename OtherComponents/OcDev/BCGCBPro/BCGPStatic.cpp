	...
	if ((dwStyle & SS_ICON) == SS_ICON)
	{
		...
		if (hIcon != NULL)
		{
			...
			if ((dwStyle & SS_REALSIZEIMAGE) == SS_REALSIZEIMAGE)
			{
				...
				sizeIcon.cx = bitmap.bmWidth;
				sizeIcon.cy = bitmap.bmHeight;

				if ((dwStyle & SS_CENTERIMAGE) == SS_CENTERIMAGE)
				{
					rectText.left += max(0, (rectText.Width() - sizeIcon.cx) / 2);
					rectText.right = rectText.left + sizeIcon.cx;

					rectText.top += max(0, (rectText.Height() - sizeIcon.cy) / 2);
...
#endif
			{
				::DrawIconEx(pDC->GetSafeHdc(), rectText.left, rectText.top, hIcon, sizeIcon.cx, sizeIcon.cy, 0, NULL, DI_NORMAL);
			}
		}
