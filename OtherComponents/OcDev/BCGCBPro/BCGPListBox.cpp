...
void CBCGPListBox::OnDrawItemContent(CDC* pDC, CRect rect, int nIndex)
{
	...
	if (m_bHasColorBars)
	{
		...
		rectColorBar.left++;
		rectColorBar.right = rectColorBar.left + globalUtils.ScaleByDPI(5);
		rectColorBar.DeflateRect(0, globalUtils.ScaleByDPI(1));
		
		COLORREF color = GetItemBarColor(nIndex);
		if (color != (COLORREF)-1)
		{
			OnDrawItemColorBar(pDC, rectColorBar, nIndex, color);
		}
		
		rect.left = rectColorBar.right;
	}
	...
	if (!bIsCaption)
	{
		if (m_bHasCheckBoxes)
		{
			...
			{
				sizeCheckBox = CBCGPVisualManager::GetInstance ()->CBCGPVisualManager::GetCheckRadioDefaultSize();
			}
