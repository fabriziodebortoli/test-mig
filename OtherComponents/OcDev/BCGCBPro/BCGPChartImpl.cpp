							  ...
							  CBCGPChartSeries* pSeries, int nDataPointIndex)
{
	...
	const double dblRadiusX1 = 0.5 * dblRadiusX * (1.0 + dblDoughnutPercent);
	const double dblRadiusY1 = 0.5 * dblRadiusY * (1.0 + dblDoughnutPercent);

	double dblDiff = ptAngles.y > ptAngles.x ? ptAngles.x + (ptAngles.y - ptAngles.x) / 2 : ptAngles.x + (ptAngles.x - ptAngles.y) / 2;
	CBCGPPoint ptPieCenter (ptCenter.x + dblRadiusX1 * sin(dblDiff), ptCenter.y - dblRadiusY1 * cos(dblDiff));
