...
BOOL CBCGPScrollBarImpl::ProcessMouseUp(CWnd* pWnd, CPoint point)
{
	...
	if (bSendEvent && pWndNotify->GetSafeHwnd () != NULL)
	{
		...
		if (bThumb)
		{
			...
			pWndNotify->SendMessage(dwScroll, MAKEWPARAM(SB_THUMBPOSITION, _GetScrollPos(pWnd)), (LPARAM)(m_bInternal ? 0 : pWnd->GetSafeHwnd()));
		}
...
LRESULT CBCGPInternalScrollBarWrapper::InternalSB_WrapperWindowProc(UINT message, WPARAM wParam, LPARAM lParam, BOOL& bProcessed)
{
	...
	if (message == BCGM_CHANGEVISUALMANAGER && !m_bSupressMessage)
	{
	...
	else if (message == WM_PRINT && !m_bSupressMessage && m_dwScrool != 0)
	{
	...
	else if (message == WM_NCPAINT && !m_bSupressMessage && m_dwScrool != 0)
	{
		m_bSupressMessage = TRUE;
	...
	else if (message == WM_NCHITTEST && !m_bSupressMessage)
	{
		HWND hwndOwner = m_pWndOwner->GetSafeHwnd();
	...
	else if ((message == WM_STYLECHANGING || message == WM_STYLECHANGED) && !m_bSupressMessage)
	{
