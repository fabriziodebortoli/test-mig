...
void CBCGPGridItem::OnDrawIcon (CDC* pDC, CRect& rect)
{
	...
	if (pWndList->GetImageList () != NULL && m_iImage >= 0)
	{
		...
		if (rect.left + cx <= rect.right)
		{
			...
			CPoint pt = CPoint(bRightAlign ? rect.right - cx - 1 : rect.left + 1, (rect.top + 1 + rect.bottom - cy) / 2);
...
void CBCGPGridCheckItem::OnDrawValue (CDC* pDC, CRect rect)
{
	...
	CSize sizeCheckBox = CBCGPVisualManager::GetInstance ()->GetCheckRadioDefaultSize();
	...
	{
		rectCheck.left = rect.CenterPoint ().x - sizeCheckBox.cx / 2;
	...
	if (rectCheck.Width() != rectCheck.Height())
	{
		...
		{
			const DWORD dwVAlign = GetVertAlign ();

			if (dwVAlign == BCGP_GRID_ITEM_VTOP)
			{
				rectCheck.top = rect.top;
				rectCheck.bottom = rectCheck.top + rectCheck.Width();
			}
			else if (dwVAlign == BCGP_GRID_ITEM_VBOTTOM)
			{
				rectCheck.bottom = rect.bottom;
				rectCheck.top = rectCheck.bottom - rectCheck.Width();
			}
			else // BCGP_GRID_ITEM_VCENTER
			{
	...
	if (m_b3State)
	{
		visualManager->OnDrawCheckBoxEx(pDC, rectCheck, (long) m_varValue, FALSE, FALSE, m_bEnabled);
	} 
	else
	{
		visualManager->OnDrawCheckBox(pDC, rectCheck, FALSE, (bool) m_varValue, m_bEnabled);
	}
...
CBCGPGridPopupDlgItem::CBCGPGridPopupDlgItem () :
	CBCGPGridItem (_variant_t ((LPCTSTR) _T("")), 0),
	m_pPopupDlgRTI (NULL), m_lpszPopupDlgTemplateName (NULL), m_bPopupDlgIsResizable (FALSE),
	m_pPopupDlg (NULL)
{
}
//*****************************************************************************************
CBCGPGridPopupDlgItem::CBCGPGridPopupDlgItem(CString str, CRuntimeClass* pRTI, UINT nIDTemplate, BOOL bIsResizable) :
	CBCGPGridItem (_variant_t ((LPCTSTR) str), 0),
	m_pPopupDlgRTI (pRTI), m_lpszPopupDlgTemplateName (NULL), m_bPopupDlgIsResizable (bIsResizable),
	m_pPopupDlg (NULL)
{
...
//*****************************************************************************************
void CBCGPGridPopupDlgItem::EnablePopupDialog(CRuntimeClass* pRTI, UINT nIDTemplate, BOOL bIsResizable)
{
	m_pPopupDlgRTI = pRTI;
	m_lpszPopupDlgTemplateName = MAKEINTRESOURCE(nIDTemplate);
...
BOOL CBCGPGridPopupDlgItem::CreatePopupDlg (CRect rectEdit)
{
	...
	if (m_pPopupDlgRTI != NULL && m_lpszPopupDlgTemplateName != NULL)
	{
		...
		if (pPopupMenu != NULL)
		{
			...
			pPopupMenu->Create (pWndGrid, rectCombo.left, rectCombo.bottom, NULL, FALSE, FALSE);
...
HRESULT CBCGPGridRow::get_accParent(IDispatch **ppdispParent)
{
	...
	*ppdispParent = NULL;
	...
	return AccessibleObjectFromWindow(m_pWndList->GetSafeHwnd(), (DWORD)OBJID_WINDOW, IID_IAccessible, (void**)ppdispParent);
}
//*******************************************************************************************
...
void CBCGPGridCtrl::InitColors ()
{
	...
	COLORREF clrLineDefault = !globalData.IsHighContastMode() ? globalData.clrBtnFace : RGB(0, 0, 0);
...
void CBCGPGridCtrl::OnDraw(CDC* pDCPaint)
{
	...
	{
		pDCPaint->FillRect (rectGripper, m_bControlBarColors ? &globalData.brBarFace : &globalData.brBtnFace);
	}
...
void CBCGPGridCtrl::OnDrawList (CDC* pDC)
{
	...
	if (IsEmpty() || m_bOutOfFilter)
	{
		...
		if (!strLabel.IsEmpty())
		{
			COLORREF clrOld = pDC->SetTextColor(m_bOutOfFilter ? globalData.clrBarDkShadow : GetTextColor());
		...
		if (pItem->m_Rect.bottom >= m_rectClip.top)
		{
			...
			if (GetColumnsInfo ().GetColumnCount (TRUE) > 0)
			{
				...
				if (nCol0Idx != -1)
				{
					...
					if (nCol0Width > 0 && !m_bHighlightGroups || !pItem->IsGroup ())
					{
...
void CBCGPGridCtrl::OnDrawHeaderItemBorder (CDC* pDC, CRect rect, int nCol)
{
	...
	params.m_nItemSelected = (!IsWholeRowSel() && IsColumnSelected(nCol)) ? CBCGPGridHeaderParams::Selected : CBCGPGridHeaderParams::NotSelected;
	...
	for (int i = 0; i < GetColumnsInfo ().GetGroupColumnCount (); i++)
	{
		...
		if (nCol != -1)
		{
			...
			int nItemWidth = pDC->GetTextExtent (strColumn).cx + 
				BCGPGRID_GROUPBYBOX_COLUMNWIDTH + GRID_TEXT_MARGIN;
...
//********************************************************************************
void CBCGPGridCtrl::SetVisualManagerColorTheme(BOOL bSet, BOOL bRedraw, BOOL bAutoUpdateOnVMChange)
{
...
CFont& CBCGPGridCtrl::GetUnderlineFont(BOOL bScaled)
{
	...
	return CBCGPGridCtrl::m_bUseSystemFont ? globalData.fontDefaultGUIUnderline : globalData.fontUnderline;
}
//********************************************************************************
...
void CBCGPGridCtrl::pfnCallbackExportItem (CBCGPGridItem* pItem, const CBCGPGridRange&, LPARAM lParam)
{
	...
	if (params != NULL)
	{
		...
		{
			strTD.Format(_T("<TD style=\"%s\">"), strColors);
		}
