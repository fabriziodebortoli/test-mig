...
{
	if (globalData.m_nBitsPerPixel <= 8 || globalData.IsHighContastMode () || 
		(pTabWnd->IsDialogControl() && !pTabWnd->IsPropertySheetTab()) || !IsDarkTheme())
	{
...
void CBCGPVisualManagerVS2012::OnDrawTabDot(CDC* pDC, CBCGPBaseTabWnd* pWndTab, int nShape, const CRect& rectIn, int iTab, BOOL bIsActive, BOOL bIsHighlighted)
{
	...
	if (globalData.m_nBitsPerPixel <= 8 || globalData.IsHighContastMode () || m_Style == VS2012_LightBlue ||
		(pWndTab->IsDialogControl () && !pWndTab->IsVisualManagerStyle ()))
	{
...
COLORREF CBCGPVisualManagerVS2012::GetStatusBarPaneTextColor (CBCGPStatusBar* pStatusBar, CBCGStatusBarPaneInfo* pPane)
{
	...
	if (GetRValue (m_clrAccent) <= 128 || GetGValue (m_clrAccent) <= 128 || GetBValue (m_clrAccent) <= 128)
	{
		return (pPane->nStyle & SBPS_DISABLED) ? CBCGPDrawManager::ColorMakeLighter(m_clrAccent, .5) : RGB(255, 255, 255);
	}
...
BOOL CBCGPVisualManagerVS2012::OnEraseMDIClientArea (CDC* pDC, CRect rectClient)
{
	if (globalData.m_nBitsPerPixel > 8 && !globalData.IsHighContastMode ())
	{
		...
		{
			pDC->FillRect(rectClient, m_Style != VS2012_Dark ? &globalData.brBarFace : &m_brHighlight);
			return TRUE;
		}
	}
...
void CBCGPVisualManagerVS2012::OnDrawRibbonMainButtonLabel(CDC* pDC, CBCGPRibbonButton* pButton, CRect rectText, const CString strText)
{
	...
	pButton->DoDrawText(pDC, strLabel, rectText, DT_SINGLELINE | DT_CENTER | DT_VCENTER, RGB(253, 253, 253));
}
//****************************************************************************
					...
					COLORREF& clrFill)
{
	...
	if (DYNAMIC_DOWNCAST(CBCGPRibbonStatusBar, pSlider->GetParentRibbonBar()) != NULL && IsRibbonStatusBarDark())
	{
			...
			BOOL bIsHighlighted, BOOL bIsPressed, BOOL bIsDisabled)
{
	...
	if ((bIsHighlighted || bIsPressed) && m_Style == VS2012_Dark)
	{
	...
	if (nState == 1)
	{
		...
		else
		{
			...
			CBCGPMenuImages::Draw (pDC, CBCGPMenuImages::IdCheck, rect,
				bHighlighted ? CBCGPMenuImages::ImageBlack2 : CBCGPMenuImages::ImageGray, sizeDest);
		}
	}
	else if (nState == 2)
	{
