	...
	if (m_nIndex >= 0)
	{
		if (!m_pOwner->IsDisabled () || m_pOwner->IsDrawDisabledItems())
		{
			...
			if (m_pOwner->m_bDefaultButtonStyle || !m_pOwner->m_bIsOwnerDraw)
			{
				...
				if (m_bIsDisabled || (!m_pOwner->IsFocused() && !m_pOwner->m_bDrawSelectionAlways))
				{
					m_bIsChecked = FALSE;
				}
...
CSize CBCGPRibbonPaletteIcon::GetRegularSize (CDC* pDC)
{
	...
	if (!m_strDescription.IsEmpty())
	{
		...
		CFont* pOldFont = pDC->SelectObject((pTopLevelRibbon->GetSafeHwnd() != NULL && m_pOwner->m_pParentControl == NULL) ? pTopLevelRibbon->GetBoldFont() : &globalData.fontBold);
												  ...
												  COLORREF clrText)
{
	...
	{
		CRect rectText = rectIcon;
		rectText.left += GetIconSize().cx + globalUtils.ScaleByDPI(pIcon->m_szMargin.cx);
