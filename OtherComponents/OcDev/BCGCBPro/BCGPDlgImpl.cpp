...
//*******************************************************************************************
void CBCGPDlgImpl::SetControlInfoTip(UINT nCtrlID, LPCTSTR lpszInfoTip, DWORD dwVertAlign, BOOL bRedrawInfoTip)
{
	...
	if (m_Dlg.GetSafeHwnd() == NULL)
	{
		...
		{
			m_mapCtrlInfoTipDelayed.SetAt(nCtrlID, CBCGPControlInfoTip(NULL, lpszInfoTip, dwVertAlign));
		}

		return;
	}

	SetControlInfoTip(m_Dlg.GetDlgItem(nCtrlID), lpszInfoTip, dwVertAlign, bRedrawInfoTip);
}
//*******************************************************************************************
void CBCGPDlgImpl::SetControlInfoTip(CWnd* pWndCtrl, LPCTSTR lpszInfoTip, DWORD dwVertAlign, BOOL bRedrawInfoTip)
{
	...
	{
		m_mapCtrlInfoTip.SetAt(hwndCtrl, CBCGPControlInfoTip(pWndCtrl, lpszInfoTip, dwVertAlign));
	}

	if (bRedrawInfoTip && m_Dlg.GetSafeHwnd() != NULL)
	{
		CRect rect;
		GetControlInfoTipRect(pWndCtrl, rect, dwVertAlign);
	...
	case WM_MOUSEMOVE:
		{
			...
			if (m_pToolTipInfo->GetSafeHwnd () != NULL && pMsg->hwnd == m_Dlg.GetSafeHwnd() && !m_mapCtrlInfoTip.IsEmpty())
			{
				...
				HWND hwnd = GetControlInfoTipFromPoint(pt, strTipText, strDescription);
		...
		if (hSysMenu != NULL)
		{
			...
			if ((m_Dlg.GetExStyle() & WS_EX_TOOLWINDOW) == 0)
			{
				...
				if ((dwStyle & DS_CONTEXTHELP) == DS_CONTEXTHELP ||
					(m_Dlg.GetExStyle() & WS_EX_CONTEXTHELP) == WS_EX_CONTEXTHELP)
				{
...
//****************************************************************************
void CBCGPDlgImpl::GetControlInfoTipRect(CWnd* pWndCtrl, CRect& rect, DWORD dwVertAlign)
{
	...
	const int nSize = globalUtils.ScaleByDPI(12);
	...
	rect.OffsetRect(rect.Width() + nSize / 3, nVertOffset);
...
void CBCGPDlgImpl::DrawControlInfoTips(CDC* pDC)
{
	...
	for (POSITION pos = m_mapCtrlInfoTip.GetStartPosition(); pos != NULL;)
	{
		...
		{
			CRect rectWnd;
			GetControlInfoTipRect(pWnd, rectWnd, info.m_dwVertAlign);

			m_mapCtrlInfoTip[hwndCtrl].m_rect = rectWnd;
...
//****************************************************************************
HWND CBCGPDlgImpl::GetControlInfoTipFromPoint(CPoint point, CString& strInfo, CString& strDescription)
{
	for (POSITION pos = m_mapCtrlInfoTip.GetStartPosition(); pos != NULL;)
	{
		HWND hwndCtrl = NULL;
...
BOOL CBCGPDlgImpl::OnNeedTipText(UINT /*id*/, NMHDR* pNMH, LRESULT* /*pResult*/)
{
	...
	CString strDescription;
	if (GetControlInfoTipFromPoint(point, strTipText, strDescription) == NULL)
	{
		return FALSE;
	}
