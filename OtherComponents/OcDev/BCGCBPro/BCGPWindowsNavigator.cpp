...
BOOL CBCGPWindowsNavigator::OnInitDialog() 
{
	...
	for (POSITION pos = lstFrames.GetHeadPosition(); pos != NULL;)
	{
		...
		if (pMDIFrame->GetSafeHwnd() != NULL)
		{
			...
			CWnd* pMDINewActive = NULL;

			if (pMDIFrame->GetSafeHwnd() == pWndOwner->GetSafeHwnd())
			{
				CBCGPMainClientAreaWnd* pMainClientArea = DYNAMIC_DOWNCAST(CBCGPMainClientAreaWnd, CWnd::FromHandlePermanent(pMDIFrame->m_hWndMDIClient));
				if (pMainClientArea != NULL)
				{
					pMDINewActive = pMainClientArea->GetNextMDITab(pFrameImpl->m_bIsPrevNavigation);
				}
			}
			...
			while (hwndMDIChild != NULL)
			{
				...
				if (pMDIChildFrame != NULL && pMDIChildFrame->IsWindowVisible() && pMDIChildFrame->CanShowOnWindowsList())
				{
					...
					if (m_pSel == NULL && pMDIChildFrame->GetSafeHwnd() == pMDINewActive->GetSafeHwnd())
					{
						SetSel(pObj);
						m_bIsActivePane = FALSE;
					}
	...
	{
		SetSel(m_arMDIChildren.GetSize() == 1 ? m_arMDIChildren[0] : 
			(pFrameImpl->m_bIsPrevNavigation ? m_arMDIChildren[m_arMDIChildren.GetSize() - 2] : m_arMDIChildren[1]));
	}
