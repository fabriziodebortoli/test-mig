...
void CBCGPMessageBox::DrawHeader(CDC* pDC)
{
	...
	UINT nFormat = DT_NOPREFIX | DT_SINGLELINE | DT_NOCLIP | DT_END_ELLIPSIS;
...
void CBCGPMessageBox::DoInitDialog()
{
	...
	// Calculating gaps, spaces and margins
	{
		...
		m_rectClientMargins.top = ::MulDiv(m_rectClientMargins.top, szBaseUnits.cy, 8);
		m_rectClientMargins.right = ::MulDiv(m_rectClientMargins.right, szBaseUnits.cx, 4);
		m_rectClientMargins.bottom = ::MulDiv(m_rectClientMargins.bottom, szBaseUnits.cy, 8);
	...
	if (!m_strMessageHeader.IsEmpty())
	{
		...
		sizeHeader.cx = min(globalData.m_rectVirtual.Width() * 9 / 10, sizeHeader.cx);

		m_rectHeader = CRect(CPoint(0, 0), sizeHeader);
		nHeaderHeight = 4 * sizeHeader.cy / 3;
	}
	else
	{
