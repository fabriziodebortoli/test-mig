...
namespace WeifenLuo.WinFormsUI.Docking
{
	...
	public partial class DockPane : UserControl, IDockDragSource
	{
		...
		public bool HideDocumentTabs 
		{
			get { return this.DockPanel.HideDocumentTabs;  }
			set { this.DockPanel.HideDocumentTabs = value; this.TabStripControl.Visible = !value; }
		}
		...
		private List<IDockContent> activatedContentOrder = new List<IDockContent>();
		
		public IDockContent GetPreviousContent(IDockContent activeContent)
		{
			for (int i = activatedContentOrder.Count - 1; i >= 0; i--)
			{
				if (activatedContentOrder[i] == activeContent)
					return i == 0
						? activatedContentOrder[activatedContentOrder.Count - 1]
						: activatedContentOrder[i - 1];
			}
			return null;
		}
		public virtual IDockContent ActiveContent
		{
			...
			set
			{
				...
				//the active window is always at the end of the array, so navigating 
				//the array from the end I have the activation sequence
				for (int i = 0; i < activatedContentOrder.Count; i++)
					if (activatedContentOrder[i] == value)
					{
						activatedContentOrder.RemoveAt(i);
						break;
					}
				activatedContentOrder.Add(value);
		...
		internal void RemoveContent(IDockContent content)
		{
			...
			if (activatedContentOrder.Contains(content))
				activatedContentOrder.Remove(content);
		}
		...
		internal void ValidateActiveContent()
		{
			...
			IDockContent prevActivated = null;
			while (activatedContentOrder.Count != 0)
			{
				prevActivated = activatedContentOrder[activatedContentOrder.Count - 1];
				if (Contents.IndexOf(prevActivated) != -1 && prevActivated.DockHandler.DockState == DockState)
				{
					ActiveContent = prevActivated;
					return;
				}
				activatedContentOrder.RemoveAt(activatedContentOrder.Count - 1);
			}
