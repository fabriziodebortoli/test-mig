...
namespace WeifenLuo.WinFormsUI.Docking
{
	internal class VS2005AutoHideStrip : AutoHideStripBase
	{
		...
		private void DrawTab(Graphics g, TabVS2005 tab)
		{
			...
			//BEGIN PERASSO UPDATE:
			/*if (rectTabOrigin.IsEmpty)
				return;
			 */
			
			if (rectTabOrigin.Width == 0 || rectTabOrigin.Height == 0)
				return;
			//END PERASSO UPDATE:
			...
			{
				if (dockState == DockState.DockLeftAutoHide || dockState == DockState.DockRightAutoHide)
					g.DrawString(content.DockHandler.TabText, TextFont, brush, rectText, StringFormatTabVertical);
				else
					g.DrawString(content.DockHandler.TabText, TextFont, brush, rectText, StringFormatTabHorizontal);
			}
			// Set rotate back
