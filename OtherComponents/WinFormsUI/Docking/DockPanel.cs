...
namespace WeifenLuo.WinFormsUI.Docking
{
	...
	public partial class DockPanel : Panel
	{
		...
		private DockContent previousToActivate = null;

		private int dockLeftPanelMinSize = 24;
		private int dockTopPanelMinSize = 24;
		private int dockBottomPanelMinSize = 24;
		private int dockRightPanelMinSize = 24;

		public int DockLeftPanelMinSize { get { return dockLeftPanelMinSize; } set { dockLeftPanelMinSize = value; } }
		public int DockTopPanelMinSize { get { return dockTopPanelMinSize; } set { dockTopPanelMinSize = value; } }
		public int DockBottomPanelMinSize { get { return dockBottomPanelMinSize; } set { dockBottomPanelMinSize = value; } }
		public int DockRightPanelMinSize { get { return dockRightPanelMinSize; } set { dockRightPanelMinSize = value; } }
        ...
        [Browsable(false)]
        public bool IsLayoutSuspended { get { return m_focusManager.IsFocusTrackingSuspended; } }
		...
		private bool m_showCloseButtonOnTab= false;
		[DefaultValue(false)]
		[LocalizedCategory("Category_Docking")]
		[LocalizedDescription("DockPanel_ShowCloseButtonOnTab_Description")]
		public bool ShowCloseButtonOnTab
		{
			get { return m_showCloseButtonOnTab; }
			set
			{
				if (m_showCloseButtonOnTab == value)
					return;

				m_showCloseButtonOnTab = value;
				Refresh();
			}
		}

		private bool m_FitTabs = false;
		[DefaultValue(false)]
		[LocalizedCategory("Category_Docking")]
		[LocalizedDescription("DockPanel_FitTabs_Description")]
		public bool FitTabs
		{
			get { return m_FitTabs; }
			set
			{
				if (m_FitTabs == value)
					return;
		...
		private bool m_OnlyTabsInDropdownMenu = true;
		[DefaultValue(false)]
		[LocalizedCategory("Category_Docking")]
		[LocalizedDescription("DockPanel_OnlyTabsInDropdownMenu_Description")]
		public bool OnlyTabsInDropdownMenu
		{
			get { return m_OnlyTabsInDropdownMenu; }
			set
			{
				if (m_OnlyTabsInDropdownMenu == value)
					return;

				m_OnlyTabsInDropdownMenu = value;
				Refresh();
			}
		}
        ...
        private int GetDockWindowSize(DockState dockState)
        {
            if (dockState == DockState.DockLeft || dockState == DockState.DockRight)
            {
				...
				if (dockLeftSize < dockLeftPanelMinSize)
					dockLeftSize = dockLeftPanelMinSize;

                //if (dockRightSize < MeasurePane.MinSize)
                //   dockRightSize = MeasurePane.MinSize;
				if (dockRightSize < dockRightPanelMinSize)
					dockRightSize = dockRightPanelMinSize;
            ...
            else if (dockState == DockState.DockTop || dockState == DockState.DockBottom)
            {
				...
				if (dockTopSize < dockTopPanelMinSize)
					dockTopSize = dockTopPanelMinSize;

				//if (dockBottomSize < MeasurePane.MinSize)
				//   dockBottomSize = MeasurePane.MinSize;
				if (dockBottomSize < dockBottomPanelMinSize)
					dockBottomSize = dockBottomPanelMinSize;
		...
		{
			if (ActiveDocumentPane == null || ActiveDocument == null)
				return;

			if (previousToActivate == null ||
				previousToActivate.IsDisposed || 
				previousToActivate.DockState != DockState.Document)
			{
				previousToActivate = ActiveDocumentPane.GetPreviousContent(ActiveDocument) as DockContent;
			}
			if (previousToActivate == null )
				return;

			DockContent tmp = previousToActivate;
			previousToActivate = ActiveDocumentPane.GetPreviousContent(previousToActivate) as DockContent;
			tmp.Show(this);
		}
