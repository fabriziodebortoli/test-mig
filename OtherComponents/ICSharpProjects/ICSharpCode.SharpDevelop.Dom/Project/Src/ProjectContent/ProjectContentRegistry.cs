...
namespace ICSharpCode.SharpDevelop.Dom
{
	...
	public class ProjectContentRegistry : IDisposable
	{
		...
		public virtual IProjectContent GetProjectContentForReference(string itemInclude, string itemFileName, Assembly asm)
		{
			lock (contents) {
				...
				try {
					pc = LoadProjectContent(itemInclude, itemFileName, asm);
		...
		protected virtual IProjectContent LoadProjectContent(string itemInclude, string itemFileName, Assembly asm)
		{
			string shortName = itemInclude;
			int pos = shortName.IndexOf(',');
			if (pos > 0)
