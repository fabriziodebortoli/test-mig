...
namespace Microarea.Common.ExpressionManager
{
	...
	public class TbScript
	{
		...
		//--------------------------------------------------------------------------------
		public Microarea.Common.CoreTypes.FunctionsList FunctionsTable { get { return scriptingSession.Functions; } }
		//--------------------------------------------------------------------------------
	...
	internal class DynamicVariable : Variable
	{
		...
		public override object Data
		{
			get
			{
				...
				throw new Exception(string.Format(ExpressionManagerStrings.InvalidProperty, objectVariable.Data.GetType().FullName, propName));
			}
			set
			{
				//prima verifico se l'oggetto ha una proprietà con quel nome
		...
		public override bool Equals(object item)
		{
			...
			return var.objectVariable.Equals(objectVariable) &&
				string.Compare(propName, var.propName, StringComparison.OrdinalIgnoreCase) == 0;
		}
