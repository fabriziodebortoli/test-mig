...
namespace Microarea.Common.NameSolver
{
    ...
    public class BrandLoader : IBrandLoader
    {
			...
			try
			{
				...
				foreach (XmlNode aBrandNode in brandsNodes)
				{
                   ...
                   // IBrandInfo brandInfo = GetBrandInfo(brandName);
                    ...
                    //if (brandInfo == null)
                   // {
                        BrandInfo brandInfo = new BrandInfo(aBrandNode, isMainBrand);
		...
		//-----------------------------------------------------------------------------
		public string GetBrandedStringBySourceString(string source)
		{
			...
			{
				if (String.Compare(bki.Source, source, StringComparison.OrdinalIgnoreCase) == 0)
					return bki.Branded;
			}
			return source;
		}
        ...
        //-----------------------------------------------------------------------------
        public IImage GetMenuManagerSplash()       // Image al posto di IImage           TODO rsweb
        {
        ...
        //-----------------------------------------------------------------------------
        public IImage GetConsoleSplash()                              // Image al posto di IImage       TODO rsweb
        {
        ...
        //------------------------------------------------------------------------------
        public IImage GetCompanyLogo()                    // Image al posto di IImage          TODO rsweb
        {
        ...
        //------------------------------------------------------------------------------
        private IImage GetImageFromNamespace(string ns)              // Image al posto di IImage        TODO rsweb
        {
            string image;
            try
            {
    ...
    public class BrandInfo : IBrandInfo
    {
		...
		public IApplicationBrandInfo GetApplicationBrandInfo(string aApplicationName)
		{
			...
			{
				if (String.Compare(info.Name, aApplicationName, StringComparison.OrdinalIgnoreCase) == 0)
				    return info;
			}
			return null;
		}
        ...
        //todo ilaria da rivedere
		//-----------------------------------------------------------------------------
		public bool IsMagoNet()
		{
            return (String.Compare(ProductTitle, "Mago.Net", StringComparison.OrdinalIgnoreCase) == 0 ||
                 String.Compare(ProductTitle, "Mago4", StringComparison.OrdinalIgnoreCase) == 0 ||
                String.Compare(ProductTitle, "M4go", StringComparison.OrdinalIgnoreCase) == 0);
		}
