...
namespace Microarea.ProvisioningDatabase.Libraries.DatabaseManager
{
	...
	public class CheckDBStructureInfo
	{
		...
		public bool GetApplicationModuleInfo(string tableName, out ModuleDBInfo modDBInfo)
		{
			...
			foreach (AddOnApplicationDBInfo addOnAppDBInfo in appStructInfo.ApplicationDBInfoList)
			{
				...
				{					
					...
					{
						if (string.Compare(entryDBInfo.Name, tableName, StringComparison.OrdinalIgnoreCase) == 0)
						{
		...
		public void GetDatabaseStatus()
		{
			...
			dbMarkInfo = new DBMarkInfo(this.currentConnection);
			...
			if (dbStatus == DatabaseStatus.NOT_EMPTY)
			{
				...
				dbMarkInfo.DBMarkTable.Load();

				if (kindOfDb == KindOfDatabase.Company)
				{
					// controllo per capire se il db di partenza e' di una 3.x
					int dbRelCore = dbMarkInfo.DBMarkTable.GetDBReleaseFromDBMark(DatabaseLayerConsts.ERPSignature, DatabaseLayerConsts.ERPCoreModuleName, true);
					if (dbRelCore < 400)
					{
        ...
        public void CheckMissingTBGuidColumn()
        {
            ...
            foreach (string tblName in registeredTables)
            {
                ...
                {
                    EntryDBInfo entryDBInfo = moduleDBInfo.TablesList.Find(a => string.Compare(a.Name, tblName, StringComparison.OrdinalIgnoreCase) == 0);
		...
		private bool CheckModulesWithPreviousSignature()
		{
			...
			foreach (AddOnApplicationDBInfo app in appStructInfo.ApplicationDBInfoList)
			{
				foreach (ModuleDBInfo module in app.ModuleList)
				{
					...
					PreviousSignatureInfo psi = dbMarkInfo.DBMarkTable.CreatePreviousSignatureInfo(module);
					if (psi != null)
					{
		...
		private bool CheckDBRecoveryStatus()
		{
			...
			DBMarkTable dtRecoveryModules = dbMarkInfo.DBMarkTable.GetRecoveryRows();

			for (int i = dtRecoveryModules.Rows.Count - 1; i >= 0; i--)
			{
				DBMarkRow row = dtRecoveryModules.Rows[i];

				ModuleDBInfo info = GetModuleItemFromSignature(row.Application, row.AddOnModule);
				if (info != null)
				{
		...
		private bool CheckDBUpgradeStatus()
		{
			...
			foreach (AddOnApplicationDBInfo addOnAppDBInfo in appStructInfo.ApplicationDBInfoList)
			{
				foreach (ModuleDBInfo moduleDBInfo in addOnAppDBInfo.ModuleList)
				{
					...
					numRelMark = dbMarkInfo.DBMarkTable.GetDBReleaseFromDBMark(moduleDBInfo.ApplicationSign, moduleDBInfo.ModuleSign);
		...
		private bool FindModuleMissingObjects(ModuleDBInfo moduleDBInfo)
		{
			...
			{
				if (string.Compare(moduleDBInfo.ModuleName, info.ModuleName, StringComparison.OrdinalIgnoreCase) == 0)
					return false;
			}
		...
		private ModuleDBInfo GetModuleItemFromSignature(string appName, string moduleName)
		{
			foreach (AddOnApplicationDBInfo addOnAppDBInfo in appStructInfo.ApplicationDBInfoList)
			{
				foreach (ModuleDBInfo moduleDBInfo in addOnAppDBInfo.ModuleList)
				{
					if (string.Compare(moduleDBInfo.ApplicationSign, appName, StringComparison.OrdinalIgnoreCase) == 0 &&
						string.Compare(moduleDBInfo.ModuleSign, moduleName, StringComparison.OrdinalIgnoreCase) == 0)
						return moduleDBInfo;
				}
			}
			return null;
