...
namespace Microarea.AdminServer.Libraries.DatabaseManager
{
	...
	public class ApplicationDBStructureInfo
	{
		...
		public void InitApplicationList(StringCollection appList)
		{
			...
			foreach (string appName in appList)
			{
				...
				foreach (Microarea.Common.NameSolver.ModuleInfo modInfo in appInfo.Modules)
				{
					...
					if (!PathFinder.PathFinderInstance.FileSystemManager.ExistFile(modInfo.GetDatabaseObjectsPath()))
						continue;
		...
		public void LoadTablesInfo(string appName, ModuleDBInfo modDBInfo)
		{
            ...
            Microarea.Common.NameSolver.ModuleInfo modInfo = pathFinder.GetModuleInfoByName(appName, modDBInfo.ModuleName);
		...
		public void ReadRowSecurityObjects(StringCollection appList)
		{
			foreach (string appName in appList)
			{
				...
				foreach (Microarea.Common.NameSolver.ModuleInfo modInfo in appInfo.Modules)
				{
					if (!PathFinder.PathFinderInstance.FileSystemManager.ExistFile(modInfo.GetRowSecurityObjectsPath()))
						continue;
