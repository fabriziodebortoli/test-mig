...
namespace Microarea.AdminServer.Libraries
{
    internal class AuthenticationProcess
    {
        ...
        {
            SlotType calType = GetCalType(processName);

            if (calType == SlotType.Invalid)
                return (int)LoginReturnCodes.InvalidProcessError;

            if (calType == SlotType.Gdi)//con gdi devi essere gdi
                return (gdiLogin) ? (int)LoginReturnCodes.NoError : (int)LoginReturnCodes.GDIApplicationAccessDenied;

            else if (calType == SlotType.Mobile)
                return (!webLogin && !gdiLogin) ? (int)LoginReturnCodes.NoError : (int)LoginReturnCodes.InvalidProcessError;

            else if (!((calType == SlotType.EasyLook && (webLogin || gdiLogin)) ||//easylook può essere gdi o web
                 (calType == SlotType.ThirdPart && gdiLogin) ||//solo gdi
                 (calType == SlotType.MagicDocument && gdiLogin))) //solo gdi
        ...
        //-----------------------------------------------------------------------
        internal static SlotType GetCalType(string processName)
        {
            if (processName == null || processName.Length == 0)
                return SlotType.Invalid;

            if (
                string.Compare(processName, ProcessType.MenuManager, StringComparison.OrdinalIgnoreCase) == 0 ||
                string.Compare(processName, ProcessType.MicroareaConsole, StringComparison.OrdinalIgnoreCase) == 0 ||
                string.Compare(processName, ProcessType.SchedulerAgent, StringComparison.OrdinalIgnoreCase) == 0 ||
                string.Compare(processName, ProcessType.SchedulerManager, StringComparison.OrdinalIgnoreCase) == 0 ||
                string.Compare(processName, ProcessType.WMS, StringComparison.OrdinalIgnoreCase) == 0
                )
                return SlotType.Gdi;

            if (
                string.Compare(processName, ProcessType.InvisibleWMS, StringComparison.OrdinalIgnoreCase) == 0 ||
                string.Compare(processName, ProcessType.InvisibleMAN, StringComparison.OrdinalIgnoreCase) == 0 ||
                string.Compare(processName, ProcessType.InvisibleWARMAN, StringComparison.OrdinalIgnoreCase) == 0
                )
                return SlotType.Mobile;

            if (string.Compare(processName, ProcessType.EasyLook, StringComparison.OrdinalIgnoreCase) == 0)
                return SlotType.EasyLook;

            if (string.Compare(processName, ProcessType.MagicDocuments, StringComparison.OrdinalIgnoreCase) == 0)
                return SlotType.MagicDocument;

            return SlotType.ThirdPart;
        }
    }
    [Flags]
    ...
    internal abstract class AuthenticationSlots
    {
        ...
        protected ActivationToken activationManager = null;
        ...
        //-----------------------------------------------------------------------
        public static AuthenticationSlots Create(ActivationToken activationManager)
        {
            if (activationManager.IsEnterprise())
        ...
        //-----------------------------------------------------------------------
        public int Init(ActivationToken activationManager, SqlConnection sysDBConnection/*, IDiagnostic diagnostic*/)
        {
            ...
            for (int i = 0; i < gdiCal; i++)
                GDISlots[i] = new AuthenticationSlot(SlotType.Gdi, usedCal.Clone() as Hashtable);

            GDIConcurrentSlots = new ConcurrentSlot[gdiConcurrent];
            for (int i = 0; i < gdiConcurrent; i++)
                GDIConcurrentSlots[i] = new ConcurrentSlot(SlotType.Gdi, usedCal.Clone() as Hashtable);

            int totmobile = wmsCal + manufacturingCal;
            WMSSlots = new ConcurrentSlot[totmobile];
            for (int i = 0; i < totmobile; i++)
                WMSSlots[i] = new ConcurrentSlot(SlotType.Gdi, mobileUsedCal.Clone() as Hashtable);
            ...
            ManufacturingSlots = new ConcurrentSlot[0];//todo lascio vuoto a zero se no crea slot nulli---esiste un solo slot per le mobile, questo non usato
                                                       ...
                                                       //    ManufacturingSlots[i] = new ConcurrentSlot(SlotType.Gdi, mobileUsedCal.Clone() as Hashtable);
        ...
        //-----------------------------------------------------------------------
        protected virtual int AssignUserToLicenzaUtente(int loginID, ActivationToken activationManager, AuthenticationSlot slot, SlotType calType)
        {
            return (int)LoginReturnCodes.NoError;
        }
        ...
        public int Login(int loginID, string loginName, string macIp, int companyID, string companyName, string processName, bool webAllowed, bool gdiAllowed, bool concurrent, bool overwriteLogin, out string authenticationToken)
        {
            ...
            SlotType calType = AuthenticationProcess.GetCalType(processName);
            if (calType == SlotType.Invalid)
                return (int)LoginReturnCodes.InvalidProcessError;
            ...
            if ((string.Compare(processName, ProcessType.SchedulerAgent, StringComparison.OrdinalIgnoreCase) == 0 ||
                 string.Compare(processName, ProcessType.SchedulerManager, StringComparison.OrdinalIgnoreCase) == 0))
            {
                AuthenticationSlot slot = new AuthenticationSlot(SlotType.Gdi, null);
            ...
            if ((string.Compare(processName, ProcessType.InvisibleMAN, StringComparison.OrdinalIgnoreCase) == 0 ||
                 string.Compare(processName, ProcessType.InvisibleWMS, StringComparison.OrdinalIgnoreCase) == 0 ||
                 string.Compare(processName, ProcessType.InvisibleWARMAN, StringComparison.OrdinalIgnoreCase) == 0))
            {
                foreach (AuthenticationSlot s in skedulerSlots)
                {
                    if (s != null && String.Compare(s.ProcessName, processName, StringComparison.OrdinalIgnoreCase) == 0)
                    {
            ...
            if (calType == SlotType.ThirdPart ||
                calType == SlotType.MagicDocument ||
                (calType == SlotType.EasyLook && webAllowed && !gdiAllowed && !concurrent))
                return LoginSpecificSlot(calType, loginID, loginName, companyID, companyName, processName, out authenticationToken);

            //WMS + manufacturing
            if (string.Compare(processName, ProcessType.WMS, StringComparison.OrdinalIgnoreCase) == 0)
            {
        ...
        //-----------------------------------------------------------------------
        internal AuthenticationSlot FreePendingWMSSlot()
        {//liberolo slot se la cal wms non è piu stata rinfrescata, devo però verificare quella non connessa da più tempo.
        ...
        //-----------------------------------------------------------------------
        protected int LoginWMSSlot(SlotType slotType, int loginID, string loginName, int companyID, string companyName, string processName, string macip, out string authenticationToken)
        {
            ...
            if (!string.IsNullOrWhiteSpace(macip))
            {
                for (int i = 0; i < WMSSlots.Length; i++)
                    if (!WMSSlots[i].Reserved && String.Compare(WMSSlots[i].MACAddress, macip, StringComparison.OrdinalIgnoreCase) == 0)
                    {
                        if (WMSSlots[i].Logged)
                        {
                            ...
                            Logout(WMSSlots[i].GetTokenByApplication(SlotType.Gdi));
            ...
            return LoginWMSSlot(FreePendingWMSSlot(), loginID, loginName, companyID, companyName, processName, macip, authenticationToken);
        ...
        private int LoginWMSSlot(AuthenticationSlot slot, int loginID, string loginName, int companyID, string companyName, string processName, string macip, string authenticationToken)
        {
            ...
            ConsumeMobileCal(slot, authenticationToken);
            return (int)LoginReturnCodes.NoError;

        }
        //----------------------------------------------------------------------
        private MobileCal ConsumeMobileCal(AuthenticationSlot slot, string authenticationToken)
        {
            ...
            try
            {
                ...
                if (wms != null)
                {
                    int reswms = AssignUserToMobileArticle(slot.LoginID, authenticationToken, wms, MobileCal.WMS);
                ...
                if (manuf != null)
                {
                    int resmanuf = AssignUserToMobileArticle(slot.LoginID, authenticationToken, manuf, MobileCal.Manufacturing);
        ...
        //-----------------------------------------------------------------------
        protected int LoginSpecificSlot(SlotType slotType, int loginID, string loginName, int companyID, string companyName, string processName, out string authenticationToken)
        {
        ...
        //-----------------------------------------------------------------------
        protected abstract int LoginSlot(SlotType calType, int loginID, string macIp, string loginName, int companyID, string companyName, string processName, bool overwriteLogin, out string authenticationToken);
        ...
        //-----------------------------------------------------------------------
        //a partre dall'xml èseguo un reload delle informazioni in modo da ricaricare la situazione .
        public bool Reload()
        {
            return true;
        }
        ...
        public string ToXml(bool writeTokens)
        {
            ...
            foreach (AuthenticationSlot slot in allSlots)
            {
                ...
                switch (slot.SlotType)
                {
                    ...
                    case SlotType.ThirdPart:
                        parent = tpNode;
                        break;
                    case SlotType.MagicDocument:
                        parent = magicNode;
                        break;
        ...
        /// <returns></returns>
        //-----------------------------------------------------------------------
        protected AuthenticationSlot GetFirstAvailableSlot(SlotType slotType)
        {
            if (slotType == SlotType.Gdi)
            {
                for (int i = 0; i < GDISlots.Length; i++)
            ...
            if (slotType == SlotType.EasyLook)
            {
                AuthenticationSlot elSlot = new AuthenticationSlot(SlotType.EasyLook, null);
                if (ELSlots.Add(elSlot))
                    return elSlot;
                return null;
            }

            if (slotType == SlotType.ThirdPart)
            {
                AuthenticationSlot mlSlot = new AuthenticationSlot(SlotType.ThirdPart, null);
                if (MLSlots.Add(mlSlot))
                    return mlSlot;
                return null;
            }

            if (slotType == SlotType.MagicDocument)
            {
                AuthenticationSlot mdSlot = new AuthenticationSlot(SlotType.MagicDocument, null);
        ...
        {
            foreach (AuthenticationSlot slot in GDISlots)
                if (String.Compare(slot.ssologinDataBag.SSOID, ssoid, StringComparison.OrdinalIgnoreCase) == 0)
                    return slot;
        ...
        internal int GetTokenProcessType(string token)
        {
            ...
            SlotType slotType = AuthenticationSlot.GetSlotTypeFromApplicationName(app);
            return (int)slotType;
        }
        ...
        //----------------------------------------------------------------------
        public abstract int AssignUserToMobileArticle(int loginID, string token, ArticleInfo article, MobileCal mc);
        ...
        internal int Login()
        {
            ...
            //Per ora non so ancora i parametri
        }
    }
    ...
    internal class GeneralSlots : AuthenticationSlots
    {
        ...
        /// </remarks>
        //-----------------------------------------------------------------------
        protected override int LoginSlot(SlotType calType, int loginID, string macIp, string loginName, int companyID, string companyName, string processName, bool overwriteLogin, out string authenticationToken)
        {
            authenticationToken = null;
            ...
            if (slot != null)//gdi riservata
            {
                ...
                else//lo slot è già occupato
                {
                    ...
                    if (slot.CompanyID == -1 || slot.CompanyID == companyID)
                    {
                        ...
                        {
                            if (calType == SlotType.EasyLook && slot.CalType == SlotType.EasyLook)
            ...
            //else // gdi non riservata o riservata occupata da altra company
            {
                ...
                if (slot != null)//esiste un gdi già occupato dallo stesso ut-comp
                {
                    ...
                    else if (calType == SlotType.EasyLook && slot.CalType == SlotType.EasyLook)//login el su el
                                                                                               //se è easylook e la login era easylook torno lo stesso token
                    {
                        authenticationToken = slot.GetTokenByApplication(calType);
                        if (string.IsNullOrEmpty(authenticationToken))
                    ...
                    else if (calType == SlotType.Gdi) //gdi su el 
                    {
                        authenticationToken = Guid.NewGuid().ToString();
                ...
                slot = GetFirstAvailableSlot(SlotType.Gdi);
                if (slot != null)
                {
                    if (calType == SlotType.Gdi)//riservo solo se login gdi
                    {
                ...
                {
                    if (calType == SlotType.EasyLook)
                        return LoginSpecificSlot(SlotType.EasyLook, loginID, loginName, companyID, companyName, processName, out authenticationToken);
                    return (int)LoginReturnCodes.NoCalAvailableError;
                }
            }
        }
        ...
        //-----------------------------------------------------------------------
        protected override int AssignUserToLicenzaUtente(int loginID, ActivationToken activationManager, AuthenticationSlot slot, SlotType calType)
        {
            ArticleInfo articleInfo = activationManager.GetUserLicenceArticle();

            int autaRes = 0;
            if (calType == SlotType.Gdi)
            {
        ...
        {
            AuthenticationSlot slot = GetFirstAvailableSlot(SlotType.Gdi);
            if (slot == null)
                return false;
        ...
        public List<int> GetUserForArticle(string articleName)
        {
            ...
            try
            {
                ...
                using (SqlDataReader dataReader = aSqlCommand.ExecuteReader())
                {

                    while (aSqlDataReader.Read())
                    {
                        int loginID = (int)aSqlDataReader["LoginID"];
                        list.Add(loginID);
                    }
                }
            }
        ...
        //----------------------------------------------------------------------
        public override int AssignUserToMobileArticle(int loginID, string token, ArticleInfo article, MobileCal mc)
        {
        ...
        private int AssignUserToArticle(int loginID, string token, string name, object namedCalNumber)
        {
            throw new NotImplementedException();
        }
        ...
        private int AssignUserToArticle(int loginID, string token, string articleName, int calNumber)
        {
            ...
            {
                if (String.Compare(v.ArtName, artname, StringComparison.OrdinalIgnoreCase) == 0)
                    a++;
            }
            int cal = 3;
    ...
    internal class FunctionalSlots : AuthenticationSlots
    {
        ...
        private bool EvaluateFloatingMark()
        {
            ...
            try
            {
                ...
                sqlCommand.Parameters["@ArticolName"].Value = FloatingMarkString;
                sqlCommand.Parameters["@loginID"].Value = FloatingMarkNumber;
    ...
    internal class AuthenticationSlot
    {
        ...
        private readonly SlotType slotType = SlotType.Invalid;
        ...
        {
            get
            { return TimeSpan.TicksPerMinute * 6/* InstallationData.ServerConnectionInfo.WMSCalPurgeMinutes*/; }
        }
        ...
        public bool UnNamed
        {
            get
            {
                return (SlotType == SlotType.EasyLook) || (SlotType == SlotType.MagicDocument);
            }
        }

        //-----------------------------------------------------------------------
        public AuthenticationSlot(SlotType slotType, Hashtable usedCALForArticle)
        {
        ...
        //-----------------------------------------------------------------------
        public static string GetApplicationNameFromSlotType(SlotType slotType)
        {
            switch (slotType)
            {
                case SlotType.EasyLook:
        ...
        //-----------------------------------------------------------------------
        public static SlotType GetSlotTypeFromApplicationName(string applicationName)
        {
        ...
        public bool ContainsToken(string token)
        {
            ...
            {
                if (string.Compare(appToken, token, StringComparison.OrdinalIgnoreCase) == 0)
                    return true;
            }
        ...
        public string GetApplicationByToken(string token)
        {
            ...
            {
                if (string.Compare((string)de.Value, token, StringComparison.OrdinalIgnoreCase) == 0)
        ...
        //-----------------------------------------------------------------------
        public string GetTokenByApplication(SlotType calType)
        {
        ...
        public string GetTokenByApplication(string application)
        {
            foreach (DictionaryEntry de in ApplicationTokens)
            {
                if (string.Compare((string)de.Key, application, StringComparison.OrdinalIgnoreCase) == 0)
        ...
        {
            return ((string.Compare(processName, ProcessType.InvisibleMAN, StringComparison.OrdinalIgnoreCase) == 0 ||
                 string.Compare(processName, ProcessType.InvisibleWMS, StringComparison.OrdinalIgnoreCase) == 0 ||
                 string.Compare(processName, ProcessType.InvisibleWARMAN, StringComparison.OrdinalIgnoreCase) == 0));
        }
    ...
    {
        //-----------------------------------------------------------------------
        public ConcurrentSlot(SlotType slotType, Hashtable usedCALForArticle)
    ...
    internal class VerticalFloatingCalTableItem : IComparable, IComparable<VerticalFloatingCalTableItem>
    {
        ...
        public override bool Equals(object obj)
        {
            ...
            if (Object.ReferenceEquals(tempDataStr, null))
                return false;
            return (
                (String.Compare(tempDataStr.Token, Token, StringComparison.OrdinalIgnoreCase) == 0) &&
                String.Compare(tempDataStr.ArtName, ArtName, StringComparison.OrdinalIgnoreCase) == 0);
        }
        ...
        {
            bool ok = (
                (String.Compare(other.Token, Token, StringComparison.OrdinalIgnoreCase) == 0) &&
                String.Compare(other.ArtName, ArtName, StringComparison.OrdinalIgnoreCase) == 0);
            if (ok)
                return 0;
            return -1;
        }
    ...
    //----------------------------------------------------------------------
    internal class SSOLoginDataBag//bhoooo
    {
