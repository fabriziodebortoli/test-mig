...
namespace Microarea.TaskBuilderNet.Woorm.WoormEngine
{
	...
	public class FromTablesList : ArrayList
	{
		...
		public TableNames GetTableNames(string tableName)
		{
			foreach (TableNames tn in this)
				if (string.Compare(tableName, tn.TableName, true, CultureInfo.InvariantCulture) == 0)
					return tn;
		...
		{
			foreach (TableNames tn in this)
				if (string.Compare(tableAlias, tn.AliasName, true, CultureInfo.InvariantCulture) == 0)	
					return tn.TableName;
	...
	public class DataTableRule : RuleObj, IDisposable
	{
		...
		protected void Init()
		{
			whereClause = new IfWhereClause(Session, Engine.RepSymTable.Fields, this, WhereClause.EClauseType.Where);
			havingClause = new IfWhereClause(Session, Engine.RepSymTable.Fields, this, WhereClause.EClauseType.Having);
		}
		...
		public bool IsQualifiedName(string columnName)
		{
			...
			foreach (TableNames info in FromTables)
				if	(string.Compare(info.TableName, tableName, true, CultureInfo.InvariantCulture) == 0)
					return true;
		...
		bool ExpandContentOfClause (ref string  strSql)
		{
			...
			string f = Token.CONTENTOF.ToString().ToLower(CultureInfo.InvariantCulture);

            while (true)
			{
				string s = strSql.ToLower(CultureInfo.InvariantCulture);
		...
		CultureInfo GetCollateCulture(FromTablesList fromTables, Field f)
		{
			...
			try
			{
				if (tbConnection == null)
				{
					...
					tbConnection = new TBConnection(Engine.Report.ReportSession.CompanyDbConnection, TBDatabaseType.GetDBMSType(Engine.Report.ReportSession.Provider));
					tbConnection.Open();
				}
				string collation = Microarea.TaskBuilderNet.Data.DatabaseLayer.TBCheckDatabase.GetColumnCollation(tbConnection, table, column);
				return (collation.Length == 0 || collation == NameSolverDatabaseStrings.SQLLatinCollation)
		...
		public bool Parse(Parser lex)
		{
			...
			*/
			...
			do
			{
                ...
                {
                    //TODO join
                    IfWhereClause onClause = new IfWhereClause(Engine.Report.ReportSession, Engine.RepSymTable.Fields, this, WhereClause.EClauseType.JoinOn);
			...
			*/
			...
			if 	(lex.Parsed(Token.GROUP))
			{
                ...
                {
                    string s = groupBy.ToLower(CultureInfo.InvariantCulture);
                    string f = Token.CONTENTOF.ToString().ToLower(CultureInfo.InvariantCulture);
                    if (s.IndexOf(f) >= 0)
                    {
                        groupByDynamic = groupBy;
			...
			if 	(lex.Parsed(Token.ORDER))
			{
                ...
                {
                    string s = orderBy.ToLower(CultureInfo.InvariantCulture);
                    string f = Token.CONTENTOF.ToString().ToLower(CultureInfo.InvariantCulture);
                    if (s.IndexOf(f) >= 0)
                    {
                        orderByDynamic = orderBy;
		...
		private bool ExecuteCommand()
		{
			...
			try
			{
				...
				tbCommand = new TBCommand(select, tbConnection);
