...
namespace Microarea.TaskBuilderNet.Woorm.WoormEngine
{
	...
	public class Field : Variable, IDisposable
	{
		...
		//----------------------------------------------------------------------------
		public bool IsArray 
		{ get { return string.Compare(Language.GetTokenString(Token.ARRAY), WoormType, true, CultureInfo.InvariantCulture) == 0; } } 
			...
			switch (lex.LookAhead())
			{
				...
				case Token.COLTOTAL:
					...
					else
					{
						...
						if (name.Length != 0 && string.Compare(field.PublicName, name, true, CultureInfo.InvariantCulture) == 0)
						{
