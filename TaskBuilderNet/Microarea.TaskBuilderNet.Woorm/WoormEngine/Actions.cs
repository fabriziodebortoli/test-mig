...
namespace Microarea.TaskBuilderNet.Woorm.WoormEngine
{
	...
	public class WoormEngineExpression : Expression
	{
        ...
        override public Value ApplySpecializedFunction(FunctionItem function, Stack paramStack)
		{
			...
			else if (string.Compare(function.Name, "Framework.TbWoormViewer.TbWoormViewer.CloseConnection", true) == 0)
			{
				...
				{
					//bag.remove(o)
					TBConnection conn = (TBConnection)o;
					conn.Close();
					conn.Dispose();
				}
	...
	public class MessageBoxAction : ActionObj
	{
		...
		public bool Parse(Parser lex)
		{
            ...
            message.StopTokens = new StopTokens(new Token[] { Token.SEP, Token.COMMA});
            bool ok = message.Compile(lex, CheckResultType.Match, "String");
            if (lex.Parsed(Token.COMMA))
            {
                ok = lex.ParseString(out iconName);
            }
