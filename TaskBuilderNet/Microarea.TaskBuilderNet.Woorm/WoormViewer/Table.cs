...
namespace RSjson
{
	...
	public class Cell : ISerializable
	{
		...
		public Color	DefaultBkgColor			{ get { return column.Table.Transparent ? Color.Transparent : Value.BkgColor; } }
	...
	public class Column : ISerializable
	{
		...
		public Color TitleBkgColor
		{
			get
			{
				...
				return Table.Transparent ? Color.Transparent : Title.BkgColor;
			}
		}
		...
		internal void SetStyle(Column templateCol)
		{
			...
			if (Cells.Count > 0 && templateCol.Cells.Count > 0)
			{
				...
				if (Cells[0].DefaultBkgColor == Color.White)
					SetCellsBkgColor(templateCol.Cells[0].DefaultBkgColor);

				if (Cells[0].DefaultTextColor == Color.Black)
		...
		internal int CalculateFieldWidth(string strText)
		{
			...
			int nWidth = this.ColumnRect.Width;
		...
		public bool GetFieldWidthFactors(ref FieldWidthFactors fwf, bool isSubTotal = false)
		{
			...
			FontElement fe = this.Table.Document.GetFontElement(fontStyleName);
			fwf.font = new Font(fe.FaceName, (fe.Size * (float)72 / 100));
			...
			int width = this.ColumnRect.Width;
			...
			fwf.m_nWidth = Math.Max(1, (width - 2 * 2/*MARGIN_FROM_BORDER*/));
			return true;
		}
	...
	public class Table : BaseObj
	{
		...
		private bool ParseSubTotalCells(WoormParser lex)
		{
			...
			allColors[(int)ElementColor.VALUE] = Color.Black;
			allColors[(int)ElementColor.BACKGROUND] = Color.White;
		...
		private bool ParseColumnTitleCells(WoormParser lex)
		{
			Color[] allColors = new Color[(int)ElementColor.MAX];
			allColors[(int)ElementColor.VALUE] = Color.Black;
			allColors[(int)ElementColor.BACKGROUND] = Color.DarkGray;
		...
		**/
		...
		private void UnparseSingleTotal(CellTotalDiff aTotalDiff, Unparser unparser)
		{
			...
			bool brd = (colors[(int)ElementColor.BORDER] != Color.Black);
		...
		private void UnparseSingleColumnTitle(CellColumnTitleDiff aColumnTitle, Unparser unparser)
		{
			...
			bool brd = (rectColors[(int)ElementColor.BORDER] != Color.Black);
		...
		{
			if (aColumnPen.Color != Color.Black)
