...
namespace Microarea.TaskBuilderNet.Core.MenuManagerLoader
{
	...
	public class MenuXmlNode : IMenuXmlNode
	{
			...
			set
			{
				...
				if (node != null)
				{
                    ...
                    if (IsShortcut)
                    {
                        ...
                        IsStartupShortcut = sStartup.Equals("true", StringComparison.InvariantCultureIgnoreCase);
                    }
				}
			}
		}
		...
		public MenuXmlNodeCollection GetAllCommands(string aCommandItemObject, MenuLoader.CommandsTypeToLoad commandsTypeToSearch)
		{
			...
			try
			{
				...
				if ((commandsTypeToSearch & MenuLoader.CommandsTypeToLoad.Form) == MenuLoader.CommandsTypeToLoad.Form)
					commandTypesExpression = "self::" + MenuXmlNode.XML_TAG_DOCUMENT;
				if ((commandsTypeToSearch & MenuLoader.CommandsTypeToLoad.Batch) == MenuLoader.CommandsTypeToLoad.Batch)
				{
					if (commandTypesExpression.Length > 0)
						commandTypesExpression += " or ";
					commandTypesExpression += "self::" + MenuXmlNode.XML_TAG_BATCH;
				}
				if ((commandsTypeToSearch & MenuLoader.CommandsTypeToLoad.Report) == MenuLoader.CommandsTypeToLoad.Report)
				{
					if (commandTypesExpression.Length > 0)
						commandTypesExpression += " or ";
					commandTypesExpression += "self::" + MenuXmlNode.XML_TAG_REPORT;
				}
				if ((commandsTypeToSearch & MenuLoader.CommandsTypeToLoad.Function) == MenuLoader.CommandsTypeToLoad.Function)
				{
					if (commandTypesExpression.Length > 0)
						commandTypesExpression += " or ";
					commandTypesExpression += "self::" + MenuXmlNode.XML_TAG_FUNCTION;
				}
				if ((commandsTypeToSearch & MenuLoader.CommandsTypeToLoad.Text) == MenuLoader.CommandsTypeToLoad.Text)
				{
					if (commandTypesExpression.Length > 0)
						commandTypesExpression += " or ";
					commandTypesExpression += "self::" + MenuXmlNode.XML_TAG_TEXT;
				}
				if ((commandsTypeToSearch & MenuLoader.CommandsTypeToLoad.Exe) == MenuLoader.CommandsTypeToLoad.Exe)
				{
					if (commandTypesExpression.Length > 0)
						commandTypesExpression += " or ";
					commandTypesExpression += "self::" + MenuXmlNode.XML_TAG_EXE;
				}
				if ((commandsTypeToSearch & MenuLoader.CommandsTypeToLoad.ExternalItem) == MenuLoader.CommandsTypeToLoad.ExternalItem)
				{
					if (commandTypesExpression.Length > 0)
						commandTypesExpression += " or ";
					commandTypesExpression += "self::" + MenuXmlNode.XML_TAG_MENU_EXTERNAL_ITEM;
				}
				if ((commandsTypeToSearch & MenuLoader.CommandsTypeToLoad.OfficeItem) != 0)
				{
					if (commandTypesExpression.Length > 0)
						commandTypesExpression += " or ";
					commandTypesExpression += "self::" + MenuXmlNode.XML_TAG_OFFICE_ITEM;
					if ((commandsTypeToSearch & MenuLoader.CommandsTypeToLoad.ExcelItem) == MenuLoader.CommandsTypeToLoad.ExcelItem)
						commandTypesExpression += "[@" + XML_ATTRIBUTE_OFFICE_ITEM_APP +"='" + OfficeItemApplication.Excel.ToString() + "']" ;
					else if ((commandsTypeToSearch & MenuLoader.CommandsTypeToLoad.WordItem) == MenuLoader.CommandsTypeToLoad.WordItem)
	...
	//=================================================================================
	public class MenuXmlNodeException : ApplicationException 
	{
		private MenuXmlNode node = null;
