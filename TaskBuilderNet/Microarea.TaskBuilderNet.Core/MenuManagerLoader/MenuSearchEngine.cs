	...
	public class MenuSearchEngine : IDisposable
	{
		...
		private string ValidateExpression(string searchExpression)
		{
			...
			for(int i=0; i < expressionTokens.Length; i++)
			{
				...
				string token = expressionTokens[i].ToUpper(Thread.CurrentThread.CurrentUICulture);
		...
		private MenuXmlNodeCollection SearchSingleToken(MenuXmlNodeCollection setTosearch, string aTokenToSearch)
		{
			...
			if (!CaseSensitive)
				aTokenToSearch = aTokenToSearch.ToUpper(Thread.CurrentThread.CurrentUICulture);
			...
			foreach(MenuXmlNode node in setTosearch)
			{
				...
				{
					string aStringToSearch = CaseSensitive ? node.Title : node.Title.ToUpper(Thread.CurrentThread.CurrentUICulture);
				...
				if (SearchDescriptions && pathFinder != null)
				{
					...
					{
						if (!CaseSensitive)
							nodeDescription = nodeDescription.ToUpper(Thread.CurrentThread.CurrentUICulture);
                ...
                {
                    string aStringToSearch = CaseSensitive ? node.ItemObject : node.ItemObject.ToUpper(Thread.CurrentThread.CurrentUICulture);
					int foundIdx = aStringToSearch.IndexOf(aTokenToSearch);
					if (foundIdx != -1)
					{
						if 
