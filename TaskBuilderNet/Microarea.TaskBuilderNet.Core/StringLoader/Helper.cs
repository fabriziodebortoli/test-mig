...
namespace Microarea.TaskBuilderNet.Core.StringLoader
{
	...
	public class Helper
	{
		...
		public static DictionaryStringBlock RetrieveStrings(XmlDocument doc, string resourceName)
		{
			...
			foreach (XmlNode resource in resourceNodes)
			{
				...
				if (resourceName != null && el != null)
				{
					...
					if (string.Compare(name, resourceName, true, CultureInfo.InvariantCulture) != 0) 
						continue;
				}
		...
		private static bool ParseStringElement(XmlNode n, out string baseString, out string targetString)
		{
			...
			if (string.Compare(el.GetAttribute(Helper.validConst), "false", true, CultureInfo.InvariantCulture) == 0)
				return false;
