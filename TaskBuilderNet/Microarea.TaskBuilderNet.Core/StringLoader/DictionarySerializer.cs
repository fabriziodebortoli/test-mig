...
namespace Microarea.TaskBuilderNet.Core.StringLoader
{
	...
	public class DictionaryBinaryFile : Hashtable
	{
		...
		//--------------------------------------------------------------------------------
		public void AddDictionaryStringBlock(string type, string id, string name, DictionaryStringBlock block, bool lowerCase = true)
		{
			DictionaryBinaryIndexItem item = new DictionaryBinaryIndexItem(type, id, name, lowerCase);
	...
	public class DictionaryResourceIndexItem
	{
		...
		{
			this.Number = number;
			this.Url = Path.GetFileNameWithoutExtension(url).ToLower(CultureInfo.InvariantCulture);
		}
	...
	public class DictionaryBinaryIndexItem
	{
		...
		private string	id;
		private string	name;
		private static readonly string[] types = {"","databasescript","dialog","other","report","source","stringtable","xml","menu", "jsonforms", "web"};
		...
		//--------------------------------------------------------------------------------
		public DictionaryBinaryIndexItem(string	type, string id, string name, bool lowerCase = true)
		{
			this.type = TypeToUInt(type);
			this.id = lowerCase ? id.ToLower(CultureInfo.InvariantCulture) : id;
			this.name = lowerCase ? name.ToLower(CultureInfo.InvariantCulture) : name;
		}
