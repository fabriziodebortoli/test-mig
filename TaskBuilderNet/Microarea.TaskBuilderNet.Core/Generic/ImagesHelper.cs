...
namespace Microarea.TaskBuilderNet.Core.Generic
{
	...
	public static class ImagesHelper
	{
		...
		public static string TempImagesPath 
		{
			get
			{
				if (tmpImagesPath == "" && HttpContext.Current != null) 
		...
		/// </summary>
		private static ReaderWriterLock lockTicket = new ReaderWriterLock();
		...
		//--------------------------------------------------------------------------------------
		public static Bitmap LoadBitmapWithoutLockFile(string file)
		{
			Image image;
		...
		{
			lockTicket.AcquireWriterLock(Timeout.Infinite);
			try
			{
		...
		{
			lockTicket.AcquireReaderLock(Timeout.Infinite);
			try
			{
