...
namespace Microarea.TaskBuilderNet.Core.Generic
{
	...
	public class InstallationData
	{
		...
		private static BrandLoader brandLoader = null;
        ...
        private static GenericBrand genericBrand;
			...
			get
			{
				...
				lock (typeof(InstallationData))
				{
					...
					if (File.Exists(BasePathFinder.BasePathFinderInstance.ServerConnectionFile) &&
						...
						throw new ApplicationException(string.Format(Messages.ErrorReadingFile, BasePathFinder.BasePathFinderInstance.ServerConnectionFile));
		...
		//----------------------------------------------------------------------------
		public static BrandLoader BrandLoader
		{
			get
			{
				if (brandLoader == null)
        ...
        //----------------------------------------------------------------------------
        public static GenericBrand GenericBrand
        {
            get
            {
                if (genericBrand == null)
                {
                    lock (typeof(GenericBrand))
                    {
                        if (genericBrand == null)
                        {
                            genericBrand = GenericBrand.Load();
                        }
                    }
                }
                return genericBrand;
		...
		private static void CalculateInstallationInfo()
		{
			try
			{
				...
				if (!m.Success)
				{
					m = Regex.Match(AppDomain.CurrentDomain.BaseDirectory, pattern, RegexOptions.IgnoreCase);
					if (!m.Success)
						return;
				}
		...
		{
			return InternalGetInstalledDictionaries(AppDomain.CurrentDomain.BaseDirectory);
		}
