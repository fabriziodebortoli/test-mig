	...
	public class FontStylesGroup
	{
		...
		private FontElement	BestFontForContext (INameSpace context)
		{
			...
			foreach (FontElement font in fontStyles)
			{
				...
				if (
					string.Compare(font.Owner.Application, context.Application, true, CultureInfo.InvariantCulture) == 0 && 
		...
		private bool HasPriority (FontElement oldFont, FontElement newFont, INameSpace context, INameSpace moduleContext)
		{
			...
			return	forArea &&
				string.Compare(oldFont.Owner.FullNameSpace, newFont.Owner.FullNameSpace, true, CultureInfo.InvariantCulture) == 0 &&
	...
	public class FontStyles : Hashtable
	{
		...
		private bool ParseNewStyle (Parser lex)
		{
			...
			if (lex.LookAhead(Token.ID))
			{
				string id;
				lex.ParseID(out id);
				if (string.Compare(id, "name", true, CultureInfo.InvariantCulture) !=0 )
					return false;
			}
			if (!(
