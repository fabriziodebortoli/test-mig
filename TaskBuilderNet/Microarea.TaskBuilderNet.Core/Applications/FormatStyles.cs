...
namespace Microarea.TaskBuilderNet.Core.Applications
{
	...
	public class FormatStylesGroup
	{
		...
		private Formatter BestFormatterForContext(INameSpace context)
		{
			...
			foreach (Formatter formatter in formatStyles)
			{
				...
				if	(
						string.Compare(formatter.Owner.Application, context.Application, true, CultureInfo.InvariantCulture) == 0 && 
		...
		private bool HasPriority(Formatter oldFmt, Formatter newFmt, INameSpace context, INameSpace moduleContext)
		{
			...
			if (!forArea)
				...
				{	
					if	(
							string.Compare(context.FullNameSpace,		area, true, CultureInfo.InvariantCulture)	== 0 ||
							string.Compare(moduleContext.FullNameSpace,	area, true, CultureInfo.InvariantCulture)	== 0 ||
							string.Compare(context.Application,			area, true, CultureInfo.InvariantCulture)	== 0
						)
					{
						forArea = true;
						break;
			...
			return	forArea &&
					string.Compare(oldFmt.Owner.FullNameSpace, newFmt.Owner.FullNameSpace, true, CultureInfo.InvariantCulture) == 0 &&
	...
	public class LongFormatter : Formatter
	{
		...
		public override void RestoreFromLocale ()
		{
			...
			if (string.Compare(ThousSeparator, FormatStyles.ApplicationLocale.NumberFormat.NumberGroupSeparator, true, CultureInfo.InvariantCulture) == 0 && isThousSeparatorDefault)
				ThousSeparator = "";
		}
	}
	...
	{		
		...
		{
			return ch == char.ToLower(ch, CollateCulture);
		}
		...
		public string Format(string aValue, bool padEnabled)
		{
			...
			switch (FormatType)
			{
				...
				case FormatTag.UPPERCASE:
					result.Append(aValue.ToUpper(CollateCulture));
					break;
				case FormatTag.LOWERCASE:
					result.Append(aValue.ToLower(CollateCulture));
					break;
				case FormatTag.CAPITALIZED:
					...
					for(int i=0;i<aValue.Length;i++)
					{
						if	(
							...
							result.Append(char.ToUpper(aValue[i], CollateCulture));
						else 
	...
	{	
		...
		public virtual string Format(DateTime aValue, bool padEnabled)
		{
			...
			if (TimeSeparator != null)	customFormat.TimeSeparator = TimeSeparator;
	...
	public class FormatStyles : Hashtable
	{
            ...
            base(StringComparer.InvariantCultureIgnoreCase)
		{
			...
			applicationLocale = CultureInfo.CreateSpecificCulture(Thread.CurrentThread.CurrentCulture.Name);
		}
...
//				case Token.BLOB				:	fm = new BlobFormatter();				ok = fm.Parse(lex); break;
	...
	// parsa la release solo per gli stili di applicazione
		...
		private void UnparseFmtTimeAMPM(Unparser unparser, Formatter formatter)
		{
			...
			if (timeSeparator.CompareNoCase(FormatStyles.ApplicationLocale.DateTimeFormat.TimeSeparator))
				return;
	...
	public class FormatStyleLocale
	{
		...
		public static DateTimeFormatter.OrderTag ShortDateFormat 
		{
			get
			{
				string sep = FormatStyles.ApplicationLocale.DateTimeFormat.DateSeparator;
		...
		public static string DateSeparator 
		{
			get 
			{
				return FormatStyles.ApplicationLocale.DateTimeFormat.DateSeparator;
			}
		}
	}
}
