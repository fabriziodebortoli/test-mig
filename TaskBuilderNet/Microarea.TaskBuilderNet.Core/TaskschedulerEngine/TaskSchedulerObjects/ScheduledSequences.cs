...
{
    //============================================================================================
    public class WTETaskInScheduledSequence : IComparable
	{
		...
		//--------------------------------------------------------------------------------------------------------------------------------
		public WTETaskInScheduledSequence(WTEScheduledTaskObj aSequenceTask, Guid aTaskId, int aTaskInSequenceIndex, bool aBlockingMode, bool aTBLoaderConnectionNecessaryForRun, bool aCloseOnEndFlag)
		{
			if (aSequenceTask == null || !aSequenceTask.IsSequence)
			{
				throw new ScheduledTaskException(TaskSchedulerDBEngineStrings.InvalidSequenceOperationRequestMsg);
			}

			if (aTaskId == Guid.Empty)
			{
				throw new ScheduledTaskException(TaskSchedulerDBEngineStrings.UndefinedTaskIdMsg);
			}
		...
		//------------------------------------------------------------------------------------------------
		public WTETaskInScheduledSequence(WTETaskInScheduledSequence aTaskInSequence)
		{
		...
		{
			if (obj == null || !(obj is WTETaskInScheduledSequence))
				throw new ArgumentNullException();

			return taskInSequenceIndex - ((WTETaskInScheduledSequence)obj).taskInSequenceIndex;
		}

	}

	//============================================================================
	public class WTETasksInScheduledSequenceCollection : System.Collections.CollectionBase
	{
		// Restricts to WTETaskInScheduledSequence types, items that can be added to the collection
		//-------------------------------------------------------------------------------------------
		public void Add(WTETaskInScheduledSequence aTaskInSequence)
		{
		...
		//--------------------------------------------------------------------------------------------------------------------------------
		public WTETaskInScheduledSequence this[int index]
		{
			get 
			{
				// The appropriate item is retrieved from the List object and explicitly cast 
				// to the WTETaskInScheduledSequence type, then returned to the caller.
				return (WTETaskInScheduledSequence) List[index];
			}
			set
			{
				List.RemoveAt(index);
