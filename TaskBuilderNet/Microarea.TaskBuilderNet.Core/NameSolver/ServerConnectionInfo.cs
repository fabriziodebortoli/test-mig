    ...
    public sealed class ServerConnectionInfo : IServerConnectionInfo
    {
        ...
        public string SysDBConnectionString
        {
            get
            {
				...
				try
				{
					conn = AesCrypto.DecryptString(sysDBConnectionString);
				}
				catch (Exception)
				{
                    try
                    { //se arrivo qui la stringa di connessione o è compromessa o è quella vecchia, provo a leggerla con l'algoritmo vecchio, se funziona salvo il file con la stringa nuova 
                        conn = Crypto.Decrypt(sysDBConnectionString);
                        sysDBConnectionString = AesCrypto.EncryptString(conn);
                        UnParse(serverConnectionFile);
                    }
                    catch 
                    {
                        conn = string.Empty;
                        sysDBConnectionString = string.Empty;
                    }
                }
        ...
        public bool Parse(string aServerConnectionFile)
        {
            ...
            try
            {
                ...
                if (string.Compare(PreferredLanguage, "Native", true, CultureInfo.InvariantCulture) == 0)
                ...
                if (string.Compare(ApplicationLanguage, "Native", true, CultureInfo.InvariantCulture) == 0)
                ...
                if (verboseLogElements != null && verboseLogElements.Count == 1)
                {
                    try
                    {
                        EnableLMVerboseLog = String.Compare(
                            ...
                            Boolean.TrueString, StringComparison.InvariantCultureIgnoreCase) == 0 ? true : false;
        ...
        public bool UnParse(string aServerConnectionFile)
        {
            ...
            smtpUseDefaultCredentialsNode.InnerText = this.SMTPUseDefaultCredentials.ToString(System.Globalization.CultureInfo.InvariantCulture.GetFormat(typeof(bool)) as IFormatProvider);
            ...
            if (this.SMTPUseSSL)
            {
                ...
                smtpUseSSLNode.InnerText = this.SMTPUseSSL.ToString(System.Globalization.CultureInfo.InvariantCulture.GetFormat(typeof(bool)) as IFormatProvider);
            ...
            try
            {
                ...
                serverConnectionDocument.Save(ServerConnectionFile);
            }
            catch (Exception err)
            {
