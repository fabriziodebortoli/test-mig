	...
	public class DatabaseObjectsInfo : IDatabaseObjectsInfo
	{
		...
		public ITableInfo GetTableInfoByName(string tableName)
		{
			...
			foreach (ITableInfo ti in TableInfoArray)
				if (string.Compare(ti.Name, tableName, true, CultureInfo.InvariantCulture) == 0)
					return ti;
			return null;
		}
		...
		public IDbObjectInfo GetViewInfoByName(string viewName)
		{
            ...
            foreach (IDbObjectInfo wi in ViewInfoArray)
				if (string.Compare(wi.Name, viewName, true, CultureInfo.InvariantCulture) == 0) 
					return wi;
			return null;
		}
		...
		public IDbObjectInfo GetProcedureInfoByName(string procedureName)
		{
            ...
            foreach (IDbObjectInfo pi in ProcedureInfoArray)
				if (string.Compare(pi.Name, procedureName, true, CultureInfo.InvariantCulture) == 0)
					return pi;
			return null;
		}
		...
		//---------------------------------------------------------------------
		/// <summary>
		/// Legge il file e crea gli array di Tables, Views e Procedures in memoria
		/// </summary>
			...
			try
			{
				...
				if (signList != null && signList.Count > 0)
				{
					...
					if (!string.IsNullOrWhiteSpace(dmsAttribute) &&
						string.Compare(dmsAttribute, bool.TrueString, StringComparison.InvariantCultureIgnoreCase) == 0)
						dms = true;
				}
