...
namespace Microarea.TaskBuilderNet.Data.DatabaseLayer
{
	...
	public class UpdateInfo
	{
		...
		protected void ParseDependencyUpgrade(XmlElement xDep, int numLevel)
		{
			...
			if (string.IsNullOrEmpty(rel))
			{
				...
				int relFromDBMark = dbMarkInfo.GetDBReleaseFromDBMark(appSignature, modSignature);
				if (relFromDBMark != -1)
		...
		public void LoadSingleXML(CheckDBStructureInfo dbStructInfo, string xmlPath, out string error)
		{
			...
			if (xDoc == null || 
				string.Compare(xmlPath, xDoc.BaseURI.Substring(8).Replace("/", Path.DirectorySeparatorChar.ToString()), StringComparison.InvariantCultureIgnoreCase) != 0)
			{
				xDoc = new XmlDocument();
			...
			else
			{
				...
				if (string.Compare(xmlPath, path, StringComparison.InvariantCultureIgnoreCase) != 0) 
				{
					try
					{
						xDoc.Load(xmlPath);
		...
		private void GetSignatureForDependencyEntry(string application, string module, out string applSign, out string modSign)
		{
			...
			{
				if (string.Compare(addOnAppDBInfo.ApplicationName, application, StringComparison.InvariantCultureIgnoreCase) == 0)
				{
					foreach (ModuleDBInfo moduleDBInfo in addOnAppDBInfo.ModuleList)
					{
						if (string.Compare(moduleDBInfo.ModuleName, module, StringComparison.InvariantCultureIgnoreCase) == 0)
						{
		...
		private void GetReleaseForModuleSignature(string applicationSign, string moduleSign, out int release)
		{
			...
			foreach (AddOnApplicationDBInfo addOnAppDBInfo in addOnAppList)
			{
				foreach (ModuleDBInfo moduleDBInfo in addOnAppDBInfo.ModuleList)
				{
					if (string.Compare(moduleDBInfo.ApplicationSign, applicationSign, StringComparison.InvariantCultureIgnoreCase) == 0 &&
						string.Compare(moduleDBInfo.ModuleSign, moduleSign, StringComparison.InvariantCultureIgnoreCase) == 0)
					{
	...
	//============================================================================
	public class ModuleDBInfoList : ArrayList
	{
		/// <summary>
		/// costruttore ModuleDBInfoList che deriva da ArrayList
		/// </summary>
		//---------------------------------------------------------------------------
		...
		public ModuleDBInfo GetItem(string nameSpace, bool isChild)
		{
			...
			{
				if ((string.Compare(info.ApplicationSign, app, StringComparison.InvariantCultureIgnoreCase) == 0) &&
					(string.Compare(info.ModuleSign, module, StringComparison.InvariantCultureIgnoreCase) == 0))
				{
					if (!isChild)
