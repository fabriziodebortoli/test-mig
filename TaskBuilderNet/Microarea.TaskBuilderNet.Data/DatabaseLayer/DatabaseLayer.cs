...
namespace Microarea.TaskBuilderNet.Data.DatabaseLayer
{
	...
	/// </summary>
	//============================================================================
	public class TBException : ApplicationException
	{
		#region Constructors
		//---------------------------------------------------------------------------
		...
		public int Number
		{
			get
			{
                ...
                if (InnerException is NpgsqlException)
                    return ((NpgsqlException)InnerException).ErrorCode;
	...
	public class TBDatabaseType
	{
		...
		{
            if (string.Compare(provider, NameSolverDatabaseStrings.SQLOLEDBProvider, StringComparison.InvariantCultureIgnoreCase) == 0 ||
                string.Compare(provider, NameSolverDatabaseStrings.SQLODBCProvider, StringComparison.InvariantCultureIgnoreCase) == 0)
                return DBMSType.SQLSERVER;

            if (string.Compare(provider, NameSolverDatabaseStrings.OraOLEDBProvider, StringComparison.InvariantCultureIgnoreCase) == 0 ||
                string.Compare(provider, NameSolverDatabaseStrings.MSDAORAProvider, StringComparison.InvariantCultureIgnoreCase) == 0)
                return DBMSType.ORACLE;

            if (string.Compare(provider, NameSolverDatabaseStrings.PostgreOdbcProvider, StringComparison.InvariantCultureIgnoreCase) == 0)
                ...
                case DBMSType.POSTGRE:
                    {
                        ...
                        if (password.IsNullOrEmpty()) password = DatabaseLayerConsts.postgreDefaultPassword;
                        connectionString = (winNtAuthent)
        ...
        {
            if (dbmsType != DBMSType.POSTGRE) return null;
            TBType myTBType = null;
        ...
        public static TBType GetTBType(DataType dataType, DBMSType dbmsType)
        {
            ...
            if (dataType == DataType.String)
            {
                ...
                if (dbmsType == DBMSType.ORACLE) return new TBType(OracleType.NVarChar);
            ...
            if (dataType == DataType.Integer)
            {
                ...
                if (dbmsType == DBMSType.ORACLE) return new TBType(OracleType.Int16);
            ...
            if (dataType == DataType.Long || dataType == DataType.Enum)
            {
                ...
                if (dbmsType == DBMSType.ORACLE) return new TBType(OracleType.Int32);
                ...
                dataType == DataType.Quantity || dataType == DataType.Percent)
            {
                ...
                if (dbmsType == DBMSType.ORACLE) return new TBType(OracleType.Float);
            ...
            if (dataType == DataType.Date)
            {
                ...
                if (dbmsType == DBMSType.ORACLE) return new TBType(OracleType.DateTime);
            ...
            if (dataType == DataType.Text)
            {
                ...
                if (dbmsType == DBMSType.ORACLE) return new TBType(OracleType.NClob);
                if (dbmsType == DBMSType.POSTGRE) return new TBType(NpgsqlDbType.Varchar);
            }
            ...
            if (dataType == DataType.Blob)
            {
                ...
                if (dbmsType == DBMSType.ORACLE) return new TBType(OracleType.Blob);
            ...
            if (dbmsType == DBMSType.ORACLE) return new TBType(OracleType.VarChar);
            if (dbmsType == DBMSType.POSTGRE) return new TBType(NpgsqlDbType.Varchar);
	...
	public class TBCheckDatabase
	{
		...
		public static SQLServerEdition GetSQLServerEdition(SqlConnection dbConnection)
		{
			...
			{
				if (edition.IndexOf("Desktop Engine", StringComparison.InvariantCultureIgnoreCase) >= 0)
			...
			{
				if (edition.IndexOf("Express Edition", StringComparison.InvariantCultureIgnoreCase) >= 0)
            ...
            {
				if (edition.IndexOf("SQL Azure", StringComparison.InvariantCultureIgnoreCase) >= 0)
				...
				try
				{
					...
					using (SqlDataReader reader = command.ExecuteReader())
					{
						if (reader.Read())
						{
							...
							isMixedMode = string.Compare(isMixed, "0", StringComparison.InvariantCultureIgnoreCase) == 0;
						}
					}
				}
				catch (SqlException)
		...
		public static bool IsFullTextSearchInstalled(TBConnection tbConn)
		{
			...
			try
			{
				...
				if (reader.Read())
					isInstalled = (string.Compare(reader["IsFullTextInstalled"].ToString(), "1", StringComparison.InvariantCultureIgnoreCase) == 0);
			}
			catch (SqlException)
			{
			}
		...
		public static bool IsFullTextSearchEnabled(TBConnection tbConn)
		{
			...
			try
			{
				...
				if (reader.Read())
					isEnabled = (string.Compare(reader["IsFullTextEnabled"].ToString(), "1", StringComparison.InvariantCultureIgnoreCase) == 0);
			}
			catch (SqlException)
			{
			}
		...
		public static DatabaseCheckError CheckDatabaseVersion(TBConnection conn, DBNetworkType dbNetworkType)
		{
			...
			// Small network funziona solo con MSDE
            //NON PIù 6176
			//if (string.Compare(currentDB, DatabaseVersion.MSDE.ToString(), true, CultureInfo.InvariantCulture) != 0)
			//	return DatabaseCheckError.NoActivatedDatabase;

            if (InstallationData.CheckDBSize && conn.IsSqlConnection() && Functions.IsDBSizeOverMaxLimit(conn.SqlConnect))
		...
		public static DatabaseCheckError CheckDatabase
            (TBConnection conn, DBNetworkType dbNetworkType, IBasePathFinder basePathFinder, bool isDevelopment)
		{
			if (
				basePathFinder == null ||
				conn == null ||
			...
			/*(isDevelopment) 
				...
				: CheckDBRelease(conn, basePathFinder);*/
	...
	public class TBConnection : IDbConnection
	{
		...
		//---------------------------------------------------------------------------
		private void TryToAdjustConnectionString(SystemException exc)
		{
		...
		/// <summary>
		/// crea una connessione OLEDB considerando la stringa di connessione della corrente TBConnection
		/// aggiungendo le informazioni del provider
		/// </summary>
		...
		public OleDbConnection GetOleDbConnection()
		{
			...
			if (IsSqlConnection())
				provider = string.Format(NameSolverDatabaseStrings.ProviderConnAttribute, NameSolverDatabaseStrings.SQLOLEDBProvider);
			else if (IsOracleConnection())
				provider = string.Format(NameSolverDatabaseStrings.ProviderConnAttribute, NameSolverDatabaseStrings.OraOLEDBProvider);
			else
				return null;

			return new OleDbConnection(provider + completedConnectionString);
		}
	...
	public class TBParameter : IDataParameter
	{
		...
		{
			if (dbmsType == DBMSType.SQLSERVER)
				dbParameter = new SqlParameter(parameterName, dbType.SqlDbType, size, direction, isNullable, precision, scale, sourceColumn, sourceVersion, value);
			else if (dbmsType == DBMSType.ORACLE)
                ...
                dbParameter = new NpgsqlParameter(parameterName, TBDatabaseType.GetPostgreDbType(dbType), size, sourceColumn, direction, isNullable, precision, scale, sourceVersion, value);
            else
				throw (new TBException(string.Format(DatabaseLayerStrings.UnknownDBMS, dbmsType.ToString())));
		}
		#endregion
			...
			try
			{
				switch (this.SqlDbType)
				{
					...
					case (SqlDbType.NVarChar):
						...
						string readValue = System.Web.HttpUtility.HtmlDecode(value);
					...
					case (SqlDbType.NText):
						...
						this.Value = System.Web.HttpUtility.HtmlDecode(value);
						break;
