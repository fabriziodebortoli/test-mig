...
*/		
	...
	public class DirectGraph
	{
		...
		private Vertex GetVertexData(string label)
		{
			...
			{
				v = (Vertex)vertexList[i];
				if (string.Compare(v.Label, label, true, CultureInfo.InvariantCulture) == 0)
		...
		{	
			...
			for (int i = 0; i < SortedArray.Count; i++)
			{
				...
				if (string.Compare(sortedLabel, label, true, CultureInfo.InvariantCulture) == 0)
					return i; // il vertice ESISTE nell'array
			}
	...
	public class DependencyInfoList : ArrayList
	{
		...
		public DependencyInfo GetItem(string moduleName)
		{
			foreach (DependencyInfo info in this)
				if (string.Compare(info.AncestorName, moduleName, StringComparison.InvariantCultureIgnoreCase) == 0)
					return info;
