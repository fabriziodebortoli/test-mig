		...
		public string MakeExportQuery(CatalogTableEntry entry)
		{
			...
			if (!SelectColumns)
			{
				...
				else
				{
					...
					foreach (CatalogColumn col in entry.ColumnsInfo)
					{
						...
						// colonna TBModified
						if (string.Compare(col.Name, DatabaseLayerConsts.TBModifiedColNameForSql, StringComparison.InvariantCultureIgnoreCase) == 0 ||
			...
			else
			{
				...
				foreach (string column in entry.SelectedColumnsList)
				{
					...
					// TBModified
					if (string.Compare(column, DatabaseLayerConsts.TBModifiedColNameForSql, StringComparison.InvariantCultureIgnoreCase) == 0 ||
