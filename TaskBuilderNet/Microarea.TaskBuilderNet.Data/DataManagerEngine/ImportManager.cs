...
using static Microarea.TaskBuilderNet.Data.DataManagerEngine.ImportSelections;
...
namespace Microarea.TaskBuilderNet.Data.DataManagerEngine
{
	...
	public class ImportManager
	{
		...
		/// <summary>
		/// esecuzione del processo di import
		/// </summary>
		//---------------------------------------------------------------------
			...
			try
			{
				...
				foreach (ImportItemInfo item in importSel.ImportList)
				{
					...
					foreach (ImportItem ii in item.SelectedFiles)
					{
						...
						if (file.StartsWith("DeploymentManifest", StringComparison.InvariantCultureIgnoreCase))
							continue;

						if (!importSel.IsSilent &&
							Path.GetFileNameWithoutExtension(file).EndsWith(DataManagerConsts.Append, StringComparison.InvariantCultureIgnoreCase))
						{
		...
		private bool ImportSingleFile(FileInfo importFile)
		{
			...
			isExportDataFileName =
				string.Compare(importFile.Name, DataManagerConsts.ExportDataFileName, StringComparison.InvariantCultureIgnoreCase) == 0;
			...
			reader = new XmlTextReader(importFile.FullName);
			reader.WhitespaceHandling = WhitespaceHandling.None;

			appendFile = Path.GetFileNameWithoutExtension(importFile.Name).EndsWith(DataManagerConsts.Append, StringComparison.InvariantCultureIgnoreCase);
		...
		private bool LoadReferenceInformation(ref FileInfo importFile, string currentApplication, string currentModule)
		{
			...
			XmlTextReader xtr = new XmlTextReader(importFile.FullName);
						...
						if (xtr.Name.CompareTo(DataManagerConsts.DataTables) == 0)
						{
							if (xtr.MoveToAttribute(DataManagerConsts.HasReference))
							{
								...
								if (xtr.MoveToAttribute(DataManagerConsts.RefEdition))
									refEdition = string.IsNullOrEmpty(xtr.Value)
												? this.importSel.ContextInfo.PathFinder.Edition
												: xtr.Value;
								else
		...
		private DateTime GetDateTimeValue(TBParameter tbParam)
		{
            try
			{
                ...
                if (contextInfo.DbType == DBMSType.ORACLE)
                    colTbModified = DatabaseLayerConsts.TBModifiedColNameForOracle;
                else if (contextInfo.DbType == DBMSType.SQLSERVER)
                ...
                if (string.IsNullOrWhiteSpace(reader.Value))
                    return Microarea.TaskBuilderNet.Core.CoreTypes.ObjectHelper.NullTbDateTime;
		...
		private DateTime ConvertDateTimeValue(string value, bool emptyDate)
		{
			...
			{
				if (value.IndexOf(".") > 0)
					return XmlConvert.ToDateTime(value.Substring(0, value.IndexOf(".")), SQLDateTimeFormat);
			...
			if (value.IndexOf("-", tIdx) > 0) // cerco il - dopo la T
			{
				...
				return XmlConvert.ToDateTime(value.Substring(0, value.LastIndexOf("-")), SQLDateTimeFormat);
			}

			if (emptyDate)
				return XmlConvert.ToDateTime(reader.Value, SQLDateTimeFormat);
		...
		private bool SetParameters(TBCommand tbCommand)
		{
			...
			foreach (IDataParameter param in tbCommand.Parameters)
			{
				...
				if (!reader.MoveToAttribute(colName))
				{
                    ...
                    if (contextInfo.DbType == DBMSType.ORACLE)
                        tbModified = DatabaseLayerConsts.TBModifiedColNameForOracle;
                    ...
                    if (string.Compare(colName, tbModified, StringComparison.InvariantCultureIgnoreCase) == 0)
					{
					...
					dbDiagnostic.SetMessageNoAppAndModuleName
						...
						currentFileName,
						currentTable,
						string.Format(DataManagerEngineStrings.MsgNotPresentColumn, reader.LineNumber, colName),
						currentFullName
						);
				...
				if (
					tbParam.DbType == DbType.Date || 
					tbParam.DbType == DbType.Time || 
					tbParam.DbType == DbType.DateTime
					)
				{
					tbParam.Value = GetDateTimeValue(tbParam);
					continue;
				...
				if (string.Compare(colName, DatabaseLayerConsts.TBGuidColNameForSql, StringComparison.InvariantCultureIgnoreCase) == 0)
				{
					if (
						string.Compare(columnValue, "00000000-0000-0000-0000-000000000000", StringComparison.InvariantCultureIgnoreCase) == 0 ||
						string.Compare(columnValue, "00000000-0000-0000-0008-000000000000", StringComparison.InvariantCultureIgnoreCase) == 0
						)
		...
		{
			if (currentTable.Length == 0 || catalogEntry == null)
				return;
		...
		private bool ChangeTable()
		{
			try
			{
                ...
                {
                    EntryDBInfo entryDBInfo = moduleDBInfo.TablesList.Find(a => string.Compare(a.Name, currentTable, StringComparison.InvariantCultureIgnoreCase) == 0);
				...
				foreach (string key in tableKeys)
				{
					...
					while (next && !exist)
					{
						...
						exist = string.Compare(keyName, key, StringComparison.InvariantCultureIgnoreCase) == 0;
				...
				if (importSel.DeleteTableContext && !appendFile)
				{
		...
		private bool CheckRow()
		{
			...
			if (currentTable.Length == 0 || tagTableName.CompareTo(currentTable) != 0)
			{
					...
					if (reader.AttributeCount == insertCommand.Parameters.Count)
					{
						...
						if (PrepareInsertQuery())
						{
							foreach (IDataParameter param in insertCommand.Parameters)
							{
								...
								if (currentTableIsMaster && string.Compare(colName, DatabaseLayerConsts.TBGuidColNameForSql, StringComparison.InvariantCultureIgnoreCase) == 0)
									continue;
		...
		private bool InsertRow()
		{
			...
			catch (TBException e)
			{
				...
				errorBuilder.AppendFormat(DataManagerEngineStrings.MsgImportInsertRow, reader.LineNumber.ToString(), currentFullName);
				...
				if (errorBuilder.ToString().Length > 0)
				{
					...
					if (e.Number == 2627) // Cannot insert duplicate key in object 
						dbDiagnostic.SetMessageNoAppAndModuleName
							...
							currentFileName,
							currentTable,
							string.Format(DataManagerEngineStrings.MsgDuplicateKey, reader.LineNumber),
							currentFullName
							);
					else
						dbDiagnostic.SetMessageNoAppAndModuleName
							...
							currentFileName,
							currentTable,
							string.Format(DataManagerEngineStrings.MsgErrorAtLineNumber, e.Message, reader.LineNumber),
							currentFullName
							);
		...
		private bool PrepareInsertQuery()
		{
			...
			while (exist)
			{
                ...
                if (contextInfo.DbType == DBMSType.ORACLE)
                    tbCreated = DatabaseLayerConsts.TBCreatedColNameForOracle;
                else if (contextInfo.DbType == DBMSType.POSTGRE)
                    tbCreated = DatabaseLayerConsts.TBCreatedColNameForPostgre;
                else
                    tbCreated = DatabaseLayerConsts.TBCreatedColNameForSql;
                if (string.Compare(colName, tbCreated, StringComparison.InvariantCultureIgnoreCase) == 0)
				{
		...
		private bool UpdateRow()
		{
			...
			if (updateCommand.CommandText.Length == 0)
			{
				fromUpdate = false;
				return true;
			}
		...
		private bool PrepareUpdateQuery()
		{
			...
			while (exist)
			{
                ...
                if (contextInfo.DbType == DBMSType.ORACLE)
                    updateColNameTb = DatabaseLayerConsts.TBCreatedColNameForOracle;
                else if (contextInfo.DbType == DBMSType.SQLSERVER)
                    updateColNameTb = DatabaseLayerConsts.TBCreatedColNameForSql;
                else if (contextInfo.DbType == DBMSType.POSTGRE)
                    updateColNameTb = DatabaseLayerConsts.TBCreatedColNameForPostgre;
				// colonna TBCreated
				if (string.Compare(colName, updateColNameTb, StringComparison.InvariantCultureIgnoreCase) == 0)
				{
					if (!importSel.ImportTBCreated)
					{
						exist = reader.MoveToNextAttribute();
                ...
                if (contextInfo.DbType == DBMSType.ORACLE)
                    updateColNameTb = DatabaseLayerConsts.TBModifiedColNameForOracle;
                else if (contextInfo.DbType == DBMSType.SQLSERVER)
		...
		private bool PrepareUpdateTBGuidQuery()
		{
			...
			while (exist)
			{
				...
				if (string.Compare(colName, DatabaseLayerConsts.TBGuidColNameForSql, StringComparison.InvariantCultureIgnoreCase) == 0)
				{
					...
					if (
						string.Compare(tbGuidValueInXml, "00000000-0000-0000-0000-000000000000", StringComparison.InvariantCultureIgnoreCase) == 0 ||
						string.Compare(tbGuidValueInXml, "00000000-0000-0000-0008-000000000000", StringComparison.InvariantCultureIgnoreCase) == 0
						)
		...
		/// si usa GetDate() per SQLServer e sysdate per Oracle
		/// </summary>
		//---------------------------------------------------------------------------
		private void SynchronizeDBServerDateTime()
		{
			...
			TBCommand comm = new TBCommand(query, connection);
			...
			if (!connection.DataReaderHasRows(reader))
			{
				reader.Close();
				reader.Dispose();
			}
