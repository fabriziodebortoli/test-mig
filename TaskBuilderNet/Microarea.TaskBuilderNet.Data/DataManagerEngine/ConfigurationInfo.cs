	...
	*/
	...
	public class ConfigurationInfo
	{
					...
					if (entry.Selected)
					{
						...
						if (selections.ExportSel.SelectColumns)
						{
							foreach (string column in entry.SelectedColumnsList)
							{
								...
								if (string.Compare(column, DatabaseLayerConsts.TBCreatedColNameForSql, StringComparison.InvariantCultureIgnoreCase) == 0 ||
									string.Compare(column, DatabaseLayerConsts.TBModifiedColNameForSql, StringComparison.InvariantCultureIgnoreCase) == 0 ||
									string.Compare(column, DatabaseLayerConsts.TBCreatedIDColNameForSql, StringComparison.InvariantCultureIgnoreCase) == 0 ||
									string.Compare(column, DatabaseLayerConsts.TBModifiedIDColNameForSql, StringComparison.InvariantCultureIgnoreCase) == 0)
									continue;
		...
		public bool ExistApplicationInConfigInfo(string application)
		{
			...
			{
				if (string.Compare(appNode.Name, application, StringComparison.InvariantCultureIgnoreCase) == 0)
					return true;
			}
		...
		public bool ExistModuleInConfigInfo(string application, string module)
		{
			...
			foreach (ApplicationNode appNode in ExportedObjectsArray)
			{
				...
				{
					if (string.Compare(modNode.Name, module, StringComparison.InvariantCultureIgnoreCase) == 0)
						return true;
				}
			}
		...
		public bool ExistTableInConfigInfo(string application, string module, string table)
		{
			...
			foreach (ApplicationNode appNode in ExportedObjectsArray)
			{
				...
				foreach (ModuleNode modNode in appNode.Modules)
				{
					...
					{
						if (string.Compare(tblNode.Name, table, StringComparison.InvariantCultureIgnoreCase) == 0)
							return true;
					}
				}
			}
