...
namespace Microarea.TaskBuilderNet.Data.PostgreDataAccess
{
	...
	/// </summary>
	// ========================================================================
	public partial class PostgreCredential : System.Windows.Forms.Form
	{
		...
		/// </summary>
		//---------------------------------------------------------------------
		public PostgreCredential(string serverName, string login, string password, string domain, bool winAuth, bool enableChangeCredential, int port)
		{
			InitializeComponent();

			this.Text = string.Format(PostgreDataAccessStrings.ConnectionTo, serverName);
			BtnOK.Enabled		= false;
			...
			{
				toolTip.SetToolTip(TbLogin, string.Format(PostgreDataAccessStrings.ToolTipLogin, serverName));
				toolTip.SetToolTip(TbPassword, string.Format(PostgreDataAccessStrings.ToolTipPassword, login));
			}
		}
		...
		/// </summary>
		//---------------------------------------------------------------------
		public PostgreCredential(string login, string password, string domain, bool windowsAuthentication, int port)
		{
			InitializeComponent();
			userImpersonated.Login				   = login;
			userImpersonated.Password			   = password;
		...
		/// </summary>
		//---------------------------------------------------------------------
		public PostgreCredential(string login, string password, string domain, bool winAuth, bool silent /*true*/, int port)
		{
			InitializeComponent();
			userImpersonated.Login				   = login;
			userImpersonated.Password			   = password;
		...
		private void BtnOK_Click(object sender, System.EventArgs e)
		{
			...
			if (radioCurrentCredential.Checked)
			{
				...
				if (RadioWindowsAuthentication.Checked)
				{
					...
					{
						diagnosticViewer.Message = PostgreDataAccessStrings.EmptyUserDomain;
						diagnosticViewer.Title = PostgreDataAccessStrings.Credential;
					...
					else
					{
						...
						{
							diagnosticViewer.Message = PostgreDataAccessStrings.WrongCredential;
							diagnosticViewer.Title = PostgreDataAccessStrings.Credential;
							diagnosticViewer.ShowButtons = MessageBoxButtons.OK;
							diagnosticViewer.ShowIcon    = MessageBoxIcon.Warning;
							diagnosticViewer.Show();
		...
		/// </summary>
		//---------------------------------------------------------------------
		private bool SuccessConnection(string loginName, string loginPassword, PostgreAccess tentativeConnSql)
		{
			bool successConnection = false;

			tentativeConnSql.CurrentStringConnection =
                string.Format(NameSolverDatabaseStrings.PostgreConnection, this.serverName, port, DatabaseLayerConsts.postgreMasterDatabase,  loginName.ToLower(), loginPassword, DatabaseLayerConsts.postgreDefaultSchema);
		...
		*/
		...
		private void BtnHelp_Click(object sender, System.EventArgs e)
		{
			if (OnOpenHelpFromPopUp != null)
				OnOpenHelpFromPopUp(this, "Microarea.TaskBuilderNet.Data.PostgresAccess.Credential");
		}
		#endregion
	}
}